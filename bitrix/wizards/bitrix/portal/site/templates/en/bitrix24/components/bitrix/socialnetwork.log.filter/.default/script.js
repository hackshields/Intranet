...
function __logOnReload(log_counter)
{
	...
	{
		var arTabs = BX.findChildren(BX("preset_filters"), { className: 'pagetitle-but-wrap' }, false);
		for (var i = 0; i < arTabs.length; i++)
		{
			if (arTabs[i].id == 'preset_filter_all')
				BX.addClass(arTabs[i], 'pagetitle-but-act');
			else
				BX.removeClass(arTabs[i], 'pagetitle-but-act');
		}
	}
