...
if (!is_array($arResult['USERS']) || !($USERS_CNT = count($arResult['USERS']))):
	...
	{
		$arDeptsChain[$arDept['DEPTH_LEVEL']] = array("NAME"=>htmlspecialcharsbx($arDept['NAME']), "ID"=>$arDept['ID']);//'<a class="breadcrumbs-item" href="'.$arParams['STRUCTURE_PAGE'].'?set_filter_'.$arParams['STRUCTURE_FILTER'].'=Y&'.$arParams['STRUCTURE_FILTER'].'_UF_DEPARTMENT='.$arDept['ID'].'">'.htmlspecialcharsbx($arDept['NAME']).'<i></i></a>';
