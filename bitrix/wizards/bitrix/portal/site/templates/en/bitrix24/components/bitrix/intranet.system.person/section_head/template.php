...
<div class="department-manager">
	<div class="department-titles"><?=GetMessage("INTR_STR_UF_HEAD")?></div>
		<span class="department-manager-info-block">
			<a class="department-manager-avatar" href="<?=$arUser['DETAIL_URL']?>">
				<?if (!empty($arUser["PERSONAL_PHOTO"])):?><img src="<?=$arUser["PERSONAL_PHOTO"]?>"/><?endif?>
			</a>
...
function user_action_menu<?=$user_action_menu_number?> (button, number, user_id, is_extranet) {
	BX.PopupMenu.show('more-action-menu'+number, button, [
		...
		<?if ($arResult['CAN_EDIT_USER']):?>
			<?if ($arUser["ID"] != $USER->GetID() && !(IsModuleInstalled("bitrix24") && $arUser["ID"] === "1")):?>
