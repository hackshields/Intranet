...
$iblockID = COption::GetOptionInt("intranet", "iblock_structure"); 
$rsSections = CIBlockSection::GetList(array(), array( "IBLOCK_ID" => $iblockID, "SECTION_ID" => $current_section), false, array("ID", "NAME", "SECTION_PAGE_URL", "UF_HEAD"));
...
if ($arParams['SHOW_SECTION_INFO'] == 'Y' && $current_section)
{
	...
	//uf_head:
		$dbUser = CUser::GetList($b="", $o="", array("ID" => intval($arResult['SECTION_DATA']["UF_HEAD"]), "ACTIVE" => "Y"), array('FIELDS' => array("ID", "NAME", "LAST_NAME", "SECOND_NAME", "LOGIN", "EMAIL", "PERSONAL_PHOTO", "PERSONAL_MOBILE", "WORK_POSITION", "LAST_ACTIVITY_DATE", "LAST_LOGIN"), 'SELECT' => array("UF_SKYPE")));
...
while($arUser = $rsUsers->Fetch())  
	if ($arUser["ID"] != $arResult['SECTION_DATA']["UF_HEAD"]["ID"]) $userNum++;
