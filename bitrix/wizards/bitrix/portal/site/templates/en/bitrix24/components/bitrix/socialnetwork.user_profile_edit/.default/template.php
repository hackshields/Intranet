...
if (IsModuleInstalled("bitrix24"))
{
	$arFields = array(
		'MAIN' => array(
			/*'EMAIL', 'PASSWORD', 'CONFIRM_PASSWORD',*/ 'NAME', 'LAST_NAME', 'SECOND_NAME', 'PERSONAL_PHOTO',  'PERSONAL_PHONE', 'PERSONAL_MOBILE', 'WORK_PHONE', 'UF_PHONE_INNER', 'WORK_POSITION', 'GROUP_ID',
		),

		'PERSONAL' => array(
			'AUTO_TIME_ZONE', 'TIME_ZONE',  'PERSONAL_WWW', 'PERSONAL_CITY', 'PERSONAL_GENDER', 'PERSONAL_BIRTHDAY',
			'UF_SKYPE', 'UF_WEB_SITES', 'UF_SKILLS', 'US_INTERESTS',
		),
	);
...
else
{
	$arFields = array(
		'MAIN' => array(
			/*'EMAIL', 'PASSWORD', 'CONFIRM_PASSWORD',*/ 'NAME', 'LAST_NAME', 'SECOND_NAME', 'PERSONAL_PHOTO',  'PERSONAL_PHONE', 'PERSONAL_MOBILE', 'WORK_PHONE', 'WORK_POSITION', 'GROUP_ID',
		),
...
if ($UF_DEP && $arResult["User"]["IS_EXTRANET"])
...
<?foreach ($arFields as $GROUP_ID => $arGroupFields):?>
	<tr>
		<td class="content-edit-form-header content-edit-form-header-first" colspan="3" >
			<div class="content-edit-form-header-wrap <?if ($GROUP_ID == "MAIN"):?>content-edit-form-header-wrap-blue<?endif;?>"><?=GetMessage("SOCNET_SUPE_TPL_GROUP_".$GROUP_ID)?></div>
		</td>
	</tr>
	<?if ($GROUP_ID == "MAIN"):?>
		...
		<?if (in_array('EMAIL', $arParams['EDITABLE_FIELDS'])):?>
			<tr>
				<td class="content-edit-form-field-name"><?=GetMessage("ISL_EMAIL")?></td>
	...
	<tr id="<?=$FIELD?>" <?if ((in_array($FIELD, array("UF_TWITTER", "UF_FACEBOOK", "UF_LINKEDIN", "UF_XING")) || IsModuleInstalled("bitrix24") && in_array($FIELD, array("PERSONAL_MOBILE", "WORK_PHONE", "UF_PHONE_INNER"))) && empty($value) || $FIELD == "GROUP_ID" && $arResult["User"]["IS_EXTRANET"]):?>style="display:none"<?endif;?>>
		...
		switch ($FIELD)
		{
			...
			case "AUTO_TIME_ZONE":
				...
				<select name="AUTO_TIME_ZONE" onchange="this.form.TIME_ZONE.disabled=(this.value != 'N')" class="content-edit-form-field-input-select">
			...
			case "PERSONAL_PHOTO":
				...
				$APPLICATION->IncludeComponent('bitrix:main.file.input', '', array(
					'INPUT_NAME' => 'PERSONAL_PHOTO_ID',
					'INPUT_NAME_UNSAVED' => 'PERSONAL_PHOTO_ID_UNSAVED',
					'CONTROL_ID' => 'PERSONAL_PHOTO_IMAGE_ID',
					'INPUT_VALUE' => $arResult["User"]["PERSONAL_PHOTO_FILE"]["ID"],
					'MULTIPLE' => 'N',
					'ALLOW_UPLOAD' => 'I',
				...
				function onFileUploaderChangeHandler(files) {
					var photoId = document.forms["user_profile_edit"].elements["PERSONAL_PHOTO_ID"];
					var photoDel = document.forms["user_profile_edit"].elements["PERSONAL_PHOTO_del"];
					var photoCID = document.forms["user_profile_edit"].elements["PERSONAL_PHOTO_CID"];

					if (files && files.length > 0)
					{
						photoDel.value = "";
						BX('sonet_profile_user_photo').src = files[0].fileURL;
						BX("sonet_profile_container_user_photo").style.display = "block";
						photoCID.value = window.FILE_INPUT_PERSONAL_PHOTO_IMAGE_ID.CID;
					}
					else
					{
						if (!photoId)
						...
						BX("sonet_profile_container_user_photo").style.display = "none";
					}
				}
				BX.addCustomEvent(window.FILE_INPUT_PERSONAL_PHOTO_IMAGE_ID, 'onFileUploaderChange', onFileUploaderChangeHandler);
				</script>

				<input type="hidden" name="PERSONAL_PHOTO_CID" value="" />
				<input type="hidden" name="PERSONAL_PHOTO_del" value="" />
				<div id="sonet_profile_container_user_photo" class="content-edit-form-field-photo" <?if (!isset($arResult["User"]["PERSONAL_PHOTO_FILE"]["SRC"])):?>style="display:none;"<?endif;?>>
					<div class="content-edit-form-field-photo-image">
						<img id="sonet_profile_user_photo" src="<?=$arResult["User"]["PERSONAL_PHOTO_FILE"]["SRC"]?>" width="41">
					</div>
				</div>
			...
			case 'GROUP_ID':?>
				...
				<?if (!$arResult["User"]["IS_EXTRANET"]):?>
					<div class="content-edit-form-field-input-sub"><label><input type="checkbox" class="content-edit-form-field-input-selector" id="group_admin" name="GROUP_ID[]" <?if ($arResult["IsMyProfile"] || (IsModuleInstalled("bitrix24") && $arResult["User"]["ID"] == "1")):?> readonly onclick="return false"<?else:?> onchange="modifyGroups()" <?endif;?> value="1" <?if ($arResult['User']['IS_ADMIN']) echo "checked"?> ><span class="content-edit-form-field-input-selector-name"><?=GetMessage("ISL_GROUP_ADMIN")?></span></label></div>
				<?
				endif;
				...
				<td class="content-edit-form-field-input" colspan="2">
					<select name="UF_DEPARTMENT[]" size="5" multiple="multiple" >
						...
						while($arDepartment = $rsDepartments->GetNext()):
							?><option value="<?echo $arDepartment["ID"]?>" <?if(in_array($arDepartment["ID"], $value)) echo "selected"?>><?echo str_repeat("&nbsp;.&nbsp;", $arDepartment["DEPTH_LEVEL"])?><?echo $arDepartment["NAME"]?></option><?
						endwhile;
						?>
					</select>
				</td>
		...
		<?if (IsModuleInstalled("bitrix24") && $FIELD == "PERSONAL_PHOTO" && !($arResult['User']["UF_PHONE_INNER"] && $arResult['User']["PERSONAL_MOBILE"] &&  $arResult['User']["WORK_PHONE"])): // for phones link?>
	<tr>
		<td class="content-edit-form-field-name"></td>
...
if (count($arSocialFields) > 0)
{
	...
	foreach ($arSocialFields as $FIELD):
		?><tr id="<?=$FIELD?>" <?if (empty($arResult["User"][$FIELD])):?>style="display:none"<?endif;?>>
			<td class="content-edit-form-field-name"><?=GetMessage("ISL_".$FIELD)?></td>
			<td class="content-edit-form-field-input" colspan="2">
...
{
	if (confirm("<?=$arResult["User"]["ACTIVE"] == "Y" ? GetMessage("SOCNET_CONFIRM_FIRE") : GetMessage("SOCNET_CONFIRM_RECOVER")?>"))
		return true;
	else
...
function bxUpeToggleHiddenField(tableId, fieldId)
{
...
function ShowAddSocnet(bindElement)
{
	BX.PopupMenu.show('socnet_add', bindElement, [
		...
		{ text : "<?=GetMessage("ISL_UF_TWITTER")?>", className : "menu-popup-no-icon", onclick : function() { BX("UF_TWITTER").style.display=""; this.popupWindow.close();} },
		<?endif?>
		<?if (in_array("UF_FACEBOOK", $arSocialFields)):?>
		{ text : "<?=GetMessage("ISL_UF_FACEBOOK")?>", className : "menu-popup-no-icon", onclick : function() { BX("UF_FACEBOOK").style.display=""; this.popupWindow.close();}},
		<?endif?>
		<?if (in_array("UF_LINKEDIN", $arSocialFields)):?>
		{ text : "<?=GetMessage("ISL_UF_LINKEDIN")?>", className : "menu-popup-no-icon", onclick : function() { BX("UF_LINKEDIN").style.display=""; this.popupWindow.close();} },
		<?endif?>
		<?if (in_array("UF_XING", $arSocialFields)):?>
		{ text : "<?=GetMessage("ISL_UF_XING")?>", className : "menu-popup-no-icon",  onclick : function() { BX("UF_XING").style.display=""; this.popupWindow.close();} }
		<?endif?>
	],
	{
	offsetTop:10,
...
function ShowAddPnone(bindElement)
{
	BX.PopupMenu.show('phone_add', bindElement, [
		{ text : "<?=GetMessage("ISL_PERSONAL_MOBILE")?>", className : "menu-popup-no-icon", onclick : function() { BX("PERSONAL_MOBILE").style.display=""; this.popupWindow.close();} },
		{ text : "<?=GetMessage("ISL_WORK_PHONE")?>", className : "menu-popup-no-icon", onclick : function() { BX("WORK_PHONE").style.display=""; this.popupWindow.close();}},
		{ text : "<?=GetMessage("ISL_UF_PHONE_INNER")?>", className : "menu-popup-no-icon", onclick : function() { BX("UF_PHONE_INNER").style.display=""; this.popupWindow.close();} }
	],
	{
	offsetTop:10,
	offsetLeft:30,
