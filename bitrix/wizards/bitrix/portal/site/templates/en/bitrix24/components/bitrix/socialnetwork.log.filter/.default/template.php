...
else
{
	?><script type="text/javascript">
		...
		function showLentaMenu(bindElement)
		{
			...
			BX.PopupMenu.show("lenta-sort-popup", bindElement, [
				{ text : "<?=GetMessageJS("SONET_C30_PRESET_FILTER_ALL")?>", className : "lenta-sort-item<?=(!$arResult["PresetFilterActive"] ? " lenta-sort-item-selected" : "")?>", href : "<?=CUtil::JSEscape($GLOBALS["APPLICATION"]->GetCurPageParam("preset_filter_id=clearall", array("preset_filter_id", "set_follow_type")))?>" },
			<?
			$buttonName = false;
			if (is_array($arResult["PresetFilters"]))
			{
				foreach($arResult["PresetFilters"] as $preset_filter_id => $arPresetFilter)
				{
					...
					?>{ text : "<?=$arPresetFilter["NAME"]?>", className : "lenta-sort-item<?=($arResult["PresetFilterActive"] == $preset_filter_id ? " lenta-sort-item-selected" : "")?>", href : "<?=CUtil::JSEscape($GLOBALS["APPLICATION"]->GetCurPageParam("preset_filter_id=".$preset_filter_id, array("preset_filter_id", "set_follow_type")))?>" },<?
				}
			}
			?>
				{ delimiter : true },
				{ text : "<?=GetMessageJS("SONET_C30_T_FILTER_TITLE")?>...", className : "lenta-sort-item<?=($isFiltered ? " lenta-sort-item-selected" : "")?>", onclick: function() { this.popupWindow.close(); ShowFilterPopup(BX("lenta-sort-button")); } },
				{ delimiter : true },
				{ text : "<?=GetMessageJS("SONET_C30_SMART_FOLLOW")?>", className : "lenta-sort-item<?=($arResult["FOLLOW_TYPE"] == "N" ? " lenta-sort-item-selected" : "")?>", href : "<?=CUtil::JSEscape($GLOBALS["APPLICATION"]->GetCurPageParam("set_follow_type=".($arResult["FOLLOW_TYPE"] == "Y" ? "N" : "Y"), array("set_follow_type")))?>"  }
			],
			{
				offsetTop:2,
				offsetLeft : 43,
	...
	<a href="" id="lenta-sort-button" class="lenta-sort-button" onclick="return showLentaMenu(this);" onmousedown="BX.addClass(this, 'lenta-sort-button-press')" onmouseup="BX.removeClass(this,'lenta-sort-button-press')"><span class="lenta-sort-button-left"></span><span class="lenta-sort-button-text"><?=($buttonName !== false ? $buttonName : GetMessage("SONET_C30_PRESET_FILTER_ALL") )?><?=($isFiltered ? " (".GetMessageJS("SONET_C30_T_FILTER_TITLE").")" : "")?><?if ($buttonName === false):?><span id="sonet_log_counter_preset" class="pagetitle-but-counter"><?=((intval($arResult["LOG_COUNTER"]) > 0 && $arParams["ENTITY_TYPE"] != SONET_ENTITY_GROUP) ? $arResult["LOG_COUNTER"] : "")?></span><?endif?></span><span class="lenta-sort-button-right"></span></a>
