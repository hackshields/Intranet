...
$arParams["LIST_URL_SEARCH"] = $arParams["LIST_URL"];
global $USER;
if ($USER->CanDoOperation('edit_all_users'))
	$arParams["LIST_URL_SEARCH"].= "?show_user=all";
?>
<div class="employee-search-block">
	<?if($USER->CanDoOperation('bitrix24_invite') && CModule::IncludeModule('bitrix24') && $arParams['TABLE_VIEW']!='group_table'):?>
		<a class="webform-small-button webform-small-button-accept" href="javascript:void(0)" onclick="<?=CBitrix24InviteDialog::ShowInviteDialogLink()?>"><span class="webform-small-button-left"></span><span class="webform-small-button-icon employee-invite-icon"></span><span class="webform-small-button-text"><?=GetMessage("INTR_COMP_IS_TPL_TOOLBAR_USER_INVITE")?></span><span class="webform-small-button-right"></span></a>
	<?elseif (!IsModuleInstalled("bitrix24") && $USER->CanDoOperation('edit_all_users') && $arParams['TABLE_VIEW']!='group_table'):?>
		<a class="webform-small-button webform-small-button-accept" href="javascript:void(0)" onclick="<?=$APPLICATION->GetPopupLink(array(
			...
			));?>"><span class="webform-small-button-left"></span><span class="webform-small-button-icon employee-invite-icon"></span><span class="webform-small-button-text"><?=GetMessage("INTR_COMP_IS_TPL_TOOLBAR_USER_ADD")?></span><span class="webform-small-button-right"></span></a>
	<?endif;?>
...
<script type="text/javascript">
	...
	function more_action_popup (){
		...
		BX.PopupMenu.show('more-action-menu', BX('filter-but-more'), [
			{ text : "<?=GetMessage('INTR_COMP_IS_TPL_TOOLBAR_EXCEL')?>", className: "menu-popup-no-icon", href : "<?=$APPLICATION->GetCurPageParam('current_view=table&excel=yes', array('excel', 'current_view'))?>"},
