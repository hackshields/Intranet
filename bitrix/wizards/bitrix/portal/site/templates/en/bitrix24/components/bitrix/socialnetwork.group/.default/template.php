...
else
{
		...
		<div class="bx-group-sidebar-block-inner"><?
					...
					<div class="bx-group-users-inner">
						...
						<div class="bx-group-users-list">
							<div class="bx-group-user">
								<a href="<?=htmlspecialcharsback($arResult["Owner"]["USER_PROFILE_URL"])?>" class="bx-group-user-avatar"<?if (strlen($arResult["Owner"]["USER_PERSONAL_PHOTO_FILE"]["SRC"]) > 0):?> style="background:url('<?=$arResult["Owner"]["USER_PERSONAL_PHOTO_FILE"]["SRC"]?>') no-repeat center center;"<?endif?>></a>
			...
			?><table cellspacing="0" class="bx-group-layout">
				<tr>
					...
					<td class="bx-group-layout-column bx-group-layout-right-column"><?=FormatDateFromDB($arResult["Group"]["DATE_CREATE"])?></td>
				</tr>
				<tr>
				...
				?><div class="bx-group-users bx-group-moderator">
					<div class="bx-group-users-inner">
						...
						foreach ($arResult["Moderators"]["List"] as $friend)
						{
							...
							?><div class="bx-group-user">
								<a href="<?=htmlspecialcharsback($friend["USER_PROFILE_URL"])?>" class="bx-group-user-avatar"<?if (strlen($friend["USER_PERSONAL_PHOTO_FILE"]["SRC"]) > 0):?>style="background:url('<?=$friend["USER_PERSONAL_PHOTO_FILE"]["SRC"]?>') no-repeat center center;"<?endif?>></a>
								<div class="bx-group-user-info">
				...
				?><div class="bx-group-users bx-group-member">
					<div class="bx-group-users-inner">
						...
						if ($GLOBALS["USER"]->IsAuthorized() && $arResult["CurrentUserPerms"]["UserCanInitiate"] && !$arResult["HideArchiveLinks"]):
							?><a class="webform-field-action-link" href="<?=htmlspecialcharsback($arResult["Urls"]["GroupRequestSearch"])?>" onclick="InvitePopupGroup(<?=$arParams["GROUP_ID"]?>, '<?=CUtil::JSEscape($arResult["Group"]["NAME"])?>', event)"><?=GetMessage("SONET_C6_ACT_REQU")?></a><?
						endif;
						?></div>
						<div class="bx-group-users-list"><?
	...
	$arContentFilter = array("!ITEM_ID" => "G".$arParams["GROUP_ID"]);
