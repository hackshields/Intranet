...
global $arSocNetFeaturesSettings, $CACHE_MANAGER;
...
$arParams["PATH_TO_FILES"] = (isset($arParams["PATH_TO_FILES"]) ? $arParams["PATH_TO_FILES"] : SITE_DIR."company/personal/user/#user_id#/files/lib/");
...
if (CModule::IncludeModule("bizproc") && $GLOBALS["USER"]->IsAuthorized() && CBXFeatures::IsFeatureEnabled("BizProc"))
...
if ($GLOBALS["USER"]->IsAuthorized())
{
	...
	$arResult["USER_NAME_FORMATTED"] = CUser::FormatName(CSite::GetNameFormat(false), $arUser, true);
		...
		$CACHE_MANAGER->RegisterTag("USER_CARD_".intval($arUser['ID'] / 100));
	...
	if (!is_array($GLOBALS["LEFT_MENU_COUNTERS"]))
	{
		$arLogCounters = Array();
		if (CModule::IncludeModule("socialnetwork"))
			$arLogCounters = CSocNetLogCounter::GetCodeValuesByUserID($GLOBALS["USER"]->GetID(), SITE_ID);
		$GLOBALS["LEFT_MENU_COUNTERS"] = is_array($arLogCounters) ? $arLogCounters : Array();	
	}
	$couterId = "**";
	$arResult["LOG_ITEMS_TOTAL"] = isset($GLOBALS["LEFT_MENU_COUNTERS"]) && array_key_exists($couterId, $GLOBALS["LEFT_MENU_COUNTERS"]) ? $GLOBALS["LEFT_MENU_COUNTERS"][$couterId] : 0;
