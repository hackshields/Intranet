...
foreach ($arCourses as $key => $couseCode)
{
	...
	{
		$pathToCourse = WIZARD_SERVICE_RELATIVE_PATH."/".LANGUAGE_ID."/".$couseCode."/";
		...
		if ($success)
		{
			$dbResult = CCourse::GetList(Array(), Array("CODE" => $couseCode));
	...
	*/
	...
	CopyDirFiles(
		$pathToService."/".LANGUAGE_ID."/images/".$couseCode."/",
		WIZARD_SITE_PATH,
		$rewrite = false,
		$recursive = true
	);
...
if (!WIZARD_IS_RERUN || !WIZARD_FIRST_INSTAL)
{
	...
	$urlTemlates = WIZARD_SITE_DIR."services/learning/course.php";
