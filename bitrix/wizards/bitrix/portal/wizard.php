...
class SelectTemplateStep extends CWizardStep
{
	...
	function ShowStep()
	{
		...
		$arTemplateOrder = array("light", "bitrix24", "bright", "modern", "classic");
...
/*		if ($_POST["turn_social_on"] == "Y")
	...
	function ShowStep()
	{
		...
		$this->content .= $this->PrintFeatures($this->arBXFeaturesPrintable["Portal"], -1);
		/*$this->content .= '<tr>';
		$this->content .= '<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
		$this->content .= '<input type="checkbox" name="turn_social_on" id="id_turn_social_on" value="Y"';
		if ($this->TestEnabledFeatures($this->arBXFeaturesPrintable["Social"]))
			$this->content .= ' checked';
		$this->content .= '>';
		$this->content .= '<label for="id_turn_social_on">'.GetMessage('IFS_E_Social').'</label></td>';
		$this->content .= '</tr>';*/
		$this->content .= '</table>';
	}
}
...
//For ratings
		...
		$arAllFeatures = $this->ExtractFeatures($this->arBXFeaturesPrintable["Social"]["socnet"]);
		if (!$isEnabled)
		{
			foreach ($arAllFeatures as $f)
...
class SiteSettingsStep extends CWizardStep
{
	...
	function InitStep()
	{
		...
		if($wizard->GetVar("templateID")=="light")
		{
			...
			$themeID = $wizard->GetVar("light_themeID");
			$siteLogo = $this->GetFileContentImgSrc($_SERVER['DOCUMENT_ROOT'].$siteDir."include/company_name.php", "/bitrix/wizards/bitrix/portal/site/templates/".LANGUAGE_ID."/light/themes/".$themeID."/images/logo.jpg");
		}
		else
			$siteLogo = COption::GetOptionString("main", "wizard_site_logo", "", $wizard->GetVar("siteID"));
		...
		$wizard->SetDefaultVars(
			Array(
				...
				"useSiteLogo" => COption::GetOptionString("main", "wizard_use_site_logo", "Y", $wizard->GetVar("siteID"))
			)
		);
...
JS;
		...
		$this->content .= '<tr><td><div class="line"></div></td></tr>';

		$firstStep = COption::GetOptionString("main", "wizard_first" . substr($wizard->GetID(), 7)  . "_" . $wizard->GetVar("siteID"), false, $wizard->GetVar("siteID"));
//b24 to cp		
		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/.superleft.menu.php"))  $firstStep = "Y";
		...
		if ($_SERVER["PHP_SELF"] != "/index.php" && !WIZARD_FIRST_INSTAL && $site_id != 's1')
		{
...
class LDAPSettingsStep extends CWizardStep
{
	...
	function ShowStep()
	{
		...
		$this->content .= '<tr><td align="right" width="45%" class="header"><span class="required">*</span>'.GetMessage("wiz_ldap_server").'</td><td>';
		...
		$this->content .= '<tr><td align="right" class="header"><span class="required">*</span>'.GetMessage("wiz_ldap_login").'<br><span style="font-weight: normal; font-size: 85%; color: #444444;">'.GetMessage('wiz_ldap_login_help').'</span></td><td>';
		$this->content .= $this->ShowInputField("text", "ldapLogin", Array("style" => "width:240px"));
		$this->content .= '</td></tr>';


		$this->content .= '<tr><td align="right" class="header" valign="top"><span class="required">*</span>'.GetMessage("wiz_ldap_pass").'</td><td>';
		...
		$this->content .= '<tr><td align="right" class="header"><span class="required">*</span>'.GetMessage("wiz_ldap_root").'<br><span style="font-weight: normal; font-size: 85%; color: #444444;">'.GetMessage('wiz_ldap_root_help').'</span></td><td>';
		...
		{
			$this->content .= '<tr><td align="right" valign="top" class="header">'.GetMessage('wiz_ldap_use_ntlm').'</td><td>';
			$this->content .= '<script>
...
//b24 to cp
		...
		$firstStep = COption::GetOptionString("main", "wizard_first" . substr($wizard->GetID(), 7)  . "_" . $wizard->GetVar("siteID"), false, $wizard->GetVar("siteID"));
		if (IsModuleInstalled("bitrix24"))  $firstStep = "Y";
		define("WIZARD_FIRST_INSTAL", $firstStep);
		//Copy index page
