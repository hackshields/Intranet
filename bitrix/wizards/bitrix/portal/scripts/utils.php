...
class WizardServices
{
	function GetTemplates($relativePath)
	{
		...
		while(($dirName = @readdir($handle)) !== false)
		{
			...
			if ($dirName == "bitrix24" && LANGUAGE_ID != "ru" && file_exists($absolutePath."/".$dirName."/screen_".LANGUAGE_ID.".gif"))
			    $arTemplate["SCREENSHOT"] = $relativePath."/".$dirName."/screen_".LANGUAGE_ID.".gif";
