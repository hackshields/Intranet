...
class CIBlockElement extends CAllIBlockElement
{
	function array_compare($arr1, $arr2, $bIgnoreCase = false)
	{
		...
		foreach($tmp_arr1 as $j => $tmp1)
		{
			$tmp2 = $tmp_arr2[$j];

			if (strtoupper($tmp1) != strtoupper($tmp2))
				return false;

			if ($bIgnoreCase)
			{
				if (strtoupper($arr1[$tmp1]) != strtoupper($arr2[$tmp2]))
					return false;
			}
			else
			{
	...
	{
		$arrIndexes = Array();
		if(file_exists($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/php_interface/include/iblock_add_index.php"))
		{
			include($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/php_interface/include/iblock_add_index.php");
		}
		return $arrIndexes;
	}
		...
		if(is_array($arFilter))
		{
			foreach ($arFilter as $key => $val)
			{
				...
				case "ID":
					...
					break;
				case "TIMESTAMP_FROM":
					$strSqlSearch .= " and E.TIMESTAMP_X >= ".$DB->CharToDateFunction($val, "SHORT");
					break;
				case "TIMESTAMP_TO":
					$strSqlSearch .= " and E.TIMESTAMP_X < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
					break;
				case "MODIFIED_BY":
	...
	function GetList($arOrder=Array("SORT"=>"ASC"), $arFilter=Array(), $arGroupBy=false, $arNavStartParams=false, $arSelectFields=Array())
	{
		...
		*/
				...
				"ACTIVE_FROM"=>"BE.ACTIVE_FROM as AF_TMP, ".(
						...
						:
							"
								BE.ACTIVE_FROM, case
									when (
										datepart(second, BE.ACTIVE_FROM) = 0
										and datepart(minute, BE.ACTIVE_FROM) = 0
										and datepart(hour, BE.ACTIVE_FROM) = 0
									) then ".$DB->DateToCharFunction("BE.ACTIVE_FROM", "SHORT")."
							...
							"
								BE.ACTIVE_TO, case
									when (
										...
										and datepart(minute, BE.ACTIVE_FROM) = 0
										and datepart(hour, BE.ACTIVE_FROM) = 0
									) then ".$DB->DateToCharFunction("BE.ACTIVE_TO", "SHORT")."
						...
						:
							"
								BE.ACTIVE_TO DATE_ACTIVE_TO, case
									when (
										datepart(second, BE.ACTIVE_TO) = 0
										and datepart(minute, BE.ACTIVE_TO) = 0
										and datepart(hour, BE.ACTIVE_TO) = 0
									) then ".$DB->DateToCharFunction("BE.ACTIVE_TO", "SHORT")."
									else ".$DB->DateToCharFunction("BE.ACTIVE_TO", "FULL")."
								end
							"
		...
		{
			global $BX_IBLOCK_ArrIndexes;
			foreach ($BX_IBLOCK_ArrIndexes as $ind)
			{
		...
		if($bDistinct && (
			...
			&& !in_array("SEARCHABLE_CONTENT", $arSelectFields)
			&& !in_array("DETAIL_TEXT", $arSelectFields)
			&& !in_array("PROPERTY_*", $arSelectFields)
		))
		...
		$sFrom = "b_iblock B
			...
			".ltrim($sFrom, "\t\n")."
			".(in_array("CREATED_BY_FORMATTED", $arSelectFields)? "\t\t\tLEFT JOIN b_user UC ON UC.ID=BE.CREATED_BY\n": "")."
		";

		$strSql = "SELECT ".$sHint.$sSelect."
			FROM ".$sFrom."
			WHERE 1=1 "
			.$sWhere."
			".$sGroupBy."
		...
		if(is_array($arNavStartParams))
		{
			...
			if($nTopCount > 0)
			{
				$strSql = preg_replace("#^([\\s]*select[\\s]+)(distinct[\\s]+|)(.+)$#is", "\\1\\2 TOP ".$arNavStartParams['nTopCount']." \\3", $strSql);
				$res = $DB->Query($strSql.$sOrderBy, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
			}
			elseif(
				$nElementID > 0
				&& $sGroupBy == ""
			...
			)
			{
				...
				$strRankSql = preg_replace(
					"#^([\\s]*(?:select\\s+distinct|select)[\\s]+)(.+)$#is",
					"SELECT RANK() OVER (".$sOrderBy.") AS RANK, \\2",
					str_replace(
						"E.TIMESTAMP_X,",
						"",
						str_replace(
				...
				if($nPageSize > 0)
				{
					...
					if($arRANK)
					{
						$res = $DB->Query("
							SELECT EL0.* FROM (
								".$strRankSql."
							) EL0 WHERE RANK BETWEEN ".($arRANK["RANK"] - $nPageSize)." AND ".($arRANK["RANK"] + $nPageSize)."
							ORDER BY RANK
						");
					}
					else
					{
						$strSql = preg_replace("#^([\s]*select[\s]+)(.+)$#is", "SELECT TOP ".$nPageSize." \\2", $strSql);
						$res = $DB->Query($strSql.$sOrderBy, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
					}
				}
				else
		...
		else
		{
			...
			if($bTimeStampNA)
			{
				if(strlen($strUpdate) > 0)
					$strUpdate .= ", TIMESTAMP_X=TIMESTAMP_X";
				else
					$strUpdate = "TIMESTAMP_X=TIMESTAMP_X";
			}

			if(strlen($strUpdate) > 0)
				$DB->Query("UPDATE b_iblock_element SET ".$strUpdate." WHERE ID=".$ID, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
					...
					if(strlen($val)>0)
					{
						if($prop["VERSION"]==2 && $prop["MULTIPLE"]=="N")
						{
							if ($prop["PROPERTY_TYPE"] == "N")
								...
								$val = "'".$DB->ForSql($val, 2000)."'";

							$strSql = "
								UPDATE	b_iblock_element_prop_s".$prop["IBLOCK_ID"]."
								SET
									PROPERTY_".$prop["ID"]." = ".$val."
									,DESCRIPTION_".$prop["ID"]."=".($val_desc!==false?"'".$DB->ForSQL($val_desc, 255)."'":"null")."
								WHERE	IBLOCK_ELEMENT_ID=".$ELEMENT_ID;
						}
						else
	...
	function GetRandFunction($bFull = false)
	{
		if($bFull)
			return array(
				"newid()",
