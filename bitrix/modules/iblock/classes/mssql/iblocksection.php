...
class CIBlockSection extends CAllIBlockSection
{
	...
	function GetList($arOrder=Array("SORT"=>"ASC"), $arFilter=Array(), $bIncCnt = false, $arSelect = array(), $arNavStartParams=false)
	{
		...
		$arFields = array(
			...
			"TIMESTAMP_X" =>  "BS.TIMESTAMP_X, ".$DB->DateToCharFunction("BS.TIMESTAMP_X"),
			"SORT" => "BS.SORT",
			"NAME" => "BS.NAME",
			"ACTIVE" => "BS.ACTIVE",
			...
			"DATE_CREATE" =>  "BS.DATE_CREATE, ".$DB->DateToCharFunction("BS.DATE_CREATE"),
		...
		{
			$strSelect = $sSelect.",BSC.ELEMENT_CNT".$strUserFieldsSelect;
			$strSql = "
				FROM
					b_iblock B
					INNER JOIN b_iblock_section BS ON BS.IBLOCK_ID = B.ID
