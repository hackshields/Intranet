		...
		else
		{
			...
			$res = $DB->Query("create table ".$this->_table_name."
				(
					...
					".($with_sess_id? "SESS_ID varchar2(32 char),": "")."
			...
			if($res)
				$res = $DB->Query("CREATE SEQUENCE sq_".$this->_table_name);
			if($res)
				$res = $DB->Query("CREATE OR REPLACE TRIGGER ".$this->_table_name."_insert
BEFORE INSERT
ON ".$this->_table_name."
FOR EACH ROW
BEGIN
	IF :NEW.ID IS NULL THEN
		SELECT sq_".$this->_table_name.".NEXTVAL INTO :NEW.ID FROM dual;
	END IF;
END;"
			);
	...
	*/
	...
	function Add($arFields)
	{
		...
		$strSql = "SELECT sq_".$this->_table_name.".NEXTVAL ID FROM DUAL";
		$rs = $DB->Query($strSql);
		$ar = $rs->Fetch();
		$ID = $ar["ID"];

		$strSql1 = "ID, PARENT_ID, LEFT_MARGIN, RIGHT_MARGIN, DEPTH_LEVEL, NAME";
		$strSql2 = $ID.", ".intval($arFields["PARENT_ID"]).", ".intval($arFields["LEFT_MARGIN"]).", ".intval($arFields["RIGHT_MARGIN"]).", ".intval($arFields["DEPTH_LEVEL"]).", '".$DB->ForSQL($arFields["NAME"], 255)."'";
		...
		if(array_key_exists("ATTRIBUTES", $arFields))
		{
			$strSql1 .= ", ATTRIBUTES";
			$strSql2 .= ", EMPTY_CLOB()";
			$arBinds["ATTRIBUTES"] = $arFields["ATTRIBUTES"];
		}

		if(array_key_exists("VALUE", $arFields))
		{
			$strSql1 .= ", VALUE";
			$strSql2 .= ", '".$DB->ForSQL($arFields["VALUE"], 2000)."'";
			if(strlen($arFields["VALUE"]) > 1900)
			{
				$strSql1 .= ", VALUE_CLOB";
				$strSql2 .= ", EMPTY_CLOB()";
				$arBinds["VALUE_CLOB"] = $arFields["VALUE"];
			}
		}
		...
		if(count($arBinds) > 0)
			$rs = $DB->QueryBind($strSql, $arBinds);
		else
			$rs = $DB->Query($strSql);
