	...
	function UpdateCache($cacheKey, $CACHE, $HOURS_CACHE, $bCACHED)
	{
		...
		if ($bCACHED)
		{
			$strSql =
				...
				"	CACHE = EMPTY_CLOB(), ".
				"	CACHE_DATE = SYSDATE + ".$TTL."/24/3600 ".
				"WHERE CACHE_KEY = '".$DB->ForSql($cacheKey, 0)."' ";
			$arBinds = Array();
			$arBinds["CACHE"] = $CACHE;
		...
		else
		{
			$strSql =
				...
				"VALUES('".$DB->ForSql($cacheKey, 50)."', EMPTY_CLOB(), SYSDATE + ".$TTL."/24/3600) ";
			$arBinds = Array();
			$arBinds["CACHE"] = $CACHE;
			$DB->QueryBind($strSql, $arBinds);
		}
		$db_res = $DB->Query("DELETE from b_iblock_cache WHERE CACHE_DATE < SYSDATE");
	}
