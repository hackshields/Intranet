...
class CIBlockElement extends CAllIBlockElement
{
		...
		if(is_array($arFilter))
		{
			foreach ($arFilter as $key => $val)
			{
				...
				case "ID":
					...
					break;
				case "TIMESTAMP_FROM":
					$strSqlSearch .= " and E.TIMESTAMP_X>=TO_DATE('".FmtDate($val, "D.M.Y")." 00:00:00','dd.mm.yyyy hh24:mi:ss')";
					break;
				case "TIMESTAMP_TO":
					$strSqlSearch .= " and E.TIMESTAMP_X<=TO_DATE('".FmtDate($val, "D.M.Y")." 23:59:59','dd.mm.yyyy hh24:mi:ss')";
					break;
				case "MODIFIED_BY":
	...
	function GetList($arOrder=Array("SORT"=>"ASC"), $arFilter=Array(), $arGroupBy=false, $arNavStartParams=false, $arSelectFields=Array())
	{
		...
		*/
		...
		$arIblockElementFields = Array(
				"ID"=>"BE.ID",
				"TIMESTAMP_X"=>"BE.TIMESTAMP_X TSX_TMP, ".$DB->DateToCharFunction("BE.TIMESTAMP_X"),
				"MODIFIED_BY"=>"BE.MODIFIED_BY",
				"DATE_CREATE"=>"BE.DATE_CREATE DC_TMP, ".$DB->DateToCharFunction("BE.DATE_CREATE"),
				...
				"ACTIVE_TO"=>"BE.ACTIVE_TO as AT_TMP, ".(
						CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "-")!="-"
						?
				...
				"DATE_ACTIVE_FROM"=>"BE.ACTIVE_FROM as DAF_TMP, ".(
						CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "-")!="-"
						?
							$DB->DateToCharFunction("BE.ACTIVE_FROM", CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "SHORT"))
						:
				...
				"DATE_ACTIVE_TO"=>"BE.ACTIVE_TO as DAT_TMP, ".(
						CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "-")!="-"
						?
							$DB->DateToCharFunction("BE.ACTIVE_TO", CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "SHORT"))
						:
				...
				"BP_PUBLISHED"=>"DECODE(BE.WF_STATUS_ID, 1, 'Y', 'N')",
			);
		...
		if(is_array($arNavStartParams))
		{
			...
			if($nTopCount > 0)
			{
				$strSql = "SELECT * FROM(".$strSql.$sOrderBy.")WHERE ROWNUM<=".$nTopCount;
				$res = $DB->Query($strSql, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
			}
			elseif(
				$nElementID > 0
			...
			)
			{
				...
				foreach($arSqlOrder as $i=>$val)
				{
					if(strlen($val)>0)
					{
						...
						$sOrderBy .= preg_replace("/^(.*?)\s+(asc|desc)/is", $i." \\2", $val);
					}
				}
				...
				if($nPageSize > 0)
				{
					$res = $DB->Query("
						SELECT RANK FROM (
							WITH EL0 AS (".$strSql.")
							SELECT * FROM (
								SELECT ROW_NUMBER() OVER (".$sOrderBy.") RANK, EL0.*
								FROM EL0
							)
						) WHERE ID = ".$nElementID."
					");
					...
					if($arRANK)
					{
						$res = $DB->Query("
							...
							WHERE RANK BETWEEN ".($arRANK["RANK"] - $nPageSize)." AND ".($arRANK["RANK"] + $nPageSize)."
							ORDER BY RANK
						");
					}
					else
					{
						$strSql = "SELECT * FROM(".$strSql.$sOrderBy.")WHERE ROWNUM<=".$nPageSize;
						$res = $DB->Query($strSql, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
					}
				}
				else
				{
					$res = $DB->Query("
						SELECT RANK, EL1.* FROM (
							WITH EL0 AS (".$strSql.")
							SELECT * FROM (
								SELECT ROW_NUMBER() OVER (".$sOrderBy.") RANK, EL0.*
								FROM EL0
							)
						) EL1 WHERE ID = ".$nElementID."
					");
				}
			}
			else
	...
	function Update($ID, $arFields, $bWorkFlow=false, $bUpdateSearch=true, $bResizePictures=false, $bCheckDiskQuota=true)
	{
		...
		else
		{
			...
			if(strlen($strUpdate)>0 && $bTimeStampNA)
				$strUpdate .= ", ";

			$strSql = "UPDATE b_iblock_element SET ".$strUpdate.($bTimeStampNA?"TIMESTAMP_X=NULL":"")." WHERE ID=".$ID;
			$arBinds=Array();
