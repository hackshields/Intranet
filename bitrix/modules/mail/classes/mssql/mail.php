...
class CMailbox extends CAllMailBox
{
	function CleanUp()
	{
		...
		$strSql = "DELETE FROM b_mail_log WHERE DATE_INSERT < dateadd(day, -".intval($days).", getdate())";

		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);

		$strSql = "DELETE FROM b_mail_spam_weight WHERE TIMESTAMP_X < dateadd(day, -30, getdate()) AND TOTAL_CNT<100";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);

		$strSql = "DELETE FROM b_mail_spam_weight WHERE TIMESTAMP_X < dateadd(day, -120, getdate())";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);

		$mt = GetMicroTime();
		$dbr = $DB->Query("SELECT MS.ID FROM b_mail_message MS, b_mail_mailbox MB WHERE MS.MAILBOX_ID=MB.ID AND MB.MAX_KEEP_DAYS>0 AND MS.DATE_INSERT < dateadd(day, -MB.MAX_KEEP_DAYS, getdate())");
		while($ar = $dbr->Fetch())
		{
...
class CMailMessage extends CAllMailMessage
{
	function AddAttachment($arFields)
	{
		...
		if (is_set($arFields, "FILE_DATA"))
		{
			$data = unpack("H*hex", $arFields["FILE_DATA"]);
			$arFields["FILE_DATA"] = "0x".$data["hex"];
		}
