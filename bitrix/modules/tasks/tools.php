...
class TasksException extends Exception
{
	...
	const TE_ACCESS_DENIED                     = 0x000002;
...
function tasksFormatDate($in_date)
{
	...
	if (date("d.m.Y", $now) == date("d.m.Y", $date))
	{
		$strDate = FormatDate("today", $date);
	}
	elseif (date("d.m.Y", strtotime("-1 day")) == date("d.m.Y", $date))
	{
...
function tasksGetItemMenu($task, $arPaths, $site_id = SITE_ID, $bGantt = false, $top = false, $bSkipJsMenu = false)
{
	...
	?>
		...
		<?php if (CTasks::CanCurrentUserEdit($task, $site_id)):?>
		{
		...
		if ( ! $bSkipJsMenu )
		{
			...
			elseif(
			(
				($task["RESPONSIBLE_ID"] == $USER->GetID() || CTasks::CanCurrentUserEdit($task) )
				&& $task["ALLOW_CHANGE_DEADLINE"] == "Y"
			) 
		...
		if ( ! $bSkipJsMenu )
		{
			if (($task["RESPONSIBLE_ID"] == $USER->GetID()) || ($task["CREATED_BY"] == $USER->GetID())):?>
			{ status : [2, 3,<?php if ($task["CREATED_BY"] == $USER->GetID()):?> 4, 7,<?php endif?> 6], text : "<?php echo GetMessage("TASKS_CLOSE_TASK")?>", title : "<?php echo GetMessage("TASKS_CLOSE_TASK")?>", className : "task-menu-popup-item-complete", onclick : function() { <?php if ($top):?>window.top.<?php endif?>CloseTask(<?php echo $task["ID"]?>); this.popupWindow.close();} },
			<?php if ($task["RESPONSIBLE_ID"] == $USER->GetID()):?>
				{ status : [1], text : "<?php echo GetMessage("TASKS_ACCEPT_TASK")?>", title : "<?php echo GetMessage("TASKS_ACCEPT_TASK")?>", className : "task-menu-popup-item-take", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?>AcceptTask(<?php echo $task["ID"]?>); this.popupWindow.close();} },
				{ status : [1, 2], text : "<?php echo GetMessage("TASKS_START_TASK")?>", title : "<?php echo GetMessage("TASKS_START_TASK")?>", className : "task-menu-popup-item-begin", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?>StartTask(<?php echo $task["ID"]?>); this.popupWindow.close();} },
				{ status : [1], text : "<?php echo GetMessage("TASKS_DECLINE_TASK")?>", title : "<?php echo GetMessage("TASKS_DECLINE_TASK")?>", className : "task-menu-popup-item-decline", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?>DeclineTask(<?php echo $task["ID"]?>); this.popupWindow.close();} },
				{ status : [4, 5, 6], text : "<?php echo GetMessage("TASKS_RENEW_TASK")?>", title : "<?php echo GetMessage("TASKS_RENEW_TASK")?>", className : "task-menu-popup-item-reopen", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?>StartTask(<?php echo $task["ID"]?>); this.popupWindow.close();} },
				{ status : [2, 3], text : "<?php echo GetMessage("TASKS_PAUSE_TASK")?>", title : "<?php echo GetMessage("TASKS_PAUSE_TASK")?>", className : "task-menu-popup-item-hold", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?>DeferTask(<?php echo $task["ID"]?>); this.popupWindow.close();} },
			<?php elseif ($task["CREATED_BY"] == $USER->GetID()):?>
				{ status : [5, 6, 7],  text : "<?php echo GetMessage("TASKS_RENEW_TASK")?>", title : "<?php echo GetMessage("TASKS_RENEW_TASK")?>", className : "task-menu-popup-item-reopen", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?><?php if ($task["SUBORDINATE"] == "Y"):?>AcceptTask<?php else:?>RenewTask<?php endif?>(<?php echo $task["ID"]?>); this.popupWindow.close();} },
				{ status : [4], text : "<?php echo GetMessage("TASKS_APPROVE_TASK")?>", title : "<?php echo GetMessage("TASKS_APPROVE_TASK")?>", className : "task-menu-popup-item-accept", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?>CloseTask(<?php echo $task["ID"]?>); this.popupWindow.close();} },
				{ status : [4], text : "<?php echo GetMessage("TASKS_REDO_TASK")?>", title : "<?php echo GetMessage("TASKS_REDO_TASK")?>", className : "task-menu-popup-item-remake", onclick : function(event) { <?php if ($top):?>window.top.<?php endif?><?php if ($task["SUBORDINATE"] == "Y"):?>AcceptTask<?php else:?>RenewTask<?php endif?>(<?php echo $task["ID"]?>); this.popupWindow.close();} },
			<?php endif;
			endif;
		}
		?>
		...
		if ( ! $bSkipJsMenu )
		{
			if (CTasks::CanCurrentUserDelete($task, $site_id))
			{
				?>
				{
...
function tasksRenderListItem($task, $childrenCount, $arPaths, $depth = 0, $plain = false, $defer = false, $site_id = SITE_ID, $updatesCount = 0, $projectExpanded = true, $taskAdded = false, $componentName = "bitrix:tasks.list.item", $componentTemplate = ".default", $userNameTemplate = "")
{
	global $APPLICATION;
...
function templatesRenderListItem($template, $arPaths, $depth = 0, $plain = false, $defer = false, $nameTemplate = "")
{
	...
	<script type="text/javascript"<?php echo $defer ? "  defer=\"defer\"" : ""?>>
		tasksMenuPopup[<?php echo $template["ID"]?>] = [
			{ text : "<?php echo GetMessage("TASKS_ADD_TEMPLATE_TASK")?>", title : "<?php echo GetMessage("TASKS_ADD_TEMPLATE_TASK")?>", className : "task-menu-popup-item-create", href : "<?php echo CUtil::JSEscape($addUrl)?>" },
			{ text : "<?php echo GetMessage("TASKS_EDIT_TEMPLATE")?>", title : "<?php echo GetMessage("TASKS_EDIT_TEMPLATE_EX")?>", className : "task-menu-popup-item-edit", href : "<?php echo CUtil::JSEscape($editUrl)?>" },
			{ text : "<?php echo GetMessage("TASKS_DELETE_TEMPLATE")?>", title : "<?php echo GetMessage("TASKS_DELETE_TEMPLATE")?>", className : "task-menu-popup-item-delete", onclick : function() { if(confirm("<?php echo GetMessage("TASKS_DELETE_TASKS_CONFIRM")?>")) DeleteTemplate(<?php echo $template["ID"]?>); this.popupWindow.close(); } }
		];
...
?>
	{
		...
		canEditPlanDates : <?php if ($arTask["CREATED_BY"] == $USER->GetID() || CTasks::CanCurrentUserEdit($arTask)):?>true<?php else:?>false<?php endif?>,
		...
		hasChildren : <?php echo intval($childrenCount) > 0 ? "true" : "false"?>,
		canEditDealine : <?php 
		if ($arTask["CREATED_BY"] == $USER->GetID() 
			|| 
			(
				( $arTask["RESPONSIBLE_ID"] == $USER->GetID()  || CTasks::CanCurrentUserEdit($arTask) )
...
function __checkForum($forumID)
{
	...
	{
		if (-1 === COption::GetOptionString('tasks', 'task_extranet_forum_id', -1))
		{
			try
			{
				$extranetForumID = CTasksTools::GetForumIdForExtranet();
				COption::SetOptionString('tasks', 'task_extranet_forum_id', $extranetForumID);
			}
			catch (TasksException $e)
			{
				COption::SetOptionString('tasks', 'task_extranet_forum_id', (int) $forumID);
			}
		}
	}
