...
class CTasksWebService extends IWebService
{
	...
	function __getRow($arRes, $listName, &$last_change)
	{
		...
		$change = MakeTimeStamp($arRes['CHANGED_DATE']);
		...
		$ts_start = MakeTimeStamp($arRes['START_DATE_PLAN']);
		...
		$obRow->setAttribute('ows_Created', $this->__makeDateTime(MakeTimeStamp($arRes['CREATED_DATE'])));
	...
	function GetAttachmentCollection($listName, $listItemID)
	{
		...
		if ($arTask = $rsTask->Fetch())
		{
			...
			while ($taskFile = $dbRes->Fetch())
			{
				...
				{
					$data .= '<Attachment>'.tasksServerName().$path.'</Attachment>';
				}
			}
		}
	...
	function GetListItemChangesSinceToken($listName, $viewFields = '', $query = '', $rowLimit = 0, $changeToken = '')
	{
		...
		if ($changeToken)
		{
			$arFilter['>CHANGED_DATE'] = ConvertTimeStamp($changeToken, 'FULL');
		...
		CTimeZone::Disable();
		$dbRes = CTasks::GetList(array("ID" => "ASC"), $arFilter);
		CTimeZone::Enable();
	...
	function UpdateListItems($listName, $updates)
	{
		...
		foreach ($arChanges as $obMethod)
		{
				...
				if ($rsElement && $arElement = $rsElement->Fetch())
				{
					...
					else
					{
						...
						$arElement['arParams'] = array(
							...
							str_replace(
									array('#USER_ID#', '#GROUP_ID#', '#TASK_ID#'), array($USER->GetID(), $arSection['XML_ID'], $arElement['ID']), $arPaths[$arElement['taskType']]
							),
						);
					}
				}
			...
			// this var is unused below: $pathToTask = COption::GetOptionString("tasks", "paths_task_user_entry", "/company/personal/user/#user_id#/tasks/task/view/#task_id#/", $arFields["SITE_ID"]);
			...
			if ($bUpdate)
			{
				...
				elseif ($arData['_command'] == 'New' || $arData['_command'] == 'Update')
				{
					...
					$arFields = array(
						...
						'CHANGED_DATE' => date($DB->DateFormatToPHP(CSite::GetDateFormat("FULL")), time()),
						'SITE_ID' => SITE_ID,
						'TITLE' => $arData['Title'],
						...
						else
						{
							...
							$arParams = $arElement ? $arElement['arParams'] : array(
								'PATH_TO_USER_TASKS_TASK' => str_replace(
										array('#USER_ID#', '#GROUP_ID#', '#TASK_ID#'), array($USER->GetID(), $arSection['XML_ID'], $ID), $arPaths['user']
								)
									);
						}
					}
