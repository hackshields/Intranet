...
class CTaskSync
{
	...
	public static function SyncTaskItems($type, $userId, $arUserTaskItems)
	{
		...
		$arTasksReturn = array();
		$arDelete = array();
		$strSql = "SELECT ID FROM b_tasks WHERE EXCHANGE_ID IS NOT NULL AND RESPONSIBLE_ID = ".intval($userId);
		...
		{
			$strSql = "SELECT ID, EXCHANGE_MODIFIED FROM b_tasks WHERE EXCHANGE_ID = '".$DB->ForSql($taskItem["XML_ID"])."' AND RESPONSIBLE_ID = ".intval($userId);
		...
		if (sizeof($arDelete))
		{
			...
			$strSql = "SELECT * FROM b_tasks WHERE ID IN (" . implode(",", $arDelete).") AND RESPONSIBLE_ID = " . (int) $userId;
	...
	public static function SyncModifyTaskItem($arModifyEventArray)
	{
		...
		$arFields = array(
			...
			"SITE_ID" => SITE_ID,
			"TAGS" => array($arModifyEventArray["FOLDER_ID"]),
	...
	public static function UpdateItem($arFields, $arTask)
	{
		...
		elseif (
			isset($arTask['DESCRIPTION_IN_BBCODE'])
			&& ($arTask['DESCRIPTION_IN_BBCODE'] === 'Y')
		)
		{
			$bodyType = 'text';
		}
