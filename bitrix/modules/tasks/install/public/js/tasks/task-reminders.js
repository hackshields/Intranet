		...
		if (!deadline) {
			...
			if (tmpDeadline != null) {
				...
				BX.adjust(deadlineSpan, {
					text: "(" + BX.date.format(BX.date.convertBitrixFormat(BX.message('FORMAT_DATE')), this.deadline) + ")"
			...
			var reminderNode = BX.create("div" , {
				...
				children: [
					...
					BX.create("span", {
						...
						children: [
							BX.create("span", {
								text: BX.date.format(BX.date.convertBitrixFormat(BX.message('FORMAT_DATE')), this.reminders[i].date) 
...
//								text: this.calendar.FormatDate(this.reminders[i].date, phpVars.FORMAT_DATE) + " " + (this.reminders[i].transport == "J" ? BX.message("TASKS_REMIND_VIA_JABBER") : BX.message("TASKS_REMIND_VIA_EMAIL"))
	...
	TaskReminders.prototype.__showTransportMenu = function(button, taskId) {
		var menu = [
			{
				...
				className: "task-menu-popup-item-no_icon",
				onclick: function() {
		...
		BX.TaskMenuPopup.show(taskId, button, menu, {offsetTop : -5});
	}
})(window);