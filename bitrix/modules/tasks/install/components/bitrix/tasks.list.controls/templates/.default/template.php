...
<div class="task-title-buttons"><?php
	...
	{
		?><span
		class="task-title-button-search<?php if(isset($_GET["F_SEARCH"]) && $_GET["F_SEARCH"]):?> task-title-button-search-full<?php endif?>"><span class="task-title-button-search-left"></span><span
				...
				if (isset($_GET["F_SEARCH"]) && $_GET["F_SEARCH"])
				{
					?> value="<?php
						if (isset($_GET["F_SEARCH"]))
							echo htmlspecialcharsbx($_GET["F_SEARCH"]);
						else
							echo '';
					?>"<?php
				}
	...
	if ($arParams['SHOW_TASK_LIST_MODES'] == 'Y')
	{
		?><span class="task-list-modes"
			><a class="task-list-mode<?php echo ($arParams["VIEW_TYPE"] == "tree" ? " task-list-mode-selected" : "")?>" href="<?php echo $APPLICATION->GetCurPageParam("", array("VIEW"));?>" title="<?php echo GetMessage("TASKS_TREE_LIST")?>"><i class="task-list-mode-expanded"></i></a
