	...
	BX.ready(function() {
		...
		var tasks = [
			...
			<?php foreach($arResult["TASKS"] as $arTask):?>
				<?php $i++?>
				<?php tasksRenderJSON($arTask, $arResult["CHILDREN_COUNT"]["PARENT_".$arTask["ID"]], $arPaths, false, true, false, $arParams["NAME_TEMPLATE"])?><?php if ($i != sizeof($arResult["TASKS"])):?>,<?php endif?>
			<?php endforeach?>
		];
