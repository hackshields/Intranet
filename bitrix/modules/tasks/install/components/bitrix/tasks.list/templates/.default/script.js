...
function AddQuickPopupTask(e, newTaskData)
{
	...
	BX.PreventDefault(e);
	BX.Tasks.lwPopup.showCreateForm(newTaskData);
}
...
function SetServerStatus(taskId, status, params)
{
	...
	quickInfoData[taskId].realStatus = statusMap[status];
...
function ShowMenuPopup(taskId, bindElement)
{
	if (tasksMenuPopup[taskId])
		BX.TaskMenuPopup.show(taskId, bindElement, __FilterMenuByStatus(quickInfoData[taskId]), {events : {onPopupClose: __onMenuPopupClose}});
...
function ShowMenuPopupContext(taskId, event)
{
	...
	if (tasksMenuPopup[taskId])
		BX.TaskMenuPopup.show("c" + taskId, event, __FilterMenuByStatus(quickInfoData[taskId]), {});
...
function __InvalidateMenus(menus)
{
	...
	{
		if (BX.TaskMenuPopup.tasksData[menus[i]])
		{
			BX.remove(BX.TaskMenuPopup.tasksData[menus[i]].popupWindow.popupContainer);
			delete BX.TaskMenuPopup.tasksData[menus[i]];
		}
	}
}
...
{
	popupWindow.close();
	ShowPopupTask(quickInfo.task.id, event);
}
...
{
	var timeman = false;
	var item = BX.findChild(object.popupWindow.contentContainer, {tagName: "a", className: "task-menu-popup-item task-menu-popup-item-add-to-tm"}, true);
...
function ToggleQuickTask(table, params)
{
	BX.toggleClass(BX("task-new-item-icon"), "task-list-mode-selected");
	if (BX.hasClass(BX("task-new-item-icon"), "task-list-mode-selected"))
	{
...
function ShowQuickTask(table, params)
{
	...
	if (params.group || params.parent)
	{
		if (params.group)
		{
			groupsPopup.select(params.group);
		...
		BX.removeClass(BX("task-new-item-icon"), "task-list-mode-selected");
	}
...
function HideQuickTask(taskId)
{
	BX.addClass(BX("task-new-item-row"), "task-list-item-hidden");
	BX.removeClass(BX("task-new-item-icon"), "task-list-mode-selected");
}

function onQuickResponsibleSelect(arUser)
{
	BX("task-new-item-responsible-hidden").value = arUser.id;

	quickResponsiblePopup.close();
}

function onQuickResponsibleClose()
{
	var emp = O_QUICK_RESPONSIBLE.arSelected.pop();
	if (emp)
	{
		O_QUICK_RESPONSIBLE.arSelected.push(emp);
		O_QUICK_RESPONSIBLE.searchInput.value = emp.name;
	}
}
...
{
	BX.removeClass(this.bindElement, "task-priority-box-selected");
	this.bindElement.title = BX.message("TASKS_PRIORITY") + ": " + this.listItem.name;
...
function tasksFormatDate(date)
{
	...
	var str = phpVars.FORMAT_DATETIME;
	str = str.replace(/YYYY/ig, date.getFullYear());
	str = str.replace(/MM/ig, Number(date.getMonth()+1));
	str = str.replace(/DD/ig, Number(date.getDate()));
	str = str.replace(/HH/ig, Number(date.getHours()));
	str = str.replace(/MI/ig, Number(date.getMinutes()));
	str = str.replace(/SS/ig, Number(date.getSeconds()));
	return str;
}

function ShowLegendPopup(bindElement)
{
	BX.TaskLegendPopup.show(
		[
			{ title : BX.message("TASKS_LEGEND_TITLE_1"), content : BX.message("TASKS_LEGEND_CONTENT_1")},
			{ title : BX.message("TASKS_LEGEND_TITLE_2"), content : BX.message("TASKS_LEGEND_CONTENT_2")},
			{ title : BX.message("TASKS_LEGEND_TITLE_3"), content : BX.message("TASKS_LEGEND_CONTENT_3")},
			{ title : BX.message("TASKS_LEGEND_TITLE_5"), content : BX.message("TASKS_LEGEND_CONTENT_5")}
		],
		bindElement,
		{}
	);
