...
var taskManagerForm =
{
	...
	init : function()
	{
		...
		var dateTextboxes = [BX("task-deadline-date"), BX("task-repeating-interval-start-date"), BX("task-repeating-interval-end-date")];
		...
		BX.bind(BX("task-deadline-date"), "change", 
			function()
			{
...
function onResponsibleSelect(arUser)
{
	...
	if (input && (input.value > 0) && (arUser.id > 0))
	{
		...
		if ( ! tasks_isUserMemberOfGroup(arUser.id, groupId) )
		{
			tasks_showWarning(
				BX.message('TASKS_WARNING_RESPONSIBLE_NOT_IN_TASK_GROUP')
					.replace("#GROUP_NAME#", groupName)
					.replace("#FORMATTED_USER_NAME#", arUser.name)
			);
		}
		else
...
function tasks_isUserMemberOfGroup(userId, groupId)
{
	var rc = false;
	var data = {
		sessid  : BX.message('bitrix_sessid'),
		userId  : userId,
		groupId : groupId,
		action  : 'tasks_isUserMemberOfGroup'
	}
...
function tasks_showWarning(text)
{
	if (BX('task-edit-warnings-area'))
	{
		BX('task-edit-warnings-area').style.display = 'block';
		BX('task-edit-warnings-area-message').innerHTML = text;
	}
}


function tasks_removeWarnings()
{
	if (BX('task-edit-warnings-area'))
	{
