...
if ($_REQUEST['MODE'] == 'SEARCH')
{
	...
	$dbRes = CUser::SearchUserByName(
		$arName = explode(' ', $search), '', true
	);
	
	$strUserIDs = '';
	while ($arRes = $dbRes->Fetch())
	{
		if (!is_array($arUsersToFilter) || in_array($arRes['ID'], $arUsersToFilter))
			$strUserIDs .= ($strUserIDs == '' ? '' : '|').$arRes['ID'];
	}

	if (strlen($strUserIDs) > 0)
	{
		...
		while ($arRes = $dbRes->NavNext(false))
		{
			$arUsers[] = array(
				'ID' => $arRes['ID'],
				'NAME' => str_replace(array(';', ','), ' ', CIntranetUtils::FormatName($arParams["NAME_TEMPLATE"], $arRes, false)).' <'.$arRes['EMAIL'].'>',
				'UF_DEPARTMENT' => $arRes['UF_DEPARTMENT'],
			);
		}
	}
