...
class CCrmActivity extends CAllCrmActivity
{
	...
	public static function DoSaveBindings($ID, &$arBindings)
	{
		...
		{
			$query .= ($query !== '' ? ' UNION ALL ' : '').'SELECT '.$value;
		}
	...
	public static function PrepareBindingsFilterSql(&$arBindings, $tableAlias = '')
	{
		...
		foreach($arBindings as &$binding)
		{
			...
			if($ownerID > 0)
			{
				$sql .= "(({$tableAlias}.OWNER_TYPE_ID = {$ownerTypeID} AND {$tableAlias}.OWNER_ID = {$ownerID}) OR EXISTS (SELECT TOP(1) B.ID FROM {$bindingTableName} B WHERE B.ACTIVITY_ID = {$tableAlias}.ID AND {$s}))";
			}
			else
			{
				$sql .= "({$tableAlias}.OWNER_TYPE_ID = {$ownerTypeID} OR EXISTS (SELECT TOP(1) B.ID FROM {$bindingTableName} B WHERE B.ACTIVITY_ID = {$tableAlias}.ID AND {$s}))";
			}
		}
		unset($binding);
	...
	public static function DoSaveCommunications($ID, &$arComms, $arFields = array(), $registerEvents = true, $checkPerms = true)
	{
		...
		$DB->Query(
			'INSERT INTO '.self::COMMUNICATION_TABLE_NAME.'('.$bulkColumns.') '.$query,
			false,
			'File: '.__FILE__.'<br/>Line: '.__LINE__
		);
	...
	public static function DoSaveNearestUserActivity($arFields)
	{
		...
		$updateSql = "UPDATE B_CRM_USR_ACT SET ACTIVITY_TIME = {$activityTime}, ACTIVITY_ID = {$activityID}
			WHERE USER_ID = {$userID} AND OWNER_ID = {$ownerID} AND OWNER_TYPE_ID = {$ownerTypeID}";

		$dbResult = $DB->Query($updateSql, false, 'File: '.__FILE__.'<br/>Line: '.__LINE__);
		if($dbResult->AffectedRowsCount() == 0)
		{
			$insertSql = "INSERT INTO B_CRM_USR_ACT(USER_ID, OWNER_ID, OWNER_TYPE_ID, ACTIVITY_TIME, ACTIVITY_ID, DEPARTMENT_ID)
