...
class CCrmUserCounter
{
	...
	public static function IsTypeDefined($typeID)
	{
		...
		return $typeID > self::Undefined && $typeID <= self::CurrentDealActivies;
	}
	...
	public function Synchronize()
	{
		...
		elseif($this->typeID === self::CurrentLeadActivies)
		{
			...
			if(CCrmUserCounterSettings::GetValue(CCrmUserCounterSettings::ReckonActivitylessItems, true))
			{
				...
				global $DB;
				$dbResult = $DB->Query(
					"SELECT COUNT(l.ID) AS CNT FROM {$leadTable} l WHERE l.ASSIGNED_BY_ID = {$this->userID} AND l.STATUS_ID NOT IN({$statusStr}) AND l.ID NOT IN(SELECT a.OWNER_ID FROM {$activityTable} a WHERE a.OWNER_TYPE_ID = 1)",
					false,
		...
		elseif($this->typeID === self::CurrentDealActivies)
		{
			...
			if(CCrmUserCounterSettings::GetValue(CCrmUserCounterSettings::ReckonActivitylessItems, true))
			{
				...
				$dbResult = $DB->Query(
					"SELECT COUNT(d.ID) AS CNT FROM {$dealTable} d WHERE d.ASSIGNED_BY_ID = {$this->userID} AND d.STAGE_ID NOT IN({$stageStr}) AND d.ID NOT IN(SELECT a.OWNER_ID FROM {$activityTable} a WHERE a.OWNER_TYPE_ID = 2)",
					false,
					'File: '.__FILE__.'<br/>Line: '.__LINE__
				);
				$result = $dbResult->Fetch();
