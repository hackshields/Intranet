...
<div id="#WRAPPER_ID#" style="display:none;" class="crm-fld-lhe-wrap" >#HTML#</div>'
	...
	public static function CreateMultiFields($fieldTypeID, &$fieldValues, &$formFields, $fieldParams = array(), $readOnlyMode = true)
	{
		...
		foreach($fieldValues as $ID => &$data)
		{
			...
			$fieldID = "FM.{$fieldTypeID}.{$valueType}.{$ID}";
			$field = array(
				'id' => $fieldID,
...
//                            $hasProto = preg_match('/^(?:http:\/\/)?(?:www\.)?facebook\.com\/([^\/]+)(?:\/)?/i', $value, $urlMatches) > 0;
//                            if($hasProto)
//                            {
//                                $value = $urlMatches[1];
//                            }
...
//                            $hasProto = preg_match('/^(?:http:\/\/)?(?:www\.)?twitter\.com\/([^\/]+)(?:\/)?/i', $value, $urlMatches) > 0;
//                            if($hasProto)
//                            {
//                                $value = $urlMatches[1];
//                            }
...
//                            $hasProto = preg_match('/^(?:http:\/\/)?(?:www\.)?([^\.]+).livejournal\.com(?:\/)?/i', $value, $urlMatches) > 0;
//                            if($hasProto)
//                            {
//                                $value = $urlMatches[1];
//                            }
...
//                            }
	...
	public static function PrepareUserInfo($userID, &$userInfo, $options = array())
	{
		...
		$photoInfo = CFile::ResizeImageGet(
			...
			array(
				'width' => $photoW > 0 ? $photoW : 30,
				'height'=> $photoH > 0 ? $photoH : 30
			),
	...
	public static function PrepareUpdate($ownerTypeID, &$arFields, &$arFieldNames, &$arFieldValues)
	{
		...
		for($i = 0; $i < $count; $i++)
		{
			...
			if($fieldName === 'COMMENTS')
			{
				...
				$arFields['COMMENTS'] = preg_replace("/(<script[^>]*>)(.*?)(<\\/script[^>]*>)/is", '', $fieldValue);
			}
