...
class CCrmTemplateMapper extends CCrmTemplateMapperBase
{
	...
	private function MapField(&$entityInfo, $fieldName)
	{
		...
		if($typeID === CCrmOwnerType::Lead)
		{
			switch($fieldName)
			{
				...
				case 'ADDRESS':
				case 'COMMENTS':
					$result = isset($fields[$fieldName]) ? $fields[$fieldName] : '';
					break;
				case 'SOURCE':
					$result = self::MapReferenceValue(self::PrepareSources(), $fields, 'SOURCE_ID');
					break;
				case 'STATUS':
					$result = self::MapReferenceValue(self::PrepareLeadStatuses(), $fields, 'STATUS_ID');
					break;
				case 'FORMATTED_NAME':
				...
				case 'ASSIGNED_BY_FULL_NAME':
					$result = CCrmViewHelper::GetFormattedUserName(isset($fields['ASSIGNED_BY_ID']) ? $fields['ASSIGNED_BY_ID'] : 0);
					break;
				case 'CREATED_BY_FULL_NAME':
					$result = CCrmViewHelper::GetFormattedUserName(isset($fields['CREATED_BY_ID']) ? $fields['CREATED_BY_ID'] : 0);
					break;
				case 'MODIFY_BY_FULL_NAME':
					$result = CCrmViewHelper::GetFormattedUserName(isset($fields['MODIFY_BY_ID']) ? $fields['MODIFY_BY_ID'] : 0);
					break;
				case 'DATE_CREATE':
		...
		elseif($typeID === CCrmOwnerType::Deal)
		{
			switch($fieldName)
			{
				...
				case 'COMMENTS':
					...
					break;
				case 'TYPE':
					$result = self::MapReferenceValue(self::PrepareDealTypes(), $fields, 'TYPE_ID');
					break;
				case 'STAGE':
					$result = self::MapReferenceValue(self::PrepareDealStages(), $fields, 'STAGE_ID');
					break;
				case 'PROBABILITY':
					$result = (isset($fields[$fieldName]) ? intval($fields[$fieldName]) : 0).' %';
					break;
		...
		elseif($typeID === CCrmOwnerType::Contact)
		{
			switch($fieldName)
			{
				...
				case 'SOURCE':
					...
					break;
				case 'TYPE':
					$result = self::MapReferenceValue(self::PrepareContactTypes(), $fields, 'TYPE_ID');
					break;
				case 'COMPANY':
					$result = self::ResolveEntityInfo(
						CCrmOwnerType::Company,
		...
		elseif($typeID === CCrmOwnerType::Company)
		{
			switch($fieldName)
			{
				...
				case 'COMPANY_TYPE':
				case 'TYPE':
					$result = self::MapReferenceValue(self::PrepareCompanyTypes(), $fields, 'COMPANY_TYPE');
					break;
				case 'INDUSTRY':
					$result = self::MapReferenceValue(self::PrepareIndustries(), $fields, 'INDUSTRY');
					break;
				case 'EMPLOYEES':
					$result = self::MapReferenceValue(self::PrepareEmployees(), $fields, 'EMPLOYEES');
					break;
				case 'CURRENCY':
					$result = isset($fields['CURRENCY_ID']) ? CCrmCurrency::GetCurrencyName($fields['CURRENCY_ID']) : '';
					break;
	...
	private static function MapReferenceValue(&$items, &$fields, $fieldID)
	{
		$ID = isset($fields[$fieldID]) ? $fields[$fieldID] : '';
		return isset($items[$ID]) ? $items[$ID] : $ID;
	}
