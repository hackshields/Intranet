...
class CAllCrmActivity
{
	...
	private static $STORAGE_TYPE_ID = CCrmActivityStorageType::Undefined;
	...
	public static function Add(&$arFields, $checkPerms = true, $regEvent = true, $options = array())
	{
		...
		$ID = $DB->Add(CCrmActivity::TABLE_NAME, $arFields, array('STORAGE_ELEMENT_IDS', 'SETTINGS'));
		if($ID === false)
		{
		...
		if($regEvent)
		{
			foreach($arBindings as &$arBinding)
			{
				self::RegisterAddEvent($arBinding['OWNER_TYPE_ID'], $arBinding['OWNER_ID'], $arFields, $checkPerms);
			}
			unset($arBinding);
		}
	...
	public static function Update($ID, $arFields, $checkPerms = true, $regEvent = true, $options = array())
	{
		...
		elseif(is_string($storageElementIDs))
		{
			$storageElementIDs = unserialize($storageElementIDs);
		}
		...
		if($prevResponsibleID > 0 && !empty($arPrevBindings))
		{
			foreach($arPrevBindings as &$arBinding)
			{
				if($responsibleID !== $prevResponsibleID
					|| !in_array("{$arBinding['OWNER_TYPE_ID']}_{$arBinding['OWNER_ID']}", $arSyncKeys, true))
				{
		...
		if($regEvent)
		{
			foreach($arBindings as &$arBinding)
			{
				self::RegisterUpdateEvent($arBinding['OWNER_TYPE_ID'], $arBinding['OWNER_ID'], $arCurEntity, $arPrevEntity, $checkPerms);
			}
			unset($arBinding);
		}
	...
	public static function HandleStorageElementDeletion($storageTypeID, $elementID)
	{
		...
		{
			$entityID = isset($arResult['ACTIVITY_ID']) ? intval($arResult['ACTIVITY_ID']) : 0;
			if($entityID <= 0)
			{
				continue;
			}
			...
			$typeID = isset($arEntity['STORAGE_TYPE_ID']) ? intval($arEntity['STORAGE_TYPE_ID']) : $storageTypeID;
			$elmentIDs = isset($arEntity['STORAGE_ELEMENT_IDS'])
				? self::NormalizeStorageElementIDs(unserialize($arEntity['STORAGE_ELEMENT_IDS'])) : array();

			$arEntity['STORAGE_ELEMENT_IDS'] = !empty($elmentIDs)
					? array_diff($elmentIDs, array($elementID))
					: array();

			$arEntity['STORAGE_TYPE_ID'] = $typeID;

			self::Update($entityID, $arEntity, false, true);
		}
	}
	//Check fields before ADD and UPDATE.
	private static function CheckFields($action, &$fields, $ID)
	{
		global $DB;
		self::ClearErrors();
...
//			}
			...
			{
				$fields['AUTHOR_ID'] = CCrmPerms::GetCurrentUserID();
			}
		...
		else//if($action == 'UPDATE')
		{
			...
			{
				$fields['EDITOR_ID'] = CCrmPerms::GetCurrentUserID();
			}
		}
	...
	protected static function DeleteStorageElements($ID)
	{
		...
		$storageTypeID = isset($arRes['STORAGE_TYPE_ID'])
			? intval($arRes['STORAGE_TYPE_ID']) : CCrmActivityStorageType::Undefined;
	...
	public static function GetByID($ID)
	{
		...
		{
			$res['SETTINGS'] = isset($res['SETTINGS']) ? unserialize($res['SETTINGS']) : array();
			CCrmEntityHelper::SetCached(self::CACHE_NAME, $ID, $res);
		}

		return $res;
	}
	public static function GetByOriginID($originID)
	{
		$originID = strval($originID);
		$dbRes = self::GetList(array(), array('ORIGIN_ID' => $originID));
	...
	public static function __AfterPrepareSql(/*CCrmEntityListBuilder*/ $sender, $arOrder, $arFilter, $arGroupBy, $arSelectFields)
	{
		...
		return $sql !== '' ? array('WHERE' => $sql) : false;
	}
	...
	private static function PrepareCommunicationSettings(&$arComm)
	{
		...
		if($commEntityID > 0 && $commEntityTypeID > 0)
		{
			...
			{
				$arLead = CCrmLead::GetByID($commEntityID, false);
				if($arLead)
				{
			...
			{
				$arContact = CCrmContact::GetByID($commEntityID, false);
				if($arContact)
				{
					$arComm['ENTITY_SETTINGS'] = array(
			...
			{
				$arCompany = CCrmCompany::GetByID($commEntityID, false);
				if($arCompany)
				{
	...
	public static function GetCommunications($activityID)
	{
		$activityID = intval($activityID);
		if($activityID <= 0)
		{
		...
		$commTableName = CCrmActivity::COMMUNICATION_TABLE_NAME;
		$dbRes = $DB->Query(
			"SELECT ID, TYPE, VALUE, ENTITY_ID, ENTITY_TYPE_ID, ENTITY_SETTINGS FROM {$commTableName} WHERE ACTIVITY_ID = {$activityID}",
			false,
			'FILE: '.__FILE__.'<br /> LINE: '.__LINE__
		);
	...
	public static function PrepareCommunicationInfo(&$arComm)
	{
	...
	public static function PrepareStorageElementIDs(&$arFields)
	{
		...
		$arFields['STORAGE_ELEMENT_IDS'] = is_string($field) ? unserialize($field) : $field;
	}
	public static function Exists($ID)
	{
		$dbRes = CCrmActivity::GetList(array(), array('ID'=> $ID), false, false, array('ID'));
		return is_array($dbRes->Fetch());
	}
	public static function Complete($ID, $completed = true)
	{
		$ID = intval($ID);
		if($ID <= 0)
		{
		...
		return self::Update($ID, array('COMPLETED' => $completed), true, true);
	}
	public static function SetPriority($ID, $priority)
	{
		...
		return self::Update($ID, array('PRIORITY' => $priority), true, true);
	}
	public static function GetLastErroMessage()
	{
	...
	public static function TryResolveUserFieldOwners(&$arUsefFieldData, &$arOwnerData)
	{
		$parsed = 0;
		...
		foreach($arUsefFieldData as $value)
		{
			...
			if(preg_match('/^([A-Z]+)_([0-9]+)$/', strtoupper(trim($value)), $match) <= 0)
			{
				continue;
			}
	...
	public static function CreateFromEvent(
		$eventID,
		&$arEventFields,
		$activityType = CCrmActivityType::Activity,
		$checkPerms = true,
		$regEvent = true)
	{
		...
		if(!CCrmActivityType::IsDefined($activityType))
		{
			$activityType = CCrmActivityType::Activity;
		}
		...
		$entityType = isset($arEventFields['~ENTITY_TYPE']) ? $arEventFields['~ENTITY_TYPE'] : '';
		$entityTypeID = CCrmOwnerType::ResolveID($entityType);
		$entityID = isset($arEventFields['~ENTITY_ID']) ? intval($arEventFields['~ENTITY_ID']) : 0;
		...
		$created = isset($arEventFields['~DATE_CREATE']) ? MakeTimeStamp($arEventFields['~DATE_CREATE']) : $now;

		$arFileIDs = array();
		if(isset($arEventFields['~FILES']) && is_array($arEventFields['~FILES']))
		{
			foreach($arEventFields['~FILES'] as $fileID)
			{
				$newFileID = CFile::CopyFile($fileID, true);
				if(is_int($newFileID) && $newFileID > 0)
				{
					$arFileIDs[] = $newFileID;
				}
			}
		...
		$arFields = array(
			...
			'SUBJECT' => isset($arEventFields['~EVENT_TEXT_1']) ? $arEventFields['~EVENT_TEXT_1'] : '',
			'START_TIME' => ConvertTimeStamp($created, 'FULL', SITE_ID),
			'END_TIME' => ConvertTimeStamp($created, 'FULL', SITE_ID),
			'COMPLETED' => $created <= $now ? 'Y' : 'N',
			'RESPONSIBLE_ID' => isset($arEventFields['~CREATED_BY_ID']) ? intval($arEventFields['~CREATED_BY_ID']) : 0,
			...
			'STORAGE_TYPE_ID' => CCrmActivityStorageType::File,
			'STORAGE_ELEMENT_IDS' => $arFileIDs,
			'SETTINGS' => array()
		);

		$ID = self::Add($arFields, $checkPerms, $regEvent);
		if(!is_int($ID) || $ID <= 0)
		{
			return false;
		}
		...
		$regEvent = true)
	{
		...
		if(isset($arFields['BINDINGS']) && count($arFields['BINDINGS']) > 0)
		{
			return self::Add($arFields, $checkPerms, $regEvent, array('SKIP_ASSOCIATED_ENTITY' => true));
		}
	}
	...
	private static function SetFromTask($taskID, &$arTaskFields, &$arFields)
	{
		...
		if($isNew || isset($arTaskFields['DATE_START']))
		{
		...
		if($isNew || isset($arTaskFields['DEADLINE']) || isset($arTaskFields['CLOSED_DATE']))
		{
		...
		{
			$arFields['DESCRIPTION'] = isset($arTaskFields['DESCRIPTION']) ? $arTaskFields['DESCRIPTION'] : '';
		}
		...
		if(self::TryResolveUserFieldOwners($arTaskOwners, $arOwnerData))
		{
	...
	private static function SetFromCalendarEvent($eventID, &$arEventFields, &$arFields)
	{
		...
		$arOwnerData = array();
		self::TryResolveUserFieldOwners($arEventOwners, $arOwnerData);
		...
		if(isset($arEventFields['DT_FROM']))
		{
			$arFields['START_TIME'] = $arFields['END_TIME'] = $arEventFields['DT_FROM'];
		}
		elseif(isset($arEventFields['DT_TO']))
		{
	...
	public static function OnTaskUpdate($taskID, &$arTaskFields)
	{
		...
		while($arEntity = $dbEntities->Fetch())
		{
			...
			{
				self::Update($arEntity['ID'], $arEntity, false, true, array('SKIP_ASSOCIATED_ENTITY' => true));
			}
			else
			{
		...
		if(!$isFound)
		{
			...
			if(isset($arFields['BINDINGS']) && count($arFields['BINDINGS']) > 0)
			{
				self::Add($arFields, false, true, array('SKIP_ASSOCIATED_ENTITY' => true));
			}
		}
	}
	...
	public static function OnCalendarEventEdit($eventID, $arEventFields)
	{
	...
	public static function DeleteByOwner($ownerTypeID, $ownerID)
	{
		...
		self::DeleteBindingsByOwner($ownerTypeID, $ownerID);
		self::DeleteUnbound(
	...
	public static function DeleteUnbound($arBindings = null)
	{
		...
		$arOptions = array('SKIP_BINDINGS' => true);
		if(is_array($arBindings) && !empty($arBindings))
		{
			$arOptions['ACTUAL_BINDINGS'] = $arBindings;
		}
		while($arRes = $dbRes->Fetch())
		{
			self::Delete($arRes['ID'], false, false, $arOptions);
		}
	}
	...
	private static function ResolveStorageElementName($storageTypeID, $elementID)
	{
		...
		if($storageTypeID === CCrmActivityStorageType::WebDav)
		{
			$info = CCrmWebDavHelper::GetElementInfo($elementID, false);
			return is_array($info) && isset($info['NAME']) ? $info['NAME'] : "[{$elementID}]";
		}
		elseif($storageTypeID === CCrmActivityStorageType::File)
		{
			$info = CFile::GetFileArray($elementID);
			return is_array($info) && isset($info['FILE_NAME']) ? $info['FILE_NAME'] : "[{$elementID}]";
		}
	...
	private static function PrepareFileEvent($storageTypeID, $elementID, $action, &$arRow, &$arEvents)
	{
		...
		$arEventFiles = array();
		if($action === 'ADD' && $storageTypeID !== CCrmActivityStorageType::Undefined)
		{
	...
	private static function RegisterAddEvent($ownerTypeID, $ownerID, $arRow, $checkPerms)
	{
		...
		$arEventFiles = array();
		$storageTypeID = isset($arRow['STORAGE_TYPE_ID']) ? intval($arRow['STORAGE_TYPE_ID']) : CCrmActivityStorageType::Undefined;
		if($storageTypeID !== CCrmActivityStorageType::Undefined)
		{
			$storageElementIDs = isset($arRow['STORAGE_ELEMENT_IDS']) ? unserialize($arRow['STORAGE_ELEMENT_IDS']) : array();
			if(!empty($storageElementIDs))
			{
				$arEventFiles = self::MakeRawFiles($storageTypeID, $storageElementIDs);
			}
		}
	...
	private static function RegisterUpdateEvent($ownerTypeID, $ownerID, $arNewRow, $arOldRow, $checkPerms)
	{
		...
		$newStorageTypeID = isset($arNewRow['STORAGE_TYPE_ID']) ? intval($arNewRow['STORAGE_TYPE_ID']) : CCrmActivityStorageType::Undefined;
		$oldStorageTypeID = isset($arOldRow['STORAGE_TYPE_ID']) ? intval($arOldRow['STORAGE_TYPE_ID']) : CCrmActivityStorageType::Undefined;

		$newElementIDs = isset($arNewRow['STORAGE_ELEMENT_IDS']) ? unserialize($arNewRow['STORAGE_ELEMENT_IDS']) : array();
		$oldElementIDs = isset($arOldRow['STORAGE_ELEMENT_IDS']) ? unserialize($arOldRow['STORAGE_ELEMENT_IDS']) : array();
	...
	protected static function RegisterFileEvent($ID, $arFields, $fileInfo, $eventType, $checkPerms = true)
	{
		...
		if(!is_array($arFields)
			|| count($arFields) === 0
			|| !isset($arFields['TYPE_ID'])
			|| !isset($arFields['SUBJECT']))
		{
			$dbRes = self::GetList(
				array(),
				array('=ID' => $ID),
	...
	public static function GetStorageTypeID($ID)
	{
		...
		$arRes = $dbRes->Fetch();
		return is_array($arRes) && isset($arRes['STORAGE_TYPE_ID']) ? intval($arRes['STORAGE_TYPE_ID']) : CCrmActivityStorageType::Undefined;
	}
	public static function GetDefaultStorageTypeID()
	{
		if(self::$STORAGE_TYPE_ID === CCrmActivityStorageType::Undefined)
		{
			self::$STORAGE_TYPE_ID = intval(CUserOptions::GetOption('crm', 'activity_storage_type_id', CCrmActivityStorageType::Undefined));
			if(self::$STORAGE_TYPE_ID === CCrmActivityStorageType::Undefined
				|| !CCrmActivityStorageType::IsDefined(self::$STORAGE_TYPE_ID))
			{
				self::$STORAGE_TYPE_ID = IsModuleInstalled('webdav') ? CCrmActivityStorageType::WebDav : CCrmActivityStorageType::File;
			}
		}
	...
	public static function SetDefaultStorageTypeID($storageTypeID)
	{
		...
		if(!CCrmActivityStorageType::IsDefined($storageTypeID))
		{
			$storageTypeID = IsModuleInstalled('webdav') ? CCrmActivityStorageType::WebDav : CCrmActivityStorageType::File;
		}
	...
	public static function GetNearest($ownerTypeID, $ownerID, $userID)
	{
		...
		$tableName = CCrmActivity::TABLE_NAME;
		$bindingTableName = CCrmActivity::BINDING_TABLE_NAME;
		$startTime = $DB->DateToCharFunction('START_TIME', 'FULL');

		$userID = intval($userID);

		if($userID > 0)
		{
			$sql = "SELECT a.ID, {$startTime} AS START_TIME_FORMATTED, a.START_TIME FROM {$tableName} a INNER JOIN {$bindingTableName} b ON a.ID = b.ACTIVITY_ID AND a.COMPLETED = 'N' AND a.RESPONSIBLE_ID = {$userID} AND a.START_TIME IS NOT NULL AND b.OWNER_TYPE_ID = {$ownerTypeID} AND b.OWNER_ID = {$ownerID} ORDER BY a.START_TIME ASC";
		}
		else
		{
			$sql = "SELECT a.ID, {$startTime} AS START_TIME_FORMATTED, a.START_TIME FROM {$tableName} a INNER JOIN {$bindingTableName} b ON a.ID = b.ACTIVITY_ID AND a.COMPLETED = 'N' AND a.START_TIME IS NOT NULL AND b.OWNER_TYPE_ID = {$ownerTypeID} AND b.OWNER_ID = {$ownerID} ORDER BY a.START_TIME ASC";
		}
		...
		if($arResult)
		{
			$arResult['START_TIME'] = $arResult['START_TIME_FORMATTED'];
			unset($arResult['START_TIME_FORMATTED']);
		}
		...
		else
		{
			...
			CCrmActivity::DoSaveNearestUserActivity(
				array(
					...
					'OWNER_ID' => $ownerID,
					'OWNER_TYPE_ID' => $ownerTypeID,
					'ACTIVITY_ID' => $arResult['ID'],
					'ACTIVITY_TIME' => $arResult['START_TIME']
					//'QTY' => 0
				)
			);
		}
	...
	public static function MakeRawFiles($storageTypeID, $arElementIDs)
	{
		$arRawFiles = array();
		if($storageTypeID === CCrmActivityStorageType::File)
		{
			foreach($arElementIDs as $fileID)
			{
	...
	protected static function SaveCalendarEvent(&$arFields)
	{
		...
		if($eventID > 0 && $ownerID > 0 && $ownerTypeID > 0)
		{
	...
	protected static function SaveTask(&$arFields)
	{
		...
		if(isset($arFields['COMPLETED']))
		{
			$arTaskFields['STATUS'] = $arFields['COMPLETED'] === 'Y' ? 5 : 1;
		}
...
class CCrmActivityDirection
{
	...
	public static function GetAllDescriptions($typeID)
	{
...
class CCrmActivityStorageType
{
	...
	{
		$typeID = intval($typeID);
		return $typeID > self::Undefined && $typeID <= self::WebDav;
	}
}
