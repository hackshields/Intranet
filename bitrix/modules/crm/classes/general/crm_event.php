...
class CCrmEvent
{
	...
	public function Share($srcEntity, $dstEntities, $typeName)
	{
		...
		if($typeName === 'MESSAGE')
		{
			$dbResult = $DB->Query("SELECT ID FROM b_crm_event WHERE ID IN (SELECT EVENT_ID FROM b_crm_event_relations WHERE ENTITY_TYPE = '{$srcEntityType}' AND ENTITY_ID = {$srcEntityID}) AND (EVENT_TYPE = 2 OR EVENT_ID = 'MESSAGE')");
		}
	...
	public function AddRelation($EVENT_ID, $arFields, $bPermCheck = true)
	{
		...
		foreach ($arFields as $arRel)
		{
			...
			if (!in_array($arRel['ENTITY_TYPE'], Array('LEAD', 'CONTACT', 'COMPANY', 'DEAL')))
				continue;
	...
	public function RemoveRelation($RELATION_ID, $ENTITY_TYPE, $bPermCheck = true)
	{
		...
		if (!in_array($ENTITY_TYPE, Array('LEAD', 'CONTACT', 'COMPANY', 'DEAL')))
			return false;
	...
	static public function BuildPermSql($sAliasPrefix = 'CE', $mPermType = 'READ')
	{
		$sSqlPerm = '';
		...
		if (empty($arFilter['ENTITY_TYPE']))
			$arEntity = array('LEAD', 'DEAL', 'CONTACT', 'COMPANY');
		...
		{
			$arUserAttr[$entityType] = $CCrmPerms->GetUserAttrForSelectEntity($entityType, 'READ');
		}
		...
		if (!empty($arPermission))
			$sSqlPerm = implode(' OR ', $arPermission);
