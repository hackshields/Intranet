...
class CCrmContactWS extends IWebService
{
	...
	public function GetListItemChangesSinceToken($listName, $viewFields = '', $query = '', $rowLimit = 0, $changeToken = '')
	{
		...
		CTimeZone::Disable();
		$obContact = CCrmContact::GetList(array(), $arFilter, array());
		CTimeZone::Enable();

		if ($rowLimit > 0)
		{
			$obContact->NavStart($rowLimit, false, $page);
		}
		...
		$arContacts = array();
		$arContact2Company = array();
		while ($arContact = $obContact->NavNext())
		{
			$counter++;
			$arContacts[$arContact['ID']] = $arContact;
			$arContact2Company[$arContact['COMPANY_ID']] = $arContact['ID'];
		}
		...
		if (!empty($arCID))
		{
			$res = CCrmCompany::GetList(array(), array('ID' => array_keys($arContact2Company)), array('ID', 'TITLE'));
			while($ar = $res->Fetch())
				$arContacts[$arContact2Company[$ar['ID']]]['COMPANY'] = $ar['TITLE'];
	...
	public function Add($data)
	{
		...
		if (CCrmUrlUtil::HasScheme($arFields['PHOTO'])
			&& CCrmUrlUtil::IsSecureUrl($arFields['PHOTO']))
		{
			$arFields['PHOTO'] = CFile::MakeFileArray($arFields['PHOTO']);
			if (is_array($arFields['PHOTO']))
			{
				$arFields['PHOTO'] += array('MODULE_ID' => 'crm');
			}
		}
		...
		foreach($arUserFields as $FIELD_NAME => $arUserField)
		{
			if ($arUserField['USER_TYPE']['BASE_TYPE'] == 'file')
			{
				...
				$arFiles = is_array($arFields[$FIELD_NAME]) ? $arFields[$FIELD_NAME] : array($arFields[$FIELD_NAME]);
