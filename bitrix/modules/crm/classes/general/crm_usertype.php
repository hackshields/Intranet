...
class CCrmUserType
{
	...
	public function AddFields(&$arFilterFields, $ID, $sFormName = 'form1', $bVarsFromForm = false, $bShow = false, $bParentComponent = false, $arOptions = array())
	{
		...
		foreach($arUserFields as $FIELD_NAME => $arUserField)
		{
			...
			{
				if($bShow && $userTypeID === 'file' && $fileUrlTemplate !== '')
				{
				...
				else
				{
					ob_start();
					$APPLICATION->IncludeComponent(
						'bitrix:system.field.'.($bShow ? 'view' : 'edit'),
						$userTypeID,
						array(
							'arUserField' => $arUserField,
							'bVarsFromForm' => $bVarsFromForm,
			...
			$arFilterFields[] = array(
				...
				'type' => 'custom',
				'value' => $sVal,
				'required' => !$bShow && $arUserField['MANDATORY'] == 'Y' ? true : false
			);
			$count++;
		}
	...
	private static function TryResolveFile(&$path, &$file)
	{
	...
	public function Internalize($name, $data, $delimiter = ',')
	{
		...
		if ($arUserField['USER_TYPE']['BASE_TYPE'] === 'file')
		{
			if(is_array($data))
			{
				...
				{
					$file = null;
					if(self::TryResolveFile($filePath, $file))
					{
						$files[] = $file;
					}
				}
			...
			{
				$file = null;
				if(self::TryResolveFile($data, $file))
				{
					$data = $file;
				}
			}
		...
		elseif($arUserField['USER_TYPE']['USER_TYPE_ID'] === 'employee')
		{
	...
	public function ListAddEnumFieldsValue($arParams, &$arValue, &$arReplaceValue, $delimiter = '<br />', $textonly = false)
	{
		global $APPLICATION;
		$arUserFields = $this->cUFM->GetUserFields($this->sEntityID, 0, LANGUAGE_ID);
		$bSecondLoop = false;
			...
			foreach ($arValue as $ID => $data)
			{
				...
				if ($arUserField['USER_TYPE']['USER_TYPE_ID'] == 'boolean')
				{
					...
					foreach ($arVal as $val)
					{
						$val = (string)$val;
						if (strlen($val) <= 0)
							$val = $arUserField['SETTINGS']['DEFAULT_VALUE'];
				...
				foreach ($VALUE['FIELD'] as $ID => $arFIELD_NAME)
				{
					foreach ($arFIELD_NAME as $FIELD_NAME => $FIELD_VALUE)
					{
						foreach ($FIELD_VALUE as $FIELD_VALUE_NAME => $FIELD_VALUE_ID)
						{
							...
							{
								$sname = $textonly ? CFile::GetFileSRC($arList[$KEY][$FIELD_VALUE_ID]) : '<a href="'.CFile::GetFileSRC($arList[$KEY][$FIELD_VALUE_ID]).'" target="_blank">'.htmlspecialcharsbx($arList[$KEY][$FIELD_VALUE_ID]['FILE_NAME']).'</a>';
								$arReplaceValue[$ID][$FIELD_NAME] .= (!empty($arReplaceValue[$ID][$FIELD_NAME]) ? $delimiter : '').$sname;
							}
							else if ($KEY == 'crm')
							{
								foreach($FIELD_VALUE_ID as $CID)
								{
									...
									if(!$textonly)
									{
										$tooltip = '<script type="text/javascript">BX.tooltip('.$CID.', "balloon_'.$ID.'_'.$FIELD_NAME.'_'.$FIELD_VALUE_NAME.'_'.$CID.'", "/bitrix/components/bitrix/crm.'.strtolower($FIELD_VALUE_NAME).'.show/card.ajax.php", "crm_balloon'.($FIELD_VALUE_NAME == 'LEAD' || $FIELD_VALUE_NAME == 'DEAL'? '_no_photo': '_'.strtolower($FIELD_VALUE_NAME)).'", true);</script>';
	...
	public function AddBPFields(&$arHeaders)
	{
		$arUserFields = $this->cUFM->GetUserFields($this->sEntityID, 0, LANGUAGE_ID);
		foreach($arUserFields as $FIELD_NAME => $arUserField)
		{
			...
			$arHeaders[$FIELD_NAME] = array(
				//'Name' => htmlspecialcharsbx($arUserField['EDIT_FORM_LABEL']),
				'Name' => ('' != trim($arUserField['EDIT_FORM_LABEL']) ? $arUserField['EDIT_FORM_LABEL'] : $arUserField['FIELD_NAME']),
				'Options' => $editable,
				'Type' => $sType,
	...
	public function PrepareUpdate(&$arFields)
	{
		...
		foreach($arUserFields as $FIELD_NAME => $arUserField)
		{
			if ($arUserField['USER_TYPE']['USER_TYPE_ID'] == 'boolean' && isset($arFields[$FIELD_NAME]))
			{
			...
			else if ($arUserField['USER_TYPE']['USER_TYPE_ID'] == 'employee' && $arUserField['MULTIPLE'] == 'N')
			{
				if (is_array($arFields[$FIELD_NAME]))
					list(, $arFields[$FIELD_NAME]) = each($arFields[$FIELD_NAME]);
			}
			else if ($arUserField['USER_TYPE']['USER_TYPE_ID'] == 'crm' && isset($arFields[$FIELD_NAME]))
			{
				if (!is_array($arFields[$FIELD_NAME]))
