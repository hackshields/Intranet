...
class CCrmEMail
{
	...
	public static function EmailMessageAdd($arMessageFields, $ACTION_VARS)
	{
		...
		$targActivity = $targInfo['ID'] > 0 ? CCrmActivity::GetByID($targInfo['ID']) : null;
		...
		else
		{
			...
			$arTypeNames = CCrmOwnerType::GetAllNames();
		...
		// Skip lead creation if email list is empty. Overwise we will create lead with no email-addresses. It is absolutely useless.
		...
		if(empty($arBindingData) && $emailQty > 0)
		{
			...
			if($userID > 0)
			{
				$arLeadFields['CREATED_BY_ID'] = $arLeadFields['MODIFY_BY_ID'] = $arLeadFields['ASSIGNED_BY_ID'] = $userID;
			}
			...
			$leadID = $leadEntity->Add($arLeadFields, true, array('DISABLE_USER_FIELD_CHECK' => true));
			// TODO: log error
			if($leadID > 0)
			{
		...
		foreach($arFilesData as $fileData)
		{
			...
			if($fileID > 0)
			{
				$elementID = CCrmWebDavHelper::SaveEmailAttachment(CFile::GetFileArray($fileID), '');
		...
		//$descr = $body;
		//$descr = preg_replace('/(<br[^>]*>)+/i', PHP_EOL, $descr);
		//$descr = strip_tags($descr);

		$sanitaizer = new CBXSanitizer();
		$sanitaizer->SetLevel(CBXSanitizer::SECURE_LEVEL_MIDDLE);
		$descr = $sanitaizer->SanitizeHtml(preg_replace("/(\r\n|\n|\r)+/", '<br/>', $body));
		...
		$arActivityFields = array(
			...
			'STORAGE_TYPE_ID' => CCrmActivityStorageType::WebDav,
		...
		$activityID = CCrmActivity::Add($arActivityFields, false, false);
	...
	public static function BeforeSendMail($arMessageFields)
	{
		...
		$dbActivity = CCrmActivity::GetList(
			array(),
			array('=TYPE_ID' => CCrmActivityType::Email, '=ASSOCIATED_ENTITY_ID' => $postingID),
			false,
			false,
			array('SETTINGS')
		);
		...
		$settings = isset($arActivity['SETTINGS']) ? unserialize($arActivity['SETTINGS']) : array();
