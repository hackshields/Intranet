...
class CCrmExternalSaleImport
{
	...
	public function SyncOrderData($bSkipBP = false, $bSkipNotify = false)
	{
		...
		$modificationLabelTmp = $modificationLabel = intval($this->arExternalSale["LABEL"]);
	...
	private function SaveOrderDataContact($arOrder)
	{
		...
		if (is_array($arOrder["CONTRACTOR"]["CONTACTS"]))
		{
			...
			$arMapTmp = array('MAIL' => "EMAIL", 'WORKPHONE' => "PHONE");
			$arInc = array();

			foreach ($arOrder["CONTRACTOR"]["CONTACTS"] as $val)
			{
				$t = strtoupper($val["TYPE"]);
				if (array_key_exists($t, $arMapTmp))
				{
					$bFound = false;
					$tNew = $arMapTmp[$t];
	...
	private function SaveOrderDataDeal($arOrder, $contactId = null, $companyId = null)
	{
		...
		if (array_key_exists("NEW", $arStageList))
		...
		if ($dealId == 0)
		{
			$arFields['TITLE'] = sprintf("%s #%s", $this->arExternalSale["PREFIX"], $arOrder["ID"]);
	...
	private function SaveOrderDataProducts($arOrder, $dealId)
	{
		...
		{
			$productId = 0;
			$dbProduct = CCrmProduct::GetList(array(), array("ORIGINATOR_ID" => $this->externalSaleId, "ORIGIN_ID" => $arItem["ID"], "CHECK_PERMISSIONS" => "N"));
		...
		if (is_array($arOrder["TAXES"]))
		{
			foreach ($arOrder["TAXES"] as $arItem)
			{
				...
				$productId = 0;
				$dbProduct = CCrmProduct::GetList(array(), array("ORIGINATOR_ID" => $this->externalSaleId, "ORIGIN_ID" => "tax_".$arItem["NAME"], "CHECK_PERMISSIONS" => "N"));
				if ($arProduct = $dbProduct->Fetch())
					$productId = $arProduct["ID"];
		...
		if (is_array($arOrder["DISCOUNTS"]))
		{
			foreach ($arOrder["DISCOUNTS"] as $arItem)
			{
				...
				$productId = 0;
				$dbProduct = CCrmProduct::GetList(array(), array("ORIGINATOR_ID" => $this->externalSaleId, "ORIGIN_ID" => "discount_".$arItem["NAME"], "CHECK_PERMISSIONS" => "N"));
				if ($arProduct = $dbProduct->Fetch())
					$productId = $arProduct["ID"];
	...
	private function QueryOrderData($arFilter)
	{
		$s = "";
		if (!empty($_SERVER["HTTP_HOST"]))
			$s = ($GLOBALS["APPLICATION"]->IsHTTPS() ? "https" : "http")."://".$_SERVER["HTTP_HOST"];
		...
		{
			$ar = array();
			foreach ($this->proxy->GetErrors() as $err)
				$ar[] = sprintf("[%s] %s", $err[0], $err[1]);
			CCrmExternalSale::Update($this->externalSaleId, array("LAST_STATUS" => implode(" ", $ar)));

			$this->AddError("GD1", implode(" ", $ar));
			return null;
		}
