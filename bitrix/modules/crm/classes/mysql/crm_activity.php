...
class CCrmActivity extends CAllCrmActivity
{
	...
	public static function PrepareBindingsFilterSql(&$arBindings, $tableAlias = '')
	{
		...
		foreach($arBindings as &$binding)
		{
			...
			if($ownerID > 0)
			{
				$sql .= "(({$tableAlias}.OWNER_TYPE_ID = {$ownerTypeID} AND {$tableAlias}.OWNER_ID = {$ownerID}) OR EXISTS (SELECT B.ID FROM {$bindingTableName} B WHERE B.ACTIVITY_ID = {$tableAlias}.ID AND {$s} LIMIT 1))";
			}
			else
			{
				$sql .= "({$tableAlias}.OWNER_TYPE_ID = {$ownerTypeID} OR EXISTS (SELECT B.ID FROM {$bindingTableName} B WHERE B.ACTIVITY_ID = {$tableAlias}.ID AND {$s} LIMIT 1))";
			}
		}
		unset($binding);
	...
	public static function DoSaveElementIDs($ID, $storageTypeID, $arElementIDs)
	{
		...
		{
			$DB->Query(
				'INSERT INTO '.self::ELEMENT_TABLE_NAME.'('.$bulkColumns.') VALUES'.$query,
				false,
				'File: '.__FILE__.'<br/>Line: '.__LINE__
			);
		}
	...
	public static function DoSaveNearestUserActivity($arFields)
	{
		...
		$sql = "INSERT INTO b_crm_usr_act(USER_ID, OWNER_ID, OWNER_TYPE_ID, ACTIVITY_TIME, ACTIVITY_ID, DEPARTMENT_ID)
			VALUES({$userID}, {$ownerID}, {$ownerTypeID}, {$activityTime}, {$activityID}, 0)
			ON DUPLICATE KEY UPDATE ACTIVITY_TIME = {$activityTime}, ACTIVITY_ID = {$activityID}";
