...
global $arSocNetAllowedSubscribeEntityTypesDesc, $arSocNetLogEvents, $arSocNetFeaturesSettings;
...
foreach ($arSocNetLogEvents as $event_id => $arEventTmp)
	if (!$arEventTmp["HIDDEN"] && array_key_exists("ENTITIES", $arEventTmp) && is_array($arEventTmp["ENTITIES"]))
		foreach ($arEventTmp["ENTITIES"] as $entity_type => $arEventEntityTmp)
			$arEvents[$entity_type][$event_id] = $arEventEntityTmp["TITLE"];
	...
	if(isset($_REQUEST["save"]) || isset($_REQUEST["apply"]))
	{
		...
		if($res > 0)
		{
			if ($_POST["ENTITY_TYPE"] == SONET_SUBSCRIBE_ENTITY_PROVIDER)
			{
				...
				elseif (is_array($_POST["RIGHTS_USER_ID"]))
...
if (!$scheme_type)
	$scheme_type = htmlspecialcharsbx($_REQUEST["TYPE"]);
...
?>
	...
	{?>
		<tr>
			<td><?echo GetMessage("LFP_SCHEME_EDIT_ID")?>:</td>
			<td><?echo $ID;?></td>
		</tr>
	<?}?>
	<tr>
		<td><?echo GetMessage("LFP_SCHEME_EDIT_TYPE")?>:</td>
		<td><?echo $scheme_type;?></td>
	</tr>
	<tr>
		<td><?echo GetMessage("LFP_SCHEME_EDIT_ACTIVE")?>:</td>
		<td width="60%"><input type="checkbox" name="ACTIVE" value="Y"<?if($arRes["ACTIVE"] === "Y") echo " checked"?>></td>
	</tr>
	<tr id="LID_ROW">
		<td><?echo GetMessage("LFP_SCHEME_EDIT_SITE")?></td>
		<td>
				...
				BX.ready(function()
					{
						...
						BX.bind(LIDInput, 'change', function() { 
							...
							{
								var arGroupSiteSpan = BX.findChildren(BX('LF_ENTITY_ID_NEWS_ROW'), {'tag':'span'}, true);
								for (var i = 0; i < arGroupSiteSpan.length; i++)
									arGroupSiteSpan[i].style.display = 'none';
							}
	...
	<tr class="adm-detail-required-field">
		<td><?echo GetMessage("LFP_SCHEME_EDIT_NAME")?>:</td>
		<td><input type="text" size="50" maxlength="100" name="NAME" value="<?=htmlspecialcharsbx($arRes["NAME"])?>"></td>
	</tr>
	<tr>
		<td><?echo GetMessage("LFP_SCHEME_EDIT_SORT")?>:</td>
		<td><input type="text" size="6" name="SORT" value="<?echo intval($arRes["SORT"])?>"></td>
	</tr>
	<tr class="heading">
		<td colspan="2"><?echo GetMessage("LFP_SCHEME_EDIT_DESTINATION")?></td>
	</tr>
	<tr class="adm-detail-required-field">
		<td><?echo GetMessage("LFP_SCHEME_EDIT_ENTITY_TYPE")?>:</td>
		<td>
		...
		<script type="text/javascript">
			var arSchemeEventTypes = new Array();
			var arSchemeEvents = new Array();
			<?
			foreach ($arEvents as $entity_type => $arTmp)
			{
				...
				foreach ($arTmp as $event_id)
					echo "arSchemeEvents['". $entity_type."'][arSchemeEvents['". $entity_type."'].length] = '".$event_id."';\n";
			}
			?>
			if ('__LFPSchemeChangeEntityType' != typeof window.noFunc) 
			{
				function __LFPSchemeChangeEntityType(el)
				{
					...
					else
					{
						...
						for (var i = 0; i < arEventsDiv.length; i++)
					...
					for (var i = 0; i < arRightsDiv.length; i++)
	...
	<tr id="LF_ENTITY_ID_ROW" style="display: <?=(in_array($arRes["ENTITY_TYPE"], array(SONET_SUBSCRIBE_ENTITY_GROUP, SONET_SUBSCRIBE_ENTITY_USER, SONET_SUBSCRIBE_ENTITY_NEWS, SONET_SUBSCRIBE_ENTITY_PROVIDER)) ? "none" : "table-row")?>;">
		<td><?echo GetMessage("LFP_SCHEME_EDIT_ENTITY_ID")?>:</td>
		<td>
		<input type="text" size="3" name="ENTITY_ID" id="LF_ENTITY_ID" value="<?echo intval($arRes["ENTITY_ID"])?>">
		<script type="text/javascript">
			...
			function __RecalcEntityDesc(e)
			{
				...
				{
					BX.ajax.post("xdi_lf_scheme_getentity.php", "entity_type="+entity_type+"&entity_id="+entity_id, function(result)
					{
		...
		foreach($arSites as $site_id_tmp)
		{
			if (array_key_exists($site_id_tmp, $arSocNetGroupTmp) && count($arSocNetGroupTmp[$site_id_tmp]["REFERENCE_ID"]) > 0)
			{
				?><span id="LF_ENTITY_ID_GROUP_<?=$site_id_tmp?>_ROW" style="display: <?=($arRes["LID"] == $site_id_tmp ? "inline-block" : "none")?>;"><?
					echo SelectBoxFromArray("ENTITY_ID_GROUP_".$site_id_tmp, $arSocNetGroupTmp[$site_id_tmp], $arRes["ENTITY_ID"], "", "", false, "editform");
				?></span><?
			}
		}
		?></td>
			...
			foreach($arSites as $site_id_tmp)
			{
				if (array_key_exists($site_id_tmp, $arIBlockTmp) && count($arIBlockTmp[$site_id_tmp]["REFERENCE_ID"]) > 0)
				{
					?><span id="LF_ENTITY_ID_NEWS_<?=$site_id_tmp?>_ROW" style="display: <?=($arRes["LID"] == $site_id_tmp ? "inline-block" : "none")?>;"><?
						echo SelectBoxFromArray("ENTITY_ID_NEWS_".$site_id_tmp, $arIBlockTmp[$site_id_tmp], $arRes["ENTITY_ID"], "", "", false, "editform");
					?></span><?
				}
			}
			?></td>
	...
	?>
	<tr id="LF_EVENT_ID_ROW" style="display: <?=(in_array($arRes["ENTITY_TYPE"], array(SONET_SUBSCRIBE_ENTITY_GROUP, SONET_SUBSCRIBE_ENTITY_USER)) ? "table-row" : "none")?>;">
		<td class="adm-detail-required-field"><?echo GetMessage("LFP_SCHEME_EDIT_EVENT_ID")?>:</td>
		...
		foreach ($arEntityTypes as $entity_type => $entity_type_title):
			...
			<div id="LF_EVENT_ID_<?=$entity_type?>_DIV" style="display: <?=($entity_type == $arRes["ENTITY_TYPE"] ? "block" : "none")?>;">			
				<select name="EVENT_ID_<?=$entity_type?>">
					...
					<? foreach ($arEvents[$entity_type] as $event_id => $event_title):?>
						<option value="<?=$event_id?>"<?=($event_id == $arRes["EVENT_ID"] ? " selected" : "")?>>[<?=$event_id?>] <?=$event_title?></option>
					<? endforeach;?>
				</select>
			</div>
			<?
	...
	<tr id="LF_RIGHTS_ROW" style="display: <?=($arRes["ENTITY_TYPE"] == SONET_SUBSCRIBE_ENTITY_PROVIDER ? "table-row" : "none")?>;" class="adm-detail-required-field">
		<td class="adm-detail-valign-top"><?echo GetMessage("LFP_SCHEME_EDIT_RIGHTS")?>:</td>
			...
			if ($entity_type == SONET_SUBSCRIBE_ENTITY_PROVIDER):
				...
				<div id="LF_RIGHTS_<?=$entity_type?>_DIV" style="display: <?=($entity_type == $arRes["ENTITY_TYPE"] ? "block" : "none")?>;">			
					...
					<span id="LF_RIGHTS_USER_US" style="display: <?=(is_array($arRes["RIGHTS_USER_ID"]) && !in_array($arRes["RIGHTS_USER_ID"][0], array("UA", "UN")) ? "block" : "none")?>;">
						...
						<tbody>
						<?
						if (is_array($arRes["RIGHTS_USER_ID"]))
						{
						...
						?>
						<tr>
							<td><a href="javascript:void(0)" onclick="lfpAddUser(this)" hidefocus="true" class="bx-action-href"><?echo GetMessage("LFP_SCHEME_EDIT_RIGHTS_P_ADD")?></a></td>
						</tr>
						</tbody>
						</table>
					</span>
	...
	<tr id="LF_ENABLE_COMMENTS_ROW" style="display: <?=(in_array($arRes["ENTITY_TYPE"], array(SONET_SUBSCRIBE_ENTITY_PROVIDER)) ? "table-row" : "none")?>;">
		<td><?echo GetMessage("LFP_SCHEME_EDIT_ENABLE_COMMENTS")?>:</td>
		<td width="60%"><input type="checkbox" name="ENABLE_COMMENTS" value="Y"<?if($arRes["ENABLE_COMMENTS"] === "Y") echo " checked"?>></td>
	</tr>
	<?
	if (in_array($scheme_type, array("XML", "RSS")))
	{
		?>
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("LFP_SCHEME_EDIT_SOURCE")?></td>
		</tr>
		<?
		if ($scheme_type == "XML")
		{
		...
		<tr id="LF_URI_ROW" style="display: <?=(strlen($predefined) > 0 ? "none" : "table-row")?>;" class="adm-detail-required-field">
			...
			<td><input id="LF_URI" type="text" size="50" name="URI" value="<?echo $arRes["URI"]?>"></td>
		</tr>
		<tr id="LF_HOST_ROW" style="display: <?=(strlen($predefined) > 0 ? "table-row" : "none")?>;" class="adm-detail-required-field">
			...
			<td>
				<input id="LF_HOST" type="text" size="50" name="HOST" value="<?=$arRes["HOST"].(intval($arRes["PORT"]) > 0 ? ":".$arRes["PORT"] : "")?>">
				<input id="LF_PAGE" type="hidden" name="PAGE" value="<?echo $arRes["PAGE"]?>">
			</td>
		</tr>
		<?
		if ($scheme_type == "XML")
		{
			?><tr id="LF_METHOD_ROW" style="display: <?=(strlen($predefined) > 0 ? "none" : "table-row")?>;">
				...
				<td><input type="text" id="LF_METHOD" size="50" name="METHOD" value="<?echo $arRes["METHOD"]?>"></td>
			</tr>
			<tr>
				<td><?=GetMessage("LFP_SCHEME_EDIT_METHOD_PARAMS")?>:</td>
				<td><input type="text" id="LF_PARAMS" size="50" name="PARAMS" value="<?echo $arRes["PARAMS"]?>"></td>
			</tr>
			<tr>
				<td><?echo GetMessage("LFP_SCHEME_EDIT_LOGIN")?>:</td>
				<td><input type="text" size="20" name="LOGIN" value="<?echo $arRes["LOGIN"]?>"></td>
			</tr>
			<tr>
				<td><?echo GetMessage("LFP_SCHEME_EDIT_PASSWORD")?>:</td>
				<td><input type="password" size="20" name="PASSWORD" value="<?echo $arRes["PASSWORD"]?>"></td>
			</tr><?
		}
	}
	elseif (in_array($scheme_type, array("POST")))
	{
		?>
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("LFP_SCHEME_EDIT_POINT_PARAMS")?></td>
		</tr>
		<tr>
				...
				<tbody>
					<tr>
						...
						<td>
							...
							{
								?>
								<span id="LF_HASH_SPAN"><?=$arRes["HASH"]?></span>
								<input type="hidden" id="LF_HASH" name="HASH" value="<?=$arRes["HASH"]?>">
...
if (in_array($scheme_type, array("XML", "RSS")))
{
	?>
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("LFP_SCHEME_EDIT_SCHEDULE")?></td>
		</tr>
		<tr class="adm-detail-required-field">
			<td width="40%"><?echo GetMessage("LFP_SCHEME_EDIT_LAST_EXECUTED").":"?></td>
			<td width="60%"><?echo CalendarDate("LAST_EXECUTED", $arRes["LAST_EXECUTED"], "editform", "20")?></td>
		</tr>
		<tr>
			<td><?echo GetMessage("LFP_SCHEME_EDIT_DOM")?></td>
			<td><input class="typeinput" type="text" name="DAYS_OF_MONTH" value="<?echo $arRes["DAYS_OF_MONTH"];?>" size="30" maxlength="100"></td>
		</tr>
		<tr>
			<td class="adm-detail-valign-top"><?echo GetMessage("LFP_SCHEME_EDIT_DOW")?></td>
			<td><table cellspacing=1 cellpadding=0 border=0 class="internal"><?	
				$arDoW = array(
		...
		<tr class="adm-detail-required-field">
			<td><?echo GetMessage("LFP_SCHEME_EDIT_TOD")?></td>
			<td><input type="text" name="TIMES_OF_DAY" value="<?echo $arRes["TIMES_OF_DAY"];?>" size="30" maxlength="255"></td>
		</tr>
	<?
}
$tabControl->Buttons(
...
<input type="hidden" name="TYPE" value="<?=$scheme_type?>">
<?
$tabControl->End();
?>
</form>
