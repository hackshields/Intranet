...
CModule::AddAutoloadClasses(
	...
	array(
		...
		"CXDILFScheme" => "classes/general/lf_scheme.php",
		...
		"CXDILFSchemePOST" => "classes/general/lf_scheme_post.php",
	)
);
...
class CXDILFEventHandlers
{
	...
	function FormatEvent_Data($arFields, $arParams, $bMail = false)
	{
		if ($arParams["MOBILE"] != "Y")
			$GLOBALS["APPLICATION"]->SetAdditionalCSS("/bitrix/themes/.default/xdi_sonet_log.css");
		...
		elseif (in_array($arFields["ENTITY_TYPE"], array(SONET_SUBSCRIBE_ENTITY_GROUP, SONET_SUBSCRIBE_ENTITY_USER)))
		{
			...
			if ($entity_type == SONET_SUBSCRIBE_ENTITY_GROUP) // group
			{
				$arDestination = array(
					array(
						"STYLE" => "sonetgroup",
		...
		$arResult["EVENT_FORMATTED"] = array(
			...
			"STYLE" => ($arFields["ENTITY_TYPE"] == SONET_SUBSCRIBE_ENTITY_PROVIDER ? "important" : ""),
