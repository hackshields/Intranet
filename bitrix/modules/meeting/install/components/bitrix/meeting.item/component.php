...
while ($arInstance = $dbRes->Fetch())
{
	...
	if (
		...
		&& $_REQUEST['save']
		&& $_REQUEST['INSTANCE_ID'] == $arInstance['ID'] && in_array($USER->GetID(), $arInstance['RESPONSIBLE'])
		&& check_bitrix_sessid()
	)
	{
		...
		if ($res)
		{
			CMeetingReports::SetFiles($REPORT_ID, $_REQUEST['FILES']);
		}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_REQUEST['save_item'] && check_bitrix_sessid())
{
	...
	$arFields['FILES'] = is_array($_REQUEST['ITEM_FILES']) ? $_REQUEST['ITEM_FILES'] : array();
