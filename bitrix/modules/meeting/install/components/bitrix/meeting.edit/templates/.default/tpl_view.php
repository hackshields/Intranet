...
<?
//echo '<pre>'; print_r($arResult['MEETING']); echo '</pre>';
...
?>
<div class="meeting-title-buttons">
	<a href="<?=$arParams['LIST_URL']?>" class="meeting-title-button">
		<i class="meeting-title-button-back-icon"></i><span class="meeting-link"><?=GetMessage('ME_LIST_TITLE')?></span>
	</a>
</div>
<?
$this->EndViewTarget();
...
if ($arResult['CAN_EDIT']):
?>
<script type="text/javascript">window.BXMEETINGCANEDIT = true;</script>
<div id="meeting_toolbar" class="meeting-toolbar toolbar-<?=$arResult['MEETING']['CURRENT_STATE']?>">
	...
	<div class="meeting-agenda-bot-return">
		<a href="<?=$arParams['MEETING_COPY_URL']?>" class="meeting-dash-link"><?=GetMessage('ME_COPY')?></a>
		&nbsp;
		<a onclick="meetingAction('<?=$arParams['MEETING_ID']?>', {state: '<?=CMeeting::STATE_PREPARE?>'})" href="javascript:void(0)" class="meeting-dash-link"><?=GetMessage('ME_PREPARE')?></a>
	</div>
</div>
<?
endif;
