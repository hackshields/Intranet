...
__IncludeLang($_SERVER['DOCUMENT_ROOT'].$this->GetFolder().'/lang/'.LANGUAGE_ID.'/agenda.php');
...
?>
		<div class="meeting-agenda-add-item-wrap meeting-agenda-agenda-visible">
...
<span class="meeting-ag-add-qu-form"><span class="meeting-ag-add-input-wrap">';
	...
	{
		h += '\
<input type="text" name="AGENDA_TITLE['+key+']" value="' + (data.TITLE || '<?=CUtil::JSEscape(GetMessage('ME_MEETING_TITLE_DEFAULT_1'))?>') + '" class="meeting-ag-add-input'+(!!data.TITLE ? ' meeting-ag-add-inp-active' : '')+'" onblur="if(this.value==\'\'||this.value==\'<?=addslashes(CUtil::JSEscape(GetMessage('ME_MEETING_TITLE_DEFAULT_1')))?>\') {this.value=\'<?=addslashes(CUtil::JSEscape(GetMessage('ME_MEETING_TITLE_DEFAULT_1')))?>\'; BX.removeClass(this,\'meeting-ag-add-inp-active\')}" onfocus="if(this.value==\'<?=addslashes(CUtil::JSEscape(GetMessage('ME_MEETING_TITLE_DEFAULT_1')))?>\') this.value=\'\'; BX.addClass(this, \'meeting-ag-add-inp-active\')" />';
