...
";
	...
	public static function Add($arFields)
	{
		...
		$e = GetModuleEvents('meeting', 'OnBeforeMeetingAdd');
		while ($a = $e->Fetch())
		{
		...
		if ($ID > 0)
		{
			...
			$e = GetModuleEvents('meeting', 'OnAfterMeetingAdd');
			while ($a = $e->Fetch())
	...
	public static function Update($ID, $arFields)
	{
		...
		$e = GetModuleEvents('meeting', 'OnBeforeMeetingUpdate');
		while ($a = $e->Fetch())
		{
			if (false === ExecuteModuleEventEx($a, array(&$arFields)))
			{
		...
		if ($dbRes)
		{
			if (isset($arFields['DESCRIPTION'])||isset($arFields['PROTOCOL_TEXT']))
			{
				$arBind = array();
			...
			$e = GetModuleEvents('meeting', 'OnAfterMeetingUpdate');
			while ($a = $e->Fetch())
	...
	public static function Delete($ID)
	{
		...
		{
			$rsEvents = GetModuleEvents("meeting", "OnBeforeMeetingDelete");
			...
			if ($DB->Query("DELETE FROM b_meeting WHERE ID='".$ID."'"))
			{
				...
				$rsEvents = GetModuleEvents("meeting", "OnAfterMeetingDelete");
				while ($arEvent = $rsEvents->Fetch())
	...
	public static function AddEvent($MEETING_ID, $arFields, $arParams = array())
	{
		...
		elseif (!$arFields['EVENT_ID'])
		{
			...
			$guestSection = CEventCalendar::GetSectionIDByOwnerId($USER->GetID(), 'USER', $iblockId);
			$arGuestCalendars = array();

			if(!$guestSection) // Guest does not have any calendars
			{
				$guestSection = CEventCalendar::CreateSectionForOwner($USER->GetID(), "USER", $iblockId);
			}

			$arGuestCalendars = CEventCalendar::GetCalendars(array(
			...
			{
				$arUserSet = CEventCalendar::GetUserSettings(array('static' => false, 'userId' => $USER->GetID()));
	...
	public static function GetFilesData($arInput, $arFrom = null)
	{
		...
		if (is_array($arInput) && count($arInput) > 0)
		{
			...
			{
				$fileSrc = intval($arInput[$arFile['ID']]['FILE_SRC']);
