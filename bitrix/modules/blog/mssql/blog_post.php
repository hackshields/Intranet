...
class CBlogPost extends CAllBlogPost
{
	...
	function GetByID($ID)
	{
		...
		else
		{
			...
			if (!isset($strSql))
			$strSql =
				"SELECT P.*, CASE WHEN P.DATE_PUBLISH <= GETDATE() THEN 'Y' ELSE 'N' END as DATE_PUBLISHED, ".
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		if ($blogModulePermissions < "W")
		{
			if(!CBlog::IsBlogOwner($arFilter["BLOG_ID"], $GLOBALS["USER"]->GetID()))
			{
				...
				{
						$strSqlExist = "select UGP.PERMS from b_blog_user_group_perms UGP WHERE P.ID = UGP.POST_ID AND P.BLOG_ID = UGP.BLOG_ID AND UGP.USER_GROUP_ID IN (".$strUserGroups.") AND UGP.PERMS_TYPE = '".BLOG_PERMS_POST."'";
		...
		if (is_array($arNavStartParams) && IntVal($arNavStartParams["nTopCount"])<=0)
		{
			...
			{
				if(strlen($strSqlExist) > 0 && strlen($arSqls["WHERE"]) <= 0)
					$strSql_tmp .= " exists (".$strSqlExist.") ";
				elseif(strlen($strSqlExist) > 0)
					$strSql_tmp .= " AND exists (".$strSqlExist.") ";
			}
