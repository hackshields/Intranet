	...
	extends CBPAllStateService
{
	public function SetState($workflowId, $arState, $arStatePermissions = array())
	{
		...
		$strSql = 
			...
			"	STATE_PARAMETERS = EMPTY_CLOB(), ".
			"	MODIFIED = ".$DB->CurrentTimeFunction()." ".
			"WHERE ID = '".$DB->ForSql($workflowId)."' ";
		$arBinds = array("STATE_PARAMETERS" => $stateParameters);
	...
	public static function __InsertStateHack($id, $moduleId, $entity, $documentId, $templateId, $state, $stateTitle, $stateParameters, $arStatePermissions)
	{
		...
		$strSql = "INSERT INTO b_bp_workflow_state (ID, MODULE_ID, ENTITY, DOCUMENT_ID, DOCUMENT_ID_INT, WORKFLOW_TEMPLATE_ID, MODIFIED, STATE, STATE_TITLE, STATE_PARAMETERS) ".
			"VALUES ('".$DB->ForSql($id)."', '".$DB->ForSql($moduleId)."', '".$DB->ForSql($entity)."', '".$DB->ForSql($documentId)."', ".intval($documentId).", ".intval($templateId).", ".$DB->CurrentTimeFunction().", '".$DB->ForSql($state)."', '".$DB->ForSql($stateTitle)."', EMPTY_CLOB())";
		$arBinds = array("STATE_PARAMETERS" => $stateParameters);
		$DB->QueryBind($strSql, $arBinds);
