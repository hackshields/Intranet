	...
	extends CBPAllTrackingService
{
	public function Write($workflowId, $type, $actionName, $executionStatus, $executionResult, $actionTitle = "", $actionNote = "", $modifiedBy = 0)
	{
		...
		$strSql = "INSERT INTO b_bp_tracking(WORKFLOW_ID, TYPE, MODIFIED, ACTION_NAME, ACTION_TITLE, EXECUTION_STATUS, EXECUTION_RESULT, ACTION_NOTE, MODIFIED_BY) ".
			"VALUES('".$DB->ForSql($workflowId, 32)."', ".intval($type).", ".$DB->CurrentTimeFunction().", '".$DB->ForSql($actionName, 128)."', '".$DB->ForSql($actionTitle, 255)."', ".intval($executionStatus).", ".intval($executionResult).", EMPTY_CLOB(), ".($modifiedBy > 0 ? $modifiedBy : "NULL").")";
		$arBinds = array("ACTION_NOTE" => $actionNote);
	...
	function ClearOld($days = 0)
	{
		...
		$strSql = "DELETE b_bp_tracking ".
			"WHERE WORKFLOW_ID NOT IN (SELECT ID FROM b_bp_workflow_instance) ".
			"   AND MODIFIED < trunc(sysdate) - ".$days." ".
			"   AND TYPE <> 6";
