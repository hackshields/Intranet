	...
	extends CAllBPWorkflowTemplateLoader
{
	...
	*/
	...
	public function AddTemplate($arFields)
	{
		...
		{
			$data = unpack("H*hex", $arFields["TEMPLATE"]);
			$arFields["TEMPLATE"] = "0x".$data["hex"];
		}
		if (is_set($arFields, "PARAMETERS"))
		{
			$data = unpack("H*hex", $arFields["PARAMETERS"]);
			$arFields["PARAMETERS"] = "0x".$data["hex"];
		}
		if (is_set($arFields, "VARIABLES"))
		{
			$data = unpack("H*hex", $arFields["VARIABLES"]);
			$arFields["VARIABLES"] = "0x".$data["hex"];
		}
