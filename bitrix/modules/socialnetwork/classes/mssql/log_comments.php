...
class CSocNetLogComments extends CAllSocNetLogComments
{
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arParams = array())
	{
		...
		static $arFields1 = array(
			...
			"MESSAGE" => Array("FIELD" => "CAST(LC.MESSAGE AS VARCHAR(4000))", "TYPE" => "string"),
			"TEXT_MESSAGE" => Array("FIELD" => "CAST(LC.TEXT_MESSAGE AS VARCHAR(4000))", "TYPE" => "string"),
...
//				$strSql = preg_replace("#^([\s]*select[\s]+)(.+)$#is", "SELECT TOP ".IntVal($arNavStartParams['nTopCount'])." \\2", $strSql);
	...
	{
		return $GLOBALS["DB"]->Query("DELETE SLC FROM b_sonet_log_comment AS SLC WHERE EXISTS (SELECT BC.ID FROM b_blog_comment BC WHERE BC.ID = SLC.SOURCE_ID AND BC.BLOG_ID = ".intval($blog_id).") AND (SLC.EVENT_ID = 'blog_comment_micro' OR SLC.EVENT_ID = 'blog_comment')", true);
	}
}
?>