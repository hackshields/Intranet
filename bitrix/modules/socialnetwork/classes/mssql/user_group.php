...
class CSocNetUserToGroup extends CAllSocNetUserToGroup
{
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		static $arFields1 = array(
			...
			"MESSAGE" => Array("FIELD" => "CAST(UG.MESSAGE AS VARCHAR(4000))", "TYPE" => "string"),
			"GROUP_NAME" => Array("FIELD" => "G.NAME", "TYPE" => "string", "FROM" => "INNER JOIN b_sonet_group G ON (UG.GROUP_ID = G.ID)"),
			"GROUP_DESCRIPTION" => Array("FIELD" => "CAST(G.DESCRIPTION AS VARCHAR(4000))", "TYPE" => "string", "FROM" => "INNER JOIN b_sonet_group G ON (UG.GROUP_ID = G.ID)"),
		...
		$arFields["USER_IS_ONLINE"] = Array("FIELD" => "case when U.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE()) then 'Y' else 'N' end", "TYPE" => "string", "FROM" => "INNER JOIN b_user U ON (UG.USER_ID = U.ID)");
