...
class CSocNetLog extends CAllSocNetLog
{
	...
	function ClearOld($days = 90)
	{
		...
		$DB->Query("DELETE LC FROM b_sonet_log_comment AS LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L LEFT JOIN b_sonet_log_favorites LF ON L.ID = LF.LOG_ID WHERE LF.USER_ID IS NULL AND L.LOG_UPDATE < DATEADD(day, -".$days.", GETDATE()) AND LC.LOG_ID = L.TMP_ID)", true);
		$DB->Query("DELETE LS FROM b_sonet_log_site AS LS WHERE EXISTS (SELECT L.ID FROM b_sonet_log L LEFT JOIN b_sonet_log_favorites LF ON L.ID = LF.LOG_ID WHERE LF.USER_ID IS NULL AND L.LOG_UPDATE < DATEADD(day, -".$days.", GETDATE()) AND LS.LOG_ID = L.ID)", true);
		$DB->Query("DELETE LR FROM b_sonet_log_right AS LR WHERE EXISTS (SELECT L.ID FROM b_sonet_log L LEFT JOIN b_sonet_log_favorites LF ON L.ID = LF.LOG_ID WHERE LF.USER_ID IS NULL AND L.LOG_UPDATE < DATEADD(day, -".$days.", GETDATE()) AND LR.LOG_ID = L.ID)", true);

		return $DB->Query("DELETE FROM b_sonet_log WHERE LOG_UPDATE < DATEADD(day, -".$days.", GETDATE())", true);
	}
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arParams = array())
	{
		...
		static $arFields1 = array(
			...
			"MESSAGE" => Array("FIELD" => "CAST(L.MESSAGE AS VARCHAR(4000))", "TYPE" => "string"),
			"TEXT_MESSAGE" => Array("FIELD" => "CAST(L.TEXT_MESSAGE AS VARCHAR(4000))", "TYPE" => "string"),
			...
			"PARAMS" => Array("FIELD" => "CAST(L.PARAMS AS VARCHAR(4000))", "TYPE" => "string"),
		...
		)
		{
			...
			$arFields["DATE_FOLLOW"] = Array("FIELD" => "CASE 
				...
				WHEN LFW.TYPE = 'Y' 
					THEN L.LOG_UPDATE 
				ELSE L.LOG_DATE 
				END", "TYPE" => "datetime", "FROM" => "LEFT JOIN b_sonet_log_follow LFW ON LFW.USER_ID = ".$USER->GetID()." AND LFW.CODE = ".$DB->Concat("'L'", "CAST(L.ID as varchar(17))"));

			$arFields["FOLLOW"] = Array("FIELD" => "case when LFW.USER_ID IS NULL then '".$default_follow."' else LFW.TYPE end", "TYPE" => "string", "FROM" => "LEFT JOIN b_sonet_log_follow LFW ON LFW.USER_ID = ".$USER->GetID()." AND LFW.CODE = ".$DB->Concat("'L'", "CAST(L.ID as varchar(17))"));
		...
		)
		{
			...
			$arSqls["RIGHTS"] = "EXISTS ( 
				SELECT SLR.ID FROM b_sonet_log_right SLR
...
//				$strSql = preg_replace("#^([\s]*select[\s]+)(.+)$#is", "SELECT TOP ".IntVal($arNavStartParams['nTopCount'])." \\2", $strSql);
				if(preg_match("#^\s*SELECT(\s+DISTINCT|)\s+#is", $strSql, $match) && !preg_match("#^\s*SELECT(\s+DISTINCT|)\s+TOP#is", $strSql))
					$strSql = $match[0]."TOP ".IntVal($arNavStartParams["nTopCount"])." ".substr($strSql, strlen($match[0]));
			}
	...
	function DeleteSystemEventsByGroupID($group_id = false)
	{
		...
		$DB->Query("DELETE LC FROM b_sonet_log_comment AS LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND CAST(L.MESSAGE AS VARCHAR(4000)) = '".$group_id."' AND LC.LOG_ID = L.TMP_ID)", true);
		$DB->Query("DELETE LS FROM b_sonet_log_site AS LS WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND CAST(L.MESSAGE AS VARCHAR(4000)) = '".$group_id."' AND LS.LOG_ID = L.ID)", true);
		$DB->Query("DELETE LR FROM b_sonet_log_right AS LR WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND CAST(L.MESSAGE AS VARCHAR(4000)) = '".$group_id."' AND LR.LOG_ID = L.ID)", true);
		$DB->Query("DELETE LF FROM b_sonet_log_favorites AS LF WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND CAST(L.MESSAGE AS VARCHAR(4000)) = '".$group_id."' AND LF.LOG_ID = L.ID)", true);

		return $DB->Query("DELETE FROM b_sonet_log WHERE ENTITY_TYPE = '".SONET_ENTITY_USER."' AND EVENT_ID = 'system_groups' AND CAST(MESSAGE AS VARCHAR(4000)) = '".$group_id."'", true);
	}
	...
	function Delete($ID)
	{
		...
		$DB->Query("DELETE LC FROM b_sonet_log_comment AS LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L WHERE L.ID = ".$ID." AND LC.LOG_ID = L.TMP_ID)", true);
	...
	function DeleteNoDemand($userID)
	{
		...
		$DB->Query("DELETE LC FROM b_sonet_log_comment AS LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LC.LOG_ID = L.TMP_ID)", true);
		$DB->Query("DELETE LS FROM b_sonet_log_site AS LS WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LS.LOG_ID = L.ID)", true);
		$DB->Query("DELETE LR FROM b_sonet_log_right AS LR WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LR.LOG_ID = L.ID)", true);
		$DB->Query("DELETE LF FROM b_sonet_log_favorites AS LF WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LF.LOG_ID = L.ID)", true);
	...
	{
		return $GLOBALS["DB"]->Query("DELETE SL FROM b_sonet_log AS SL WHERE EXISTS (SELECT BP.ID FROM b_blog_post BP WHERE BP.ID = SL.SOURCE_ID AND BP.BLOG_ID = ".intval($blog_id).") AND (SL.EVENT_ID = 'blog_post_micro' OR SL.EVENT_ID = 'blog_post')", true);
	}
}
?>