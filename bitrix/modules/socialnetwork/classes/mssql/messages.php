...
class CSocNetMessages extends CAllSocNetMessages
{
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$arFields["FROM_USER_IS_ONLINE"] = Array("FIELD" => "case when U.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE()) then 'Y' else 'N' end", "TYPE" => "string", "FROM" => "INNER JOIN b_user U ON (M.FROM_USER_ID = U.ID)");
		$arFields["TO_USER_IS_ONLINE"] = Array("FIELD" => "case when U1.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE()) then 'Y' else 'N' end", "TYPE" => "string", "FROM" => "INNER JOIN b_user U1 ON (M.TO_USER_ID = U1.ID)");
	...
	function GetChatLastDate($currentUserID, $userID)
	{
		...
		$strSql =
			"SELECT CONVERT(varchar(19), MAX(DATE_CREATE), 120) as DDD ".
	...
	function GetMessagesForChat($currentUserID, $userID, $date = false, $arNavStartParams = false, $replyMessId=false)
	{
		...
		if($diff !== false && $diff <> 0)
			$sDateFmt = "CONVERT(varchar(19), DATEADD(second, ".$diff.", DATE_CREATE), 120) as DATE_CREATE_FMT, ";
		else
			$sDateFmt = "CONVERT(varchar(19), DATE_CREATE, 120) as DATE_CREATE_FMT, ";
		...
		$strSql =
			...
			(($date !== false && $replyMessId <=0) ? " AND MESSAGE_TYPE = 'P' AND DATE_CREATE > convert(datetime, '".$DB->ForSql($date).".999', 120) " : "").
	...
	function GetMessagesUsers($userID, $arNavStartParams = false, $online_interval = 120)
	{
		...
		$strSql =
			...
			"	case when MAX(U.LAST_ACTIVITY_DATE) > DATEADD(second, -".intval($online_interval).", GETDATE()) then 'Y' else 'N' end as IS_ONLINE, ".
	...
	function Now()
	{
		...
		$strSql = "SELECT CONVERT(varchar(19), GETDATE(), 120) as T ";
		$dbRes = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		if ($arRes = $dbRes->Fetch())
			return $arRes["T"];
		else
