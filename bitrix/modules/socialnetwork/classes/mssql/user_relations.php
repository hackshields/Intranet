...
class CSocNetUserRelations extends CAllSocNetUserRelations
{
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$arFields["FIRST_USER_IS_ONLINE"] = Array("FIELD" => "case when U.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE()) then 'Y' else 'N' end", "TYPE" => "string", "FROM" => "INNER JOIN b_user U ON (UR.FIRST_USER_ID = U.ID)");
		$arFields["SECOND_USER_IS_ONLINE"] = Array("FIELD" => "case when U1.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE()) then 'Y' else 'N' end", "TYPE" => "string", "FROM" => "INNER JOIN b_user U1 ON (UR.SECOND_USER_ID = U1.ID)");
	...
	function GetListBirthday($userID, $number = 5, $online_interval = 120)
	{
		...
		$strSql =
			"SELECT ".($number > 0 ? "TOP ".$number."" : "")." U.ID, U.NAME, U.LAST_NAME, U.SECOND_NAME, U.LOGIN, U.EMAIL, U.PERSONAL_PHOTO, U.PERSONAL_GENDER, U.PERSONAL_BIRTHDAY as PB, ".
			"	case when U.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE()) then 'Y' else 'N' end as IS_ONLINE, ".
			"	case
	...
	function GetRelationsTop($userID, $number = 100)
	{
		...
		$strSql =
			"SELECT ".($number > 0 ? "TOP ".$number."" : "")." UR.RELATION, UR.FIRST_USER_ID, UR.SECOND_USER_ID ".
			"FROM b_sonet_user_relations UR ".
			"WHERE UR.FIRST_USER_ID = ".$userID." ".
			"UNION ".
