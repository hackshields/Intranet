...
class CSocNetMessages extends CAllSocNetMessages
{
	...
	function Add($arFields)
	{
		...
		if (strlen($arInsert[0]) > 0)
		{
			$ID = IntVal($DB->NextID("SQ_B_SONET_MESSAGES"));

			$strSql =
				"INSERT INTO b_sonet_messages(ID, ".$arInsert[0].") ".
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$arFields["FROM_USER_IS_ONLINE"] = Array("FIELD" => "CASE WHEN U.LAST_ACTIVITY_DATE > (sysdate - ".$online_interval."/86400) THEN 'Y' ELSE 'N' END", "TYPE" => "string", "FROM" => "INNER JOIN b_user U ON (M.FROM_USER_ID = U.ID)");
		$arFields["TO_USER_IS_ONLINE"] = Array("FIELD" => "CASE WHEN U1.LAST_ACTIVITY_DATE > (sysdate - ".$online_interval."/86400)	THEN 'Y' ELSE 'N' END", "TYPE" => "string", "FROM" => "INNER JOIN b_user U1 ON (M.TO_USER_ID = U1.ID)");
	...
	function GetChatLastDate($currentUserID, $userID)
	{
		...
		$strSql =
			"SELECT TO_CHAR(MAX(DATE_CREATE), 'YYYY-MM-DD')||' 00:00:00' as DDD ".
	...
	function GetMessagesForChat($currentUserID, $userID, $date = false, $arNavStartParams = false, $replyMessId=false)
	{
		...
		if($diff !== false && $diff <> 0)
			$sDateFmt = "TO_CHAR(DATE_CREATE+(".$diff."/86400), 'YYYY-MM-DD HH24:MI:SS') as DATE_CREATE_FMT, ";
		else
			$sDateFmt = "TO_CHAR(DATE_CREATE, 'YYYY-MM-DD HH24:MI:SS') as DATE_CREATE_FMT, ";
		...
		$strSql =
			...
			(($date !== false && $replyMessId <=0) ? " AND MESSAGE_TYPE = 'P' AND DATE_CREATE > TO_DATE('".$DB->ForSql($date)."', 'YYYY-MM-DD HH24:MI:SS') " : "").
	...
	function GetMessagesUsers($userID, $arNavStartParams = false, $online_interval = 120)
	{
		...
		$strSql =
			...
			"	CASE WHEN MAX(U.LAST_ACTIVITY_DATE) > (sysdate - ".intval($online_interval)."/86400) THEN 'Y' ELSE 'N' END as IS_ONLINE, ".
			...
			"	AND M.FROM_DELETED = 'N' ".
			"	) ".
			"GROUP BY U.ID, U.ACTIVE, U.LOGIN, U.NAME, U.LAST_NAME, U.SECOND_NAME, U.PERSONAL_PHOTO, U.PERSONAL_GENDER ".
	...
	function Now()
	{
		...
		$strSql = "SELECT TO_CHAR(sysdate, 'YYYY-MM-DD HH24:MI:SS') as T  FROM dual";
		$dbRes = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		if ($arRes = $dbRes->Fetch())
			return $arRes["T"];
		else
