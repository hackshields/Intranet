...
class CSocNetGroup extends CAllSocNetGroup
{
	...
	function Add($arFields)
	{
		...
		if (strlen($arInsert[0]) > 0)
		{
			$ID = IntVal($DB->NextID("SQ_B_SONET_GROUP"));

			$strSql =
				"INSERT INTO b_sonet_group(ID, ".$arInsert[0].") ".
	...
	function Update($ID, $arFields, $bAutoSubscribe = true)
	{
		...
		if (strlen($strUpdate) > 0)
		{
			...
			if(count($arSiteID)>0)
			{
				$strSqlTmp = "DELETE FROM b_sonet_group_site WHERE GROUP_ID=".$ID;
				$DB->Query($strSqlTmp, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		static $arFields1 = array(
			...
			"DESCRIPTION" => Array("FIELD" => "TO_CHAR(G.DESCRIPTION)", "TYPE" => "string"),
