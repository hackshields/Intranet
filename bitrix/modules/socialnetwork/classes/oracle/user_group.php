...
class CSocNetUserToGroup extends CAllSocNetUserToGroup
{
	...
	function Add($arFields)
	{
		...
		else
		{
			...
			if (strlen($arInsert[0]) > 0)
			{
				$ID = IntVal($DB->NextID("SQ_B_SONET_USER2GROUP"));

				$strSql =
					"INSERT INTO b_sonet_user2group(ID, ".$arInsert[0].") ".
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		static $arFields1 = array(
			...
			"MESSAGE" => Array("FIELD" => "TO_CHAR(UG.MESSAGE)", "TYPE" => "string"),
			"GROUP_NAME" => Array("FIELD" => "G.NAME", "TYPE" => "string", "FROM" => "INNER JOIN b_sonet_group G ON (UG.GROUP_ID = G.ID)"),
			"GROUP_DESCRIPTION" => Array("FIELD" => "TO_CHAR(G.DESCRIPTION)", "TYPE" => "string", "FROM" => "INNER JOIN b_sonet_group G ON (UG.GROUP_ID = G.ID)"),
		...
		$arFields["USER_IS_ONLINE"] = Array("FIELD" => "CASE WHEN U.LAST_ACTIVITY_DATE > (sysdate - ".$online_interval."/86400)	THEN 'Y' ELSE 'N' END", "TYPE" => "string", "FROM" => "INNER JOIN b_user U ON (UG.USER_ID = U.ID)");
