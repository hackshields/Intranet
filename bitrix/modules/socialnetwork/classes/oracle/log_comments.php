...
class CSocNetLogComments extends CAllSocNetLogComments
{
	...
	function Add($arFields, $bSetSource = false, $bSendEvent = true, $bSetLogUpDate = true)
	{
		...
		if (!$bSetSource || (is_array($arSource) && array_key_exists("SOURCE_ID", $arFields) && intval($arFields["SOURCE_ID"]) > 0))
		{
			...
			if (strlen($arInsert[0]) > 0)
			{
				$ID = IntVal($DB->NextID("SQ_B_SONET_LOG_COMMENT"));

				$strSql =
					"INSERT INTO b_sonet_log_comment(ID, ".$arInsert[0].") ".
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arParams = array())
	{
		...
		static $arFields1 = array(
			...
			"MESSAGE" => Array("FIELD" => "TO_CHAR(SUBSTR(LC.MESSAGE, 1, 2000))", "TYPE" => "string"),
			"TEXT_MESSAGE" => Array("FIELD" => "TO_CHAR(SUBSTR(LC.TEXT_MESSAGE, 1, 2000))", "TYPE" => "string"),
			...
			"LOG_PARAMS" => Array("FIELD" => "TO_CHAR(SUBSTR(L1.PARAMS, 1, 2000))", "TYPE" => "string", "FROM" => "INNER JOIN b_sonet_log L1 ON (LC.LOG_ID = L1.ID)"),
	...
	{
		return $GLOBALS["DB"]->Query("DELETE FROM b_sonet_log_comment SLC WHERE EXISTS (SELECT BC.ID FROM b_blog_comment BC WHERE BC.ID = SLC.SOURCE_ID AND BC.BLOG_ID = ".intval($blog_id).") AND (SLC.EVENT_ID = 'blog_comment_micro' OR SLC.EVENT_ID = 'blog_comment')", true);
	}
}
?>