...
class CSocNetLog extends CAllSocNetLog
{
	...
	function Add($arFields, $bSendEvent = true)
	{
		...
		if (strlen($arInsert[0]) > 0)
		{
			$ID = IntVal($DB->NextID("SQ_B_SONET_LOG"));

			$strSql =
				"INSERT INTO b_sonet_log(ID, ".$arInsert[0].") ".
				"VALUES(".$ID.", ".$arInsert[1].")";

			$arBinds = Array();
			if (is_set($arFields, "MESSAGE"))
				$arBinds["MESSAGE"] = $arFields["MESSAGE"];
			if (is_set($arFields, "TEXT_MESSAGE"))
				$arBinds["TEXT_MESSAGE"] = $arFields["TEXT_MESSAGE"];
			if (is_set($arFields, "PARAMS"))
				$arBinds["PARAMS"] = $arFields["PARAMS"];
	...
	function ClearOld($days = 90)
	{
		...
		$DB->Query("DELETE FROM b_sonet_log_comment LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L LEFT JOIN b_sonet_log_favorites LF ON L.ID = LF.LOG_ID WHERE LF.USER_ID IS NULL AND L.LOG_UPDATE < DATEADD(day, -".$days.", GETDATE()) AND LC.LOG_ID = L.TMP_ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_site LS WHERE EXISTS (SELECT L.ID FROM b_sonet_log L LEFT JOIN b_sonet_log_favorites LF ON L.ID = LF.LOG_ID WHERE LF.USER_ID IS NULL AND L.LOG_UPDATE < DATEADD(day, -".$days.", GETDATE()) AND LS.LOG_ID = L.ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_right LR WHERE EXISTS (SELECT L.ID FROM b_sonet_log L LEFT JOIN b_sonet_log_favorites LF ON L.ID = LF.LOG_ID WHERE LF.USER_ID IS NULL AND L.LOG_UPDATE < DATEADD(day, -".$days.", GETDATE()) AND LR.LOG_ID = L.ID)", true);

		return $DB->Query("DELETE FROM b_sonet_log WHERE LOG_UPDATE < trunc(sysdate) - ".$days."", true);
	}
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arParams = array())
	{
		...
		static $arFields1 = array(
			...
			"MESSAGE" => Array("FIELD" => "TO_CHAR(SUBSTR(L.MESSAGE, 1, 2000))", "TYPE" => "string"),
			"TEXT_MESSAGE" => Array("FIELD" => "TO_CHAR(SUBSTR(L.TEXT_MESSAGE, 1, 2000))", "TYPE" => "string"),
			...
			"PARAMS" => Array("FIELD" => "TO_CHAR(SUBSTR(L.PARAMS, 1, 2000))", "TYPE" => "string"),
	...
	function DeleteSystemEventsByGroupID($group_id = false)
	{
		...
		$DB->Query("DELETE FROM b_sonet_log_comment LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND TO_CHAR(SUBSTR(L.MESSAGE, 1, 2000)) = '".$group_id."' AND LC.LOG_ID = L.TMP_ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_site LS WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND TO_CHAR(SUBSTR(L.MESSAGE, 1, 2000)) = '".$group_id."' AND LS.LOG_ID = L.ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_right LR WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND TO_CHAR(SUBSTR(L.MESSAGE, 1, 2000)) = '".$group_id."' AND LR.LOG_ID = L.ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_favorites LF WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.EVENT_ID = 'system_groups' AND TO_CHAR(SUBSTR(L.MESSAGE, 1, 2000)) = '".$group_id."' AND LF.LOG_ID = L.ID)", true);

		return $DB->Query("DELETE FROM b_sonet_log L WHERE ENTITY_TYPE = '".SONET_ENTITY_USER."' AND EVENT_ID = 'system_groups' AND TO_CHAR(SUBSTR(L.MESSAGE, 1, 2000)) = '".$group_id."'", true);
	}
	...
	function Delete($ID)
	{
		...
		$DB->Query("DELETE FROM b_sonet_log_comment LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L WHERE L.ID = ".$ID." AND LC.LOG_ID = L.TMP_ID)", true);
	...
	function DeleteNoDemand($userID)
	{
		...
		$DB->Query("DELETE FROM b_sonet_log_comment LC WHERE EXISTS (SELECT L.TMP_ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LC.LOG_ID = L.TMP_ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_site LS WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LS.LOG_ID = L.ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_right LR WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LR.LOG_ID = L.ID)", true);
		$DB->Query("DELETE FROM b_sonet_log_favorites LF WHERE EXISTS (SELECT L.ID FROM b_sonet_log L WHERE L.ENTITY_TYPE = '".SONET_ENTITY_USER."' AND L.ENTITY_ID = ".$userID." AND LF.LOG_ID = L.ID)", true);
	...
	{
		return $GLOBALS["DB"]->Query("DELETE FROM b_sonet_log SL WHERE EXISTS (SELECT BP.ID FROM b_blog_post BP WHERE BP.ID = SL.SOURCE_ID AND BP.BLOG_ID = ".intval($blog_id).") AND (SL.EVENT_ID = 'blog_post_micro' OR SL.EVENT_ID = 'blog_post')", true);
	}
}
?>