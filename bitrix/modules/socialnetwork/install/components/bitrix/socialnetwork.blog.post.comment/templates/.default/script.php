...
function waitResult(id, data, idA)
{
	...
	else
	{
		...
		else
		{
			...
			if (
				parseInt(BX('postId').value) > 0
				&& BX('comcntleave-' + BX('postId').value)
			)
			{
				var old_value = (BX('comcntleave-' + BX('postId').value).innerHTML.length > 0 ? parseInt(BX('comcntleave-' + BX('postId').value).innerHTML) : 0);
				var val = old_value + 1;						
				BX('comcntleave-' + BX('postId').value).innerHTML = val;
			}
