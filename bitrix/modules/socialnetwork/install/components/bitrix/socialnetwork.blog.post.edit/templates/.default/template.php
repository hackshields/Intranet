...
else
{
	if(strlen($arResult["FATAL_MESSAGE"]) <= 0)
	{
		...
		{
			$bVote = true;
			if (!!$arResult["POST_PROPERTIES"]["DATA"]["UF_BLOG_POST_VOTE"]["VALUE"])
				$activeTab = "vote";
		}
		if (
		...
		{
			$bGrat = true;
			if (!empty($arResult["PostToShow"]["GRAT_CURRENT"]["ID"]) || !empty($arResult["PostToShow"]["GRAT_CURRENT"]["USERS"]))
				$activeTab = "grat";
		}
		if (array_key_exists("UF_BLOG_POST_IMPRTNT", $arResult["POST_PROPERTIES"]["DATA"]))
		{
			$bVote = true;
			if (!!$arResult["POST_PROPERTIES"]["DATA"]["UF_BLOG_POST_IMPRTNT"]["VALUE"])
				$activeTab = "important";
		}
		...
		if ($bGrat)
		{
			?><span class="feed-add-post-form-link feed-add-post-form-link-more <?=($activeTab == "important" ?
				"feed-add-post-form-ad-link" : ($activeTab == "grat" ? "feed-add-post-form-grat-link" : ""))?>" <?
				?>id="feed-add-post-form-tab-more" <?
				?>onclick="gratitude(this, <?=($arParams["TOP_TABS_VISIBLE"] == "Y" ? 'true' : 'false')?>);"><?
				?><span class="feed-add-post-form-link-text"><?=($activeTab == "important" ? GetMessage("SBPE_IMPORTANT_MESSAGE") : ($activeTab == "grat" ?
		...
		if(strlen($arResult["UTIL_MESSAGE"])>0)
		{
			?>
			<div class="feed-add-successfully">
		...
		else
		{
			...
			<form action="<?=POST_FORM_ACTION_URI?>" id="blogPostForm" name="blogPostForm" method="POST" enctype="multipart/form-data" target="_self" <?
				...
				<?=bitrix_sessid_post();?>
				<input type="hidden" name="<?=$arResult["POST_PROPERTIES"]["DATA"]["UF_BLOG_POST_IMPRTNT"]["FIELD_NAME"]?>" value="<?=htmlspecialcharsbx($arResult["POST_PROPERTIES"]["DATA"]["UF_BLOG_POST_IMPRTNT"]["VALUE"])?>" />
						...
						$APPLICATION->IncludeComponent(
							...
							$formParams = Array(
								...
								"LHE" => array(
										...
										"fontSize" => "14px",
										"bInitByJS" => (empty($arResult["ERROR_MESSAGE"]) && $arParams["ID"] <= 0)
									)
								),
								false,
								Array("HIDE_ICONS" => "Y")
				...
				function showPanelTitle_<?=$arParams["FORM_ID"]?>(show)
				{
					...
					BX.userOptions.save('socialnetwork', 'postEdit', 'showTitle', val);
				}
				...
				BX.addCustomEvent(
					...
					function() {
						...
						if (!!div[0]) {
							BX.adjust(div[0], {style:{display:"block",height:"1px", opacity:0.1}});
							BX.adjust(div[0], {style:{display:"block",height:"auto", opacity:1}}); }
						BX.adjust(div[1], {style:{display:"block",height:"1px", opacity:0.1}});
						BX.adjust(div[1], {style:{display:"block",height:"auto", opacity:1}});
				...
				BX.addCustomEvent(
					...
					function() {
						...
						if (!!div[0]) {
							BX.adjust(div[0], {style:{display:"block",height:"auto", opacity:1}});
							BX.adjust(div[0], {style:{display:"block",height:"0px", opacity:0}});}
						BX.adjust(div[1], {style:{display:"block",height:"auto", opacity:1}});
						BX.adjust(div[1], {style:{display:"block",height:"0px", opacity:0}});
						
						if(BX('blog-title').style.display != "none")
						{
							showPanelTitle_<?=$arParams["FORM_ID"]?>(false);
							bShowTitle = true;
						}
						else
						{
				...
				<?if (!(empty($arResult["ERROR_MESSAGE"]) && $arParams["ID"] <= 0)) {?>
					changePostFormTab('<?=($activeTab == "important" ? "more" : $activeTab)?>');
				<?}?>
				...
				{
					showLHEEditor('<?=$formParams["LHE"]["jsObjName"]?>', '<?=$formParams["LHE"]["id"]?>');
				}
				</script>
				<?if(false && COption::GetOptionString("blog", "use_autosave", "Y") == "Y")
				{
					$as = new CAutoSave();
					$as->Init(false);
					?>
					<script>
					BX.message({'BLOG_POST_AUTOSAVE':'<?=GetMessageJS("BLOG_POST_AUTOSAVE")?>', 'BLOG_TITLE' : '<?=GetMessageJS("BLOG_TITLE")?>'});
