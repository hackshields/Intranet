...
function gratitude(btn, params) {
	var item = BX.findChild(btn, {className:'feed-add-post-form-link-text'}, true);
	changePostFormTab((!!item.getAttribute('activePoint') ? item.getAttribute('activePoint') : 'more'), false);
	BX.PopupMenu.show('feed-add-post-form-popup', btn,
		[
			{ text : BX.message('BLOG_TAB_GRAT'),
				className : "feed-add-post-form-grat",
				href : "#",
				onclick:function(event){
					changePostFormTab('grat', false);
					BX.PreventDefault(event);
					item = BX.findChild(btn, {className:'feed-add-post-form-link-text'}, true);
					item.innerHTML = BX.message('BLOG_TAB_GRAT');
					item.setAttribute('activePoint', 'grat');
					BX.addClass(btn, "feed-add-post-form-link-active feed-add-post-form-grat-link")
					BX.removeClass(btn, 'feed-add-post-form-ad-link');
					this.popupWindow.close();
				}
			},
			{ text : BX.message('SBPE_IMPORTANT_MESSAGE'),
				className : "feed-add-post-form-ad", href : "#",
				onclick:function(event){
					changePostFormTab('more', true);
					BX.PreventDefault(event);
					...
					BX.addClass(btn, "feed-add-post-form-link-active feed-add-post-form-ad-link");
					this.popupWindow.close();
				}
			}
		],
		{
			...
			events : {
				onPopupClose : function() {
					item = BX.findChild(btn, {className:'feed-add-post-form-link-text'}, true);
					if (!item.getAttribute('activePoint')) {changePostFormTab('message', false);} }
			}
		}
	)
}
function important()
{
	if (BX('blogPostForm') && BX('blogPostForm').UF_BLOG_POST_IMPRTNT){
		...
		BX.addCustomEvent(window, "changePostFormTab", function(type){if (type!="more"){BX('blogPostForm').UF_BLOG_POST_IMPRTNT.value = 0;}});
	}
}
/*
...
BlogPostAutoSave = function (pEditorAutoSave) {
	...
	BX.addCustomEvent(form, 'onAutoSaveRestore', function(ob, data) {
		...
		if(data[titleID].length > 0 && data[titleID] != BX.message('BLOG_TITLE'))
		{
