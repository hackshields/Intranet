		...
		if(sonetLXmlHttpSet3.readyState == 4)
		{
			...
			if(sonetLXmlHttpSet3.status == 200)
			{
				...
				if (typeof(data) == "object")
				{
					...
					if (BX("feed-comments-all-cnt-" + log_id, true))
					{
						var old_value = (BX('feed-comments-all-cnt-' + log_id).innerHTML.length > 0 ? parseInt(BX('feed-comments-all-cnt-' + log_id).innerHTML) : 0);
						var val = old_value + 1;						
						BX('feed-comments-all-cnt-' + log_id).innerHTML = val;
					}
				}
			}
			else
