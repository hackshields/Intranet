	...
	function GetListEx($arOrder = Array("ID"=>"ASC"), $arFilter = Array(), $bCount = false, $iNum = 0, $arAddParams = array())
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "IS_READ":
					if (strlen($val)<=0)
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(PM.".$key." IS NULL OR LEN(PM.".$key.")<=0)";
					elseif (strToUpper($strOperation) == "IN")
		...
		$strSql = 
			"SELECT /*TOP*/ 
				...
				CASE
					WHEN ((FU.SHOW_NAME='Y') AND LEN(LTRIM(RTRIM(".CForumUser::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).")))>0)
					THEN LTRIM(RTRIM(REPLACE(".CForumUser::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).", '  ', ' ')))
					ELSE U.LOGIN
				END AS AUTHOR_NAME,
				PM.RECIPIENT_ID, UU.EMAIL AS RECIPIENT_EMAIL, UU.LOGIN AS RECIPIENT_LOGIN,
				CASE
					WHEN ((FUU.SHOW_NAME='Y') AND LEN(LTRIM(RTRIM(".CForumUser::GetNameFieldsForQuery($arAddParams["sNameTemplate"], "UU.").")))>0)
					THEN LTRIM(RTRIM(REPLACE(".CForumUser::GetNameFieldsForQuery($arAddParams["sNameTemplate"], "UU.").", '  ', ' ')))
					ELSE UU.LOGIN
