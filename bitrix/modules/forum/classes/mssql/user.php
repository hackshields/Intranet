	...
	function GetList($arOrder = Array("ID"=>"ASC"), $arFilter = Array(), $arAddParams = array())
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "ALLOW_POST":
					if (strLen($val)<=0)
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(FU.".$key." IS NULL OR LEN(FU.".$key.")<=0)";
					else
				...
				case "PERSONAL_BIRTHDAY_DATE":
					$arSqlSearch[] = ($strNegative=="Y"?" U.PERSONAL_BIRTHDAY IS NULL OR NOT ":"")." ((REPLICATE('0',2-LEN(DATEPART(mm, U.PERSONAL_BIRTHDAY)))+CONVERT(varchar(2),DATEPART(mm, U.PERSONAL_BIRTHDAY))+'-'+REPLICATE('0',2-LEN(DATEPART(dd, U.PERSONAL_BIRTHDAY)))+CONVERT(varchar(2),DATEPART(dd, U.PERSONAL_BIRTHDAY))) ".$strOperation." '".$DB->ForSql($val)."')";
					break;
				case "LAST_VISIT":
					if(strLen($val)<=0)
						...
						$arSqlSearch[] =
							"(
								(
									FU.SHOW_NAME = 'Y'
									AND
									LEN(LTRIM(RTRIM(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"])."))) > 0
									AND
									(REPLACE((".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"])."), '  ', ' ') LIKE '%".$DB->ForSql($val)."%')
								)
								OR
								(
									(
		...
		$strSql =
			"SELECT /*TOP*/ FU.ID, U.ID as USER_ID, FU.SHOW_NAME, FU.DESCRIPTION, FU.IP_ADDRESS,
		...
		if (is_array($arAddParams) && (intVal($arAddParams["nTopCount"])>0))
			$strSql = str_replace("/*TOP*/", "TOP ".intVal($arAddParams["nTopCount"]), $strSql);
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case"SUBSC_NEW_TOPIC_ONLY":
					...
					if (strLen($val)<=0)
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(FS.".$key." IS NULL OR LEN(FS.".$key.")<=0)";
					else
	...
	function SearchUser($template, $arAddParams = array())
	{
		...
		$strSql =
			"SELECT U.ID, U.NAME, U.SECOND_NAME, U.LAST_NAME, U.LOGIN, F.SHOW_NAME,
				CASE
					WHEN (F.SHOW_NAME = 'Y' AND LEN(LTRIM(RTRIM(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"])."))) > 0)
					THEN LTRIM(RTRIM(REPLACE(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).", '  ', ' ')))
					ELSE U.LOGIN
				END AS SHOW_ABC
			FROM b_user U
		...
		if (substr($template, 0, 1) == '%')
			...
			"(
				(
					...
					LEN(LTRIM(RTRIM(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"])."))) > 0
					AND
					REPLACE((".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"])."), '  ', ' ') LIKE '".$template."'
				)
				OR
				(
					(
			...
			$strSql .=
			"(
				F.SHOW_NAME = 'Y' AND LEN(RTRIM(LTRIM(U.NAME))) > 0 AND U.NAME LIKE '".$template."'
			)
			OR
			(
				F.SHOW_NAME = 'Y' AND LEN(RTRIM(LTRIM(U.NAME))) <= 0
				AND
				LEN(RTRIM(LTRIM(U.LAST_NAME))) > 0 AND U.LAST_NAME LIKE '".$template."'
			)
			OR
			(
				(
					...
					(
						F.SHOW_NAME = 'Y'
						AND
						LEN(RTRIM(LTRIM(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"])."))) <= 0
					)
				)
				AND
				U.LOGIN LIKE '".$template."'
	...
	function GetNameFieldsForQuery($sNameTemplate, $userTablePrefix = "U.")
	{
		...
		$res = str_replace(
			...
			array(
				...
				"CASE WHEN (LEN(LTRIM(RTRIM(".$userTablePrefix."LAST_NAME))) <= 0) THEN '' ELSE ".$userTablePrefix."LAST_NAME + ',' END",
				$userTablePrefix."LAST_NAME",
				$userTablePrefix."SECOND_NAME", 
				"CASE WHEN (LEN(LTRIM(RTRIM(".$userTablePrefix."NAME))) <= 0) THEN '' ELSE SUBSTRING(".$userTablePrefix."NAME,1,1)+'.' END", 
				"CASE WHEN (LEN(LTRIM(RTRIM(".$userTablePrefix."SECOND_NAME))) <= 0) THEN '' ELSE SUBSTRING(".$userTablePrefix."SECOND_NAME,1,1)+'.' END",
				" + ' ' + "
			),
			implode("", $matches[0])
		);
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "SITE_ID":
					...
					if (strlen($val) <= 0)
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(FSTAT.".$key." IS NULL OR LEN(FSTAT.".$key.")<=0)";
					elseif ($strOperation == "IN")
						$arSqlSearch[] = ($strNegative=="Y"?" FSTAT.".$key." IS NULL OR NOT ":"")."(FSTAT.".$key." IN (".$val.")".(
							$bOrNull ? " OR (FSTAT.".$key." IS NULL OR LEN(FSTAT.".$key.")<=0)" : "").")";
					else
				...
				case "PERIOD":
					if(strLen($val)<=0)
						...
						$arSqlSearch[] = ($strNegative=="Y"?" FSTAT.LAST_VISIT IS NULL OR NOT ":"")."(DATEDIFF(SECOND, FSTAT.LAST_VISIT, GETUTCDATE()) ".$strOperation." ".intVal($val).")";
						break;
				case "HIDE_FROM_ONLINE":
