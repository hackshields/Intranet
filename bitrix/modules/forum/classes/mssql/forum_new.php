...
/***************** /Event ******************************************/
	...
	function OnReindex($NS = array(), $oCallback = NULL, $callback_method = "")
	{
		...
		$strSql =
			"SELECT /*TOP*/ FT.ID as TID, FM.ID as MID, FM.ID as ID, FT.FORUM_ID, FT.TITLE, FT.DESCRIPTION,  FT.TAGS, FT.HTML as FT_HTML,
		...
		if ($search_message_count > 0)
			$strSql = str_replace("/*TOP*/", "TOP ".$search_message_count, $strSql);
