	...
	function GetList($arOrder = Array("ID"=>"ASC"), $arFilter = Array(), $bCount = false, $iNum = 0, $arAddParams = array())
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "NEW_TOPIC":
					if (strlen($val)<=0)
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(FM.".$key." IS NULL OR LEN(FM.".$key.")<=0)";
					else
						...
						foreach ($val as $value)
						{
							...
							foreach ($value as $k => $v)
							{
								...
								switch ($k)
								{
									...
									case "APPROVED":
										if (strlen($v)<=0)
											$str[] = ($strNegative=="Y"?"NOT":"")."(FM.APPROVED IS NULL OR LEN(FM.APPROVED)<=0)";
										else
		...
		if (($iNum>0) || (is_array($arAddParams) && (intVal($arAddParams["nTopCount"])>0)))
		{
			...
			$strSql = str_replace("/*TOP*/", "TOP ".$iNum, $strSql);
		}
...
//					$arSqlSelect["LAST_VISIT"] = $DB->DateToCharFunction("FUT.LAST_VISIT", "FULL");
				...
				case "TITLE":
					...
					if ($strOperation == "LIKE")
						$val = "%".$val."%";
					if (strlen($val)<=0)
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(FT.".$key." IS NULL OR LEN(FT.".$key.")<=0)";
					else
	...
	function QueryFirstUnread($arFilter)  // out-of-date function
	{
		...
		$strSql =
			"SELECT /*TOP*/ FM.ID, FM.AUTHOR_ID, FM.AUTHOR_NAME, FM.AUTHOR_EMAIL, FM.AUTHOR_IP, ".
			"	FM.AUTHOR_REAL_IP, FM.USE_SMILES, FM.POST_MESSAGE, FM.APPROVED, FM.FORUM_ID, ".
			"	FM.TOPIC_ID, FM.NEW_TOPIC, ".
			$DB->DateToCharFunction("FM.POST_DATE")." as POST_DATE, ".
			"	FM.GUEST_ID, FM.ATTACH_IMG ".
			"FROM b_forum_message FM ".
			"WHERE FM.ID IN (".
			"	SELECT MIN(FM1.ID) ".
			"	FROM b_forum_message FM1 ".
			"	WHERE FM1.FORUM_ID = ".intVal($arFilter["FORUM_ID"])." ";
		if (array_key_exists(">ID", $arFilter))
			$strSql .= " AND FM1.ID > ".$arFilter[">ID"]." ";
		if (array_key_exists("!@ID", $arFilter))
			$strSql .= " AND FM1.ID NOT IN (".$arFilter["!@ID"].") ";
		if (array_key_exists("APPROVED", $arFilter) && $arFilter["APPROVED"] == "Y")
			$strSql .= " AND FM1.APPROVED='Y' ";
		if (array_key_exists("TOPIC_ID", $arFilter))
			$strSql .= " AND FM1.TOPIC_ID=".intVal($arFilter["TOPIC_ID"])." ";
		$strSql .= ") ";
...
class CForumFiles extends CAllForumFiles
{
	...
	function CleanUp()
	{
		global $DB;
		$period = 24*3600;
		$db_res = $DB->Query("SELECT FF.FILE_ID FROM b_forum_file FF WHERE (DATEDIFF(SECOND, FF.TIMESTAMP_X, GETUTCDATE()) >= ".$period.
