	...
	function GetListEx($arOrder = Array("ID"=>"ASC"), $arFilter = Array(), $bCount = false, $iNum = 0, $arAddParams = array())
	{
		...
		$strSql = 
			"SELECT 
				...
				CASE
					WHEN ((FU.SHOW_NAME='Y') AND (LENGTH(".CForumUser::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).") IS NOT NULL))
				...
				CASE
					WHEN ((FUU.SHOW_NAME='Y') AND (LENGTH(".CForumUser::GetNameFieldsForQuery($arAddParams["sNameTemplate"], "UU.").") IS NOT NULL))
