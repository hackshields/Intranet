...
class CRatingsComponentsForum extends CAllRatingsComponentsForum
{
	...
	function CalcUserVoteForumPost($arConfigs)
	{
		...
		$strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
					...
					AND RVE.RATING_VOTING_ID = RV.ID".(IntVal($arConfigs['CONFIG']['LIMIT']) > 0 ? " AND RVE.CREATED  > SYSDATE-".IntVal($arConfigs['CONFIG']['LIMIT']) : "")."
					GROUP BY AUTHOR_ID";
	...
	function CalcUserRatingForumActivity($arConfigs)
	{
		...
		$strSql = "INSERT INTO b_rating_component_results (RATING_ID, MODULE_ID, RATING_TYPE, NAME, COMPLEX_NAME, ENTITY_ID, ENTITY_TYPE_ID, CURRENT_VALUE)
			...
			FROM (
				...
				SELECT
					AUTHOR_ID  ENTITY_ID,
					SUM(CASE WHEN POST_DATE>SYSDATE-1 THEN 1 ELSE 0 END) *".floatval($arConfigs['CONFIG']['TODAY_POST_COEF'])." +
					SUM(CASE WHEN POST_DATE>SYSDATE-7 THEN 1 ELSE 0 END) *".floatval($arConfigs['CONFIG']['WEEK_POST_COEF'])."+
				...
				SELECT
					...
					SUM(CASE WHEN START_DATE>SYSDATE-1 THEN 1 ELSE 0 END)*".floatval($arConfigs['CONFIG']['TODAY_TOPIC_COEF'])." +
					SUM(CASE WHEN START_DATE>SYSDATE-7 THEN 1 ELSE 0 END)*".floatval($arConfigs['CONFIG']['WEEK_TOPIC_COEF'])." +
					COUNT(*)*".$DB->ForSql($arConfigs['CONFIG']['MONTH_TOPIC_COEF'])."  CURRENT_VALUE
				FROM b_forum_topic
				WHERE START_DATE  > SYSDATE-30
				GROUP BY USER_START_ID
			) q
