...
/***************** /Event ******************************************/
		if (empty($arFields))
			return false;
		$ID = intVal($DB->NextID("SQ_B_FORUM"));
		$arInsert = $DB->PrepareInsert("b_forum", $arFields);
		$strSql = "INSERT INTO b_forum(ID, ".$arInsert[0].") VALUES(".$ID.", ".$arInsert[1].")";
...
/***************** /Event ******************************************/
	...
	function OnReindex($NS = array(), $oCallback = NULL, $callback_method = "")
	{
		...
		$strSql =
			"SELECT /*+FIRST_ROWS*/ FT.ID as TID, FM.ID as MID, FM.ID as ID, FT.FORUM_ID, FT.TITLE, FT.DESCRIPTION, FT.TAGS, FT.HTML as FT_HTML,
		...
		if ($search_message_count > 0)
			$strSql = "SELECT * FROM (".$strSql.") WHERE ROWNUM<".$search_message_count;
...
/***************** /Event ******************************************/
		if (empty($arFields))
			return false;
		$ID = intVal($DB->NextID("SQ_B_FORUM_GROUP"));
		$arInsert = $DB->PrepareInsert("b_forum_group", $arFields);
		$strSql = "INSERT INTO b_forum_group(ID, ".$arInsert[0].") VALUES(".$ID.", ".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
...
class CForumSmile extends CAllForumSmile
{
	function Add($arFields)
	{
		...
		$ID = intVal($DB->NextID("SQ_B_FORUM_SMILE"));
		$arInsert = $DB->PrepareInsert("b_forum_smile", $arFields);
		$strSql = "INSERT INTO b_forum_smile(ID, ".$arInsert[0].") VALUES(".$ID.", ".$arInsert[1].")";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
