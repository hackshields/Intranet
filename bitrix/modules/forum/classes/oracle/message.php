...
/***************** /Event ******************************************/
		...
		$ID = intVal($DB->NextID("SQ_B_FORUM_MESSAGE"));
		$strSql = "INSERT INTO b_forum_message(ID, ".$arInsert[0].$strDatePostField.") VALUES(".$ID.", ".$arInsert[1].$strDatePostValue.")";
		$DB->QueryBind($strSql,
...
//					$arSqlSelect["LAST_VISIT"] = $DB->DateToCharFunction("FUT.LAST_VISIT", "FULL");
	...
	function QueryFirstUnread($arFilter) // out-of-date function
	{
		...
		$strSql =
			"SELECT FM.ID, FM.AUTHOR_ID, FM.AUTHOR_NAME, FM.AUTHOR_EMAIL, FM.AUTHOR_IP, ".
			"	FM.AUTHOR_REAL_IP, FM.USE_SMILES, FM.POST_MESSAGE, FM.APPROVED, FM.SOURCE_ID, FM.FORUM_ID, ".
			"	FM.TOPIC_ID, FM.NEW_TOPIC, TO_CHAR(FM.POST_DATE, 'DD.MM.YYYY HH24:MI:SS') as POST_DATE, ".
...
class CForumFiles extends CAllForumFiles
{
	...
	function CleanUp()
	{
		global $DB;
		$period = 24*3600;
		$db_res = $DB->Query("SELECT FF.FILE_ID FROM b_forum_file FF WHERE ((CURRENT_DATE - FF.TIMESTAMP_X)*86400 >= ".$period.
