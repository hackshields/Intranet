...
class CForumUser extends CAllForumUser
{
	function GetList($arOrder = Array("ID"=>"ASC"), $arFilter = Array(), $arAddParams = array())
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "PERSONAL_BIRTHDAY_DATE":
					$arSqlSearch[] = ($strNegative=="Y"?" U.PERSONAL_BIRTHDAY IS NULL OR NOT ":"")."(TO_CHAR(U.PERSONAL_BIRTHDAY, 'mm-DD') ".$strOperation." '".$DB->ForSql($val)."')";
					break;
				case "LAST_VISIT":
					if(strLen($val)<=0)
						...
						$arSqlSearch[] =
							"(
								(
									FU.SHOW_NAME = 'Y'
									AND
									TRIM(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).") IS NOT NULL
									AND
								...
								(
									(
										...
										(
											FU.SHOW_NAME = 'Y'
											AND
											TRIM(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).") IS NULL
										)
									)
									AND
									(
		...
		if (count($arSqlOrder) > 0)
		{
			...
			if (in_array($by, array("LAST_POST", "NUM_POSTS", "POINTS")))
				$strSqlOrder .= " nulls ".($order == "ASC" ? "first" : "last");
		}
	...
	function SearchUser($template, $arAddParams = array())
	{
		...
		$strSql =
			"SELECT U.ID, U.NAME, U.SECOND_NAME, U.LAST_NAME, U.LOGIN, F.SHOW_NAME,
				CASE
					WHEN (F.SHOW_NAME = 'Y' AND (TRIM(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).") IS NOT NULL))
					THEN TRIM(REPLACE(".self::GetNameFieldsForQuery($arAddParams["sNameTemplate"]).", '  ', ' '))
					ELSE U.LOGIN
				END AS SHOW_ABC
			FROM b_user U
		...
		if (substr($template, 0, 1) == '%')
			...
			"(
				...
				(
					(
						F.SHOW_NAME = 'N' OR (F.SHOW_NAME IS NULL)
						OR
						(
							F.SHOW_NAME = 'Y'
							AND
			...
			$strSql .=
			"(
				F.SHOW_NAME = 'Y' AND (TRIM(U.NAME) IS NOT NULL) AND U.NAME LIKE '".$template."'
			)
			OR
			(
				F.SHOW_NAME = 'Y' AND (TRIM(U.NAME) IS NULL)
				AND
				(TRIM(U.LAST_NAME) IS NOT NULL) AND U.LAST_NAME LIKE '".$template."'
			)
			OR
			(
				(
	...
	function GetNameFieldsForQuery($sNameTemplate, $userTablePrefix = "U.")
	{
		...
		$res = str_replace(
			...
			array(
				...
				"CASE WHEN TRIM(".$userTablePrefix."LAST_NAME) IS NULL THEN '' ELSE ".$userTablePrefix."LAST_NAME||',' END",
				$userTablePrefix."LAST_NAME",
				$userTablePrefix."SECOND_NAME", 
				"CASE WHEN TRIM(".$userTablePrefix."NAME) IS NULL THEN '' ELSE SUBSTR(".$userTablePrefix."NAME,1,1)||'.' END", 
				"CASE WHEN TRIM(".$userTablePrefix."SECOND_NAME) IS NULL THEN '' ELSE SUBSTR(".$userTablePrefix."SECOND_NAME,1,1)||'.' END",
				"||' '||"
			),
			implode("", $matches[0])
		);
					...
					" WHEN (".
						...
						"TRIM(".
							CForumUser::GetNameFieldsForQuery(
								...
								$arParams["sUserTablePrefix"]).") IS NOT NULL".
					") ".
					" THEN TRIM(REPLACE(".
						CForumUser::GetNameFieldsForQuery(
							$arParams["sNameTemplate"],
...
class CForumRank extends CAllForumRank
{
	...
	function Add($arFields)
	{
		...
		$ID = intVal($DB->NextID("SQ_B_FORUM_RANK"));
		$arInsert = $DB->PrepareInsert("b_forum_rank", $arFields);
		$strSql = "INSERT INTO b_forum_rank(ID, ".$arInsert[0].") VALUES(".$ID.", ".$arInsert[1].")";
			...
			switch ($key)
			{
				...
				case "PERIOD":
					...
					if(strLen($val)<=0)
						...
						$arSqlSearch[] = ($strNegative=="Y"?" FSTAT.LAST_VISIT IS NULL OR NOT ":"")."((CURRENT_DATE - FSTAT.LAST_VISIT)*86400 ".$strOperation." ".intVal($val)." )";
						break;
				case "HIDE_FROM_ONLINE":
					...
					if (strLen($val)<=0)
						...
						$arSqlSearch[] = ($strNegative=="Y"?" FU.".$key." IS NULL OR NOT ":"")."(((FU.".$key." ".$strOperation." '".$DB->ForSql($val)."' ) AND (FSTAT.USER_ID  IS NOT NULL)) OR (FSTAT.USER_ID IS NULL))";
					break;
				break;
				case "ACTIVE":
