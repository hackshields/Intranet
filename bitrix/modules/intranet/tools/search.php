...
{
	function OnSearchGetFileContent($absolute_path)
	{
		global $APPLICATION;
	...
	function ProcessExec($program, $absolute_path)
	{
		...
		elseif(function_exists('exec') && file_exists($absolute_path) && is_file($absolute_path))
		{
			...
			if(strpos($program, "#FILE_NAME#") !== false)
				$program = str_replace("#FILE_NAME#", $absolute_path, $program);
			if(strpos($program, "#SHORT_FILE_NAME#") !== false)
			{
				$short_name = "";
				exec("\"\"".$_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/intranet/tools/search.bat\" \"".$absolute_path."\"\"", $ar);
				foreach($ar as $line)
					if(preg_match("/^Short PathName : (.*)$/", rtrim($line, "\n\r"), $match))
						$short_name = $match[1];
				$program = str_replace("#SHORT_FILE_NAME#", $short_name, $program);
			}
