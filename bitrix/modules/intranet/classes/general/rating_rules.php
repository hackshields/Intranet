	...
	function subordinateCheck($arConfigs)
	{
		...
		if ($iblockId > 0 && $fieldId > 0)
		{
			...
			$squery = "
				...
				SELECT '".intval($ratingId)."' RATING_ID, U2U.USER_ID ENTITY_ID, SUM(".$DB->IsNull("RU.BONUS", "RUS.BONUS").") VOTES 
				FROM
				(
				 SELECT DISTINCT U.ID USER_ID, UP.VALUE_ID SUBORDINATE_ID
				 FROM
				 b_user U
				 INNER JOIN b_utm_user UD ON UD.VALUE_ID = U.ID AND UD.FIELD_ID = ".$fieldId."
