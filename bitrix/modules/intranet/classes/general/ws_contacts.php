...
// it's needed to check is current user an intranet user if he requested intranet script
		...
		$obUsers = CUser::GetList($by='id', $order='asc', $arFilter, array('SELECT' => array('UF_DEPARTMENT')));
		if ($rowLimit > 0)
		{
			$obUsers->NavStart($rowLimit, false, $page);
		}
	...
	function GetAttachmentCollection($listName, $listItemID)
	{
		...
		{
			$arImage = CIntranetUtils::InitImage($arUser['PERSONAL_PHOTO'], 100, 100);
