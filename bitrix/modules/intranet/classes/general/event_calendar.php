...
class CEventCalendar
{
	function Init($arParams)
	{
		...
		if (CModule::IncludeModule('calendar') && COption::GetOptionString("intranet", "calendar_2", "N") == "Y")
		{
			...
			$this->pathToReserveNew = CHTTP::urlAddParams(preg_replace("/#user_id#/i", $USER->GetID(), $calendarSettings['path_to_user_calendar']), array('EVENT_ID' => 'NEW', 'CHOOSE_MR' => 'Y'));
		}
	...
	function GetDisplayedSPCalendars()
	{
		...
		$arIds = ($str !== false) ? unserialize($str) : array();
...
<?
	...
	function GetUserSettings($arParams = array())
	{
		...
		$UserSettings = $Settings ? unserialize($Settings) : $DefSettings;
		if (!$bStatic)
