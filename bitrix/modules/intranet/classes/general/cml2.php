...
class CUserCMLImport
{
	...
	function ImportMetaData($xml_root_id)
	{
		global $DB;
	...
	function ImportUsers($xml_root_id, $start_time, $interval)
	{
		global $DB;
	...
	function LoadUser($arXMLElement, &$counter)
	{
		...
		else
		{
			...
			if ($CURRENT_USER = $obUser->Add($arFields))
			{
				...
				CUser::SetUserGroup($CURRENT_USER, array($group_id));
		...
		elseif (is_array($arStateHistory) && count($arStateHistory) > 0)
		{
			...
			foreach ($arStateHistory as $date => $arState)
			{
				...
				{
					$fp = fopen($_SERVER['DOCUMENT_ROOT'].'/state.log', 'a');
					fwrite($fp, "==============================================================\r\n");
	...
	function ImportAbsence($xml_root_id, $start_time, $interval)
	{
		global $DB;
