...
$arResult["NAV_STRING"] = $dbUsers->GetPageNavString(GetMessage("SPOL_PAGES"), $arParams["NAV_TEMPLATE"]);
...
while($arUser = $dbUsers->Fetch())
{
	...
	if($componentPage == "user")
	{
		...
		if (CModule::IncludeModule('intranet'))
		{
			...
			$path = "/user_card_".intval($arUser["ID"] / 100);
			if($arParams["CACHE_TIME"] == 0 || $obCache->StartDataCache($arParams["CACHE_TIME"], $arUser["ID"], $path))
			{
				...
				while ($arRes = $dbRes->GetNext())
				{
					...
					$rsUsers1 = CIntranetUtils::GetDepartmentEmployees(array($arRes['ID']), $bRecursive=true);
