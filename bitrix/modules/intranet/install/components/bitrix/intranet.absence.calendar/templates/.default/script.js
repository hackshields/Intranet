...
/* methods  */
	...
	SetData: function(DATA, current_data_id)
	{
		...
		for(var i = 0; i < DATA.length; i++)
		{
			if (null != DATA[i].DATA)
			{
				for (var j = 0; j < DATA[i].DATA.length; j++)
				{
					...
					DATA[i].DATA[j].DATE_ACTIVE_FROM = new Date(parseInt(DATA[i].DATA[j].DATE_ACTIVE_FROM) * 1000);
					DATA[i].DATA[j].DATE_ACTIVE_TO = new Date(parseInt(DATA[i].DATA[j].DATE_ACTIVE_TO) * 1000);
					DATA[i].DATA[j].DATE_ACTIVE_FROM.setHours(DATA[i].DATA[j].DATE_ACTIVE_FROM.getHours() + (this.SETTINGS.SERVER_TIMEZONE_OFFSET/3600));
					DATA[i].DATA[j].DATE_ACTIVE_TO.setHours(DATA[i].DATA[j].DATE_ACTIVE_TO.getHours() + (this.SETTINGS.SERVER_TIMEZONE_OFFSET/3600));

					if (DATA[i].DATA[j].DATE_ACTIVE_TO.getHours() == 0)
					{
	...
	RegisterEntry: function(arEntry)
	{
		...
		if (null != arEntry.VISUAL)
		{
			...
			if (null == this._INFO_CACHE[arEntry.ID])
				this._INFO_CACHE[arEntry.ID] = new JCCalendarInfoWin(arEntry.ID, arEntry.ENTRY_TYPE, arEntry.USER_ID, this.LOADER, this.TYPE_BGCOLORS);

			arEntry.VISUAL.INFO = this._INFO_CACHE[arEntry.ID];
	...
	UnRegisterEntry: function(arEntry)
	{
		if (null != arEntry.VISUAL)
		{
			if (null != this._INFO_CACHE && null != this._INFO_CACHE[arEntry.ID])
			{
				...
				this._INFO_CACHE[arEntry.ID] = null;
			}
...
function JCCalendarInfoWin(entry_id, entry_type, user_id, loader, typeBgColors)
{
	...
	this._Show = function(data)
	{
		...
		if (null != data)
		{
			...
			{
				departments += (departments.length > 0 ? ', ' : '') + '<a href="' + jsBXAC.SETTINGS.DETAIL_URL_DEPARTMENT.replace(/#ID#/g, _this.INFO.USER.UF_DEPARTMENT[i].ID) + '">' + _this.INFO.USER.UF_DEPARTMENT[i].NAME + '</a>';
			}
			...
			_this.DIV.innerHTML = '<div class="bx-calendar-info-header"><a class="bx-calendar-info-close" id="bx_top_close_' + _this.ID + '"></a></div>'
				+ '<div class="bx-calendar-info-data">'
					...
					+ '<div class="bx-calendar-info-data-cont">'
						...
						+ '<div class="bx-info-entry bx-calendar-color-' + _this.INFO.ENTRY.TYPE + '" style="background: '+typeBgColors[_this.INFO.ENTRY.TYPE]+'">'
							+ _this.INFO.ENTRY.PROPERTY_ABSENCE_TYPE_VALUE
						+ '</div>'
					...
					+ strAdmin
				+ '</div>'
				+ '<div class="bx-calendar-info-footer"><input type="button" value="' + jsBXAC.MESSAGES.INTR_ABSC_TPL_INFO_CLOSE + '" id="bx_bottom_close_' + _this.ID + '" /></div>';
		...
		document.getElementById('bx_top_close_' + _this.ID).onclick = document.getElementById('bx_bottom_close_' + _this.ID).onclick = function() {_this.Close();}
	}
}
	...
	if (confirm(jsBXCalendarAdmin.DELETE_CONFIRM + ' "' + BX.util.htmlspecialcharsback(this.INFO.ENTRY.NAME) +'"?'))
	{
		...
		BX.ajax.get(
			...
			BX.delegate(
				function()
				{
					CloseWaitWindow(); jsBXAC.LoadData();
				},
				this
			)
		);
...
BX.AbsenceCalendar.Init = function(arParams)
{
	...
	BX.ready(BX.delegate(function()
	{
		BX.AbsenceCalendar.popup = new BX.PopupWindow("BXAbsence", null, {
			...
			closeByEsc: true,
			titleBar: {content: BX.create("span", {html: BX.message('INTR_ABSENCE_TITLE')})},
