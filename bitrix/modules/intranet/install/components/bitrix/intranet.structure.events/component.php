...
if (!$arParams['IBLOCK_TYPE']) $arParams['IBLOCK_TYPE'] = COption::GetOptionInt('intranet', 'iblock_type');
...
if ($arParams['bCache'])
{
	...
	$cache_id = $this->GetName().'|'.$arParams['NUM_USERS'].'|'.$arParams['IBLOCK_ID'].'|'.implode(';', $arParams['USER_PROPERTY']).CDBResult::NavStringForCache($arParams['NUM_USERS'], false);
...
else
{
	...
	if ($strUserIDs != '')
	{
		$dbRes = CIBlockSection::GetList(array('SORT' => 'ASC'), array('IBLOCK_ID' => COption::GetOptionInt('intranet', 'iblock_department'), 'ID' => array_unique($arDepartmentIDs)));
		...
		while ($arUser = $dbUsers->Fetch())
		{
			...
			{
				if (!in_array($key, $arUsedFields) && !in_array($key, $arParams['USER_PROPERTY']))
					unset($arUser[$key]);
			}
