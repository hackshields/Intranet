...
if (StrLen($arResult["FatalError"]) <= 0)
{
	...
	while ($arElement = $dbElements->GetNext())
	{
		...
		if ($arElement["PROPERTY_PERIOD_TYPE_VALUE"] == "DAILY")
		{
			...
			{
				$dayShift = (IntVal(Round(($weekTimeStart - $fromTimeDateOnly) / 86400)) % $arElement["PROPERTY_PERIOD_COUNT_VALUE"]);
				if ($dayShift > 0)
					$dayShift = $arElement["PROPERTY_PERIOD_COUNT_VALUE"] - $dayShift;

				$fromTimeTmp = MkTime(Date("H", $fromTime), Date("i", $fromTime), Date("s", $fromTime), Date("n", $weekTimeStart), Date("j", $weekTimeStart) + $dayShift, Date("Y", $weekTimeStart));
			}
			else
			{
		...
		elseif ($arElement["PROPERTY_PERIOD_TYPE_VALUE"] == "WEEKLY")
		{
			...
			if ($arElement["PROPERTY_PERIOD_COUNT_VALUE"] > 1)
			{
				if ($weekTimeStart > $fromTime || $weekTimeEnd <= $fromTime)
				{
					...
					$weekShift = IntVal(Round(($weekTimeStart - $wts) / 604800));
