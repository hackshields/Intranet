...
{
	public static function drawEmployeeStructure($arStructure, $arSections, $key, $name)
	{
		foreach ($arStructure[$key] as $id)
		{
			...
			if (is_array($arStructure[$id]))
				static::drawEmployeeStructure($arStructure, $arSections, $id, $name);
	...
	public static function getExtranetUsers($id)
	{
		...
		if(method_exists('CExtranet', 'GetExtranetUserGroupID'))
		{
			...
			if ($groupId !== false)
			{
				...
				if (IsModuleInstalled('bitrix24'))
				{
					$filterExtranetUsers['!LAST_LOGIN'] = false;
				}
