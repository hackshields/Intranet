	...
	&& (!CModule::IncludeModule('extranet') || CExtranet::IsIntranetUser() || $sectionId == 'extranet'))
{
	...
	$arFilter = array('ACTIVE' => $showActiveUsers);
	...
	if ($sectionId != "extranet")
	{
		...
		if ($ufHead > 0)
		{
			$dbUsers = CUser::GetList(
				...
				array('ID' => $ufHead, 'ACTIVE' => $showActiveUsers),
...
elseif ($_REQUEST['MODE'] == 'SEARCH')
{
	...
	$arUsers    = array();
	$arUserIDs  = array();
	$dbRes      = CUser::SearchUserByName(explode(' ', urldecode($search)), '', true);
	$strUserIDs = '';
	...
	//filter extranet users
	if (CModule::IncludeModule("extranet") && $showExtranetUsers != SHOW_ALL)
	{
		if ($showExtranetUsers == SHOW_FROM_MY_GROUPS)
		{
			$arFilteredUserIDs = CExtranet::GetMyGroupsUsersSimple(CExtranet::GetExtranetSiteID());
		...
		elseif ($showExtranetUsers == SHOW_NONE)
		{
			$arFilter["!UF_DEPARTMENT"] = false;
		}
	}
	elseif (IsModuleInstalled("bitrix24") && !IsModuleInstalled("extranet"))
	{
		$arFilter["!UF_DEPARTMENT"] = false;
	}
