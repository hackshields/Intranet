...
?>
		<a href="<?=htmlspecialcharsbx($arUsers[$arEntry['UF_HEAD']]['PROFILE_URL'])?>" class="structure-avatar"<?if ($arUsers[$arEntry['UF_HEAD']]['PERSONAL_PHOTO']):?> style="background: url('<?=$arUsers[$arEntry['UF_HEAD']]['PERSONAL_PHOTO']['CACHE']['src']?>') no-repeat scroll center center transparent;"<?endif;?>></a>
...
?>
		<span class="structure-avatar"<?if ($arUsers[$arEntry['UF_HEAD']]['PERSONAL_PHOTO']):?> style="background: url('<?=$arUsers[$arEntry['UF_HEAD']]['PERSONAL_PHOTO']['CACHE']['src']?>') no-repeat scroll center center transparent;"<?endif;?>></span>
...
?><span class="structure-avatar"<?if ($arEntry['EMPLOYEES'][$i]['PERSONAL_PHOTO']):?> style="background: url('<?=$arEntry['EMPLOYEES'][$i]['PERSONAL_PHOTO']['CACHE']['src']?>') no-repeat scroll center center transparent;"<?endif;?> data-user="<?=$arEntry['EMPLOYEES'][$i]['ID']?>" data-dpt="<?=$arEntry['ID']?>"></span><?

			$arJSEmployees[] = array(
				'ID' => $arEntry['EMPLOYEES'][$i]['ID'],
				'NAME' => $name,
				'POSITION' => $arEntry['EMPLOYEES'][$i]['WORK_POSITION'],
				'PHOTO' => $arEntry['EMPLOYEES'][$i]['PERSONAL_PHOTO']['CACHE']['src'],
...
<?
		if ($bHasChildren):
			$childrenNum = floor(($arEntry['RIGHT_MARGIN']-$arEntry['LEFT_MARGIN'])/2);
...
new BX.IntranetVSBlock({
	section_id: <?=$arEntry['ID']?>,
	section_level: <?=$arEntry['DEPTH_LEVEL']?>,
...
?>
<span class="webform-small-button task-list-toolbar-create" onclick="BX.IntranetStructure.ShowForm({IBLOCK_SECTION_ID: <?=$arTopEntry['ID']?>});"><span class="webform-small-button-left"></span><span class="webform-small-button-icon"></span><span class="webform-small-button-text"><?=GetMessage('ISV_ADD_DEPARTMENT')?></span><span class="webform-small-button-right"></span></span>
<?
			else:
?>
<div class="intranet-title-buttons">
	<a class="intranet-title-button" href="javascript:void(0)" onclick="BX.IntranetStructure.ShowForm({IBLOCK_SECTION_ID: <?=$arTopEntry['ID']?>});">
		<i class="intranet-title-button-create-icon"></i><span class="intranet-title-link"><?=GetMessage('ISV_ADD_DEPARTMENT')?></span>
	</a>
</div>
<?
			endif;
