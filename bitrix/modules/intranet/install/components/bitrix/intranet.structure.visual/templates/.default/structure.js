	...
	if (!this.employeesPopup)
	{
		...
		for (var i=0; i < this.employees.length; i++)
		{
			var obEmployee = BX.create('DIV', {
				...
				html: '<a' + (this.employees[i].PHOTO ? ' style="background: url(\''+this.employees[i].PHOTO+'\') no-repeat scroll center center transparent;"' : '') + ' class="structure-avatar" href="'+this.employees[i].PROFILE+'"></a><div class="structure-employee-name"><a href="'+this.employees[i].PROFILE+'">' + this.employees[i].NAME + '</a></div>' + (this.employees[i].POSITION ? '<div class="structure-employee-post">'+BX.util.htmlspecialchars(this.employees[i].POSITION)+'</div>' : '')
			});
...
BX.IntranetVSBlock.prototype._showNext = function (data) {
		...
		this.node.style.zIndex = 50 * this.section_level;
