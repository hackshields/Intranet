...
if ($arParams['bCache'])
{
	...
	$cache_id = $this->GetName().'|'.$arParams['NUM_USERS'].'|'.SITE_ID;//.'|'.$USER->GetGroups();
...
else
{
	...
	//if (!CModule::IncludeModule('extranet') || !CExtranet::IsExtranetSite())
		$arNavParams = array('iNumPage' => 1, 'nPageSize' => $arParams['NUM_USERS']);
	//else
	//		$arNavParams = false;
	...
	{
//		if (CModule::IncludeModule('extranet') && CExtranet::IsExtranetSite() && !in_array($arUser["ID"], $arUsersInMyGroupsID) && !in_array($arUser["ID"], $arPublicUsersID))
//			continue;
