...
class CIEmployeeProperty
{
	...
	function GetEditForm($value, $strHTMLControlName)
	{
		...
		$CURRENT_USER = $dbCurrentUser->Fetch();

		$bAllowCurrentUser = is_array($CURRENT_USER['UF_DEPARTMENT']) && count($CURRENT_USER['UF_DEPARTMENT']) > 0;

		$default_value = intVal($value["VALUE"]);
		$res = "";
		if ($bAllowCurrentUser && $default_value == $USER->GetID())
		{
			$select = "CU";
		...
		ob_start();
		?>
<input type="text" name="<?echo htmlspecialcharsbx($strHTMLControlName["VALUE"])?>" id="<?echo $name_x?>" value="<?echo intval($value['VALUE']) > 0 ? intval($value['VALUE']) : ''?>" size="3" class="typeinput" />&nbsp;&nbsp;<?$GLOBALS['APPLICATION']->IncludeComponent('bitrix:intranet.user.search', '', array('INPUT_NAME' => $name_x, 'MULTIPLE' => 'N', 'SHOW_BUTTON' => 'Y'), null, array('HIDE_ICONS' => 'Y'))?><IFRAME style="width:0px; height:0px; border: 0px; display: none;" src="javascript:void(0)" name="hiddenframe<?echo htmlspecialcharsbx($strHTMLControlName["VALUE"])?>" id="hiddenframe<?=$name_x?>"></IFRAME><span id="div_<?=$name_x?>"></span>
<script>
var value_<?=$name_x?> = '';
function Ch<?=$name_x?>()
{
...
class CUserTypeEmployee extends CIEmployeeProperty
{
	...
	{
		return CUser::GetList($by = 'last_name', $order='asc', array('!UF_DEPARTMENT' => false));
	}
}
...
class CIBlockPropertyEmployee extends CIEmployeeProperty
{
	...
	function GetPublicFilterHTML($arProperty, $strHTMLControlName)
	{
		...
		<span id="structure-department-head<?echo $controlID?>" class="structure-department-head" <?if ($UF_HeadName != ""):?>style="visibility:visible"<?endif;?> onclick='BX("<?echo $controlID?>").value = ""; BX("<?echo $controlID?>_name").innerHTML = ""; BX("structure-department-head").style.visibility="hidden";'></span><br>
