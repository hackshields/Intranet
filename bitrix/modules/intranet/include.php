...
*/
	...
	function onAfterForumMessageAdd($ID, $arFields)
	{
		// add log comment
		if (
				...
				if ($arElement = $rsElement->Fetch())
				{
					...
					if ($arIBlock = $rsIBlock->Fetch())
					{
						...
						if($arSite = $rsSites->Fetch())
						{
							...
							$url = str_replace(
								array("#SITE_DIR#", "#ID#", "#CODE#"),
								array($arSite["DIR"], $arElement["ID"], $arElement["CODE"]),
								$arIBlock["DETAIL_PAGE_URL"]
							);
							if (strpos($url, "/") === 0)
								$url = "/".ltrim($url, "/");
							...
							$arSoFields = Array(
								...
								"=LOG_DATE" => $GLOBALS["DB"]->CharToDateFunction($arElement["TIMESTAMP_X"], "FULL", $arSite["ID"]),
								"TITLE_TEMPLATE" => GetMessage("INTR_SOCNET_LOG_NEWS_TITLE"),
								"TITLE" => $arElement["NAME"],
								"MESSAGE" => (strlen($arElement["PREVIEW_TEXT"]) > 0 ? $arElement["PREVIEW_TEXT"] : $arElement["DETAIL_TEXT"]),
								"TEXT_MESSAGE" => "",
								"URL" => $url,
								"MODULE_ID" => "intranet",
								"CALLBACK_FUNC" => false,
