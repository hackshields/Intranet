...
class CWorkflow extends CAllWorkflow
{
	...
	function Insert($arFields)
	{
		...
		$strSql = "INSERT INTO b_workflow_document(DATE_MODIFY, DATE_ENTER,  ".$arInsert[0].") VALUES(getdate(), getdate(), ".$arInsert[1].")";
	...
	function Update($arFields, $DOCUMENT_ID)
	{
		...
		$strSql = "UPDATE b_workflow_document SET ".$strUpdate.", DATE_MODIFY=getdate(), DATE_ENTER=getdate() WHERE ID=".$DOCUMENT_ID;
		$DB->Query($strSql, false, $err_mess.__LINE__);
		if ($change)
		{
			...
			SELECT
				...
				case
					when DATE_LOCK is null then 'green'
						...
						case
							when dateadd(minute, $MAX_LOCK, DATE_LOCK) < getdate() then 'green'
							else
								case
		...
		if (is_array($arFilter))
		{
			...
			for ($i=0; $i<count($filter_keys); $i++)
			{
				...
				switch($key)
				{
					...
					case "DATE_MODIFY_2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "D.DATE_MODIFY < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						break;
					case "MODIFIED_BY":
						...
						$arSqlSearch[] = "
							case
								when D.DATE_LOCK is null then 'green'
									...
									case
										when dateadd(minute, $MAX_LOCK, D.DATE_LOCK) < getdate() then 'green'
										else
											case
		...
		if (CWorkflow::IsAdmin())
		{
			$strSql = "
				SELECT
					...
					'('+UM.LOGIN+') '+isnull(UM.NAME,'')+' '+isnull(UM.LAST_NAME,'')		MUSER_NAME,
					'('+UE.LOGIN+') '+isnull(UE.NAME,'')+' '+isnull(UE.LAST_NAME,'')		EUSER_NAME,
	...
	function GetByID($ID)
	{
		...
		$strSql = "
			SELECT
				...
				'('+UM.LOGIN+') '+isnull(UM.NAME,'')+' '+isnull(UM.LAST_NAME,'')		MUSER_NAME,
				'('+UE.LOGIN+') '+isnull(UE.NAME,'')+' '+isnull(UE.LAST_NAME,'')		EUSER_NAME,
				'('+UL.LOGIN+') '+isnull(UL.NAME,'')+' '+isnull(UL.LAST_NAME,'')		LUSER_NAME,
	...
	function GetHistoryList(&$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		if (is_array($arFilter))
		{
			...
			for ($i=0; $i<count($filter_keys); $i++)
			{
				...
				switch($key)
				{
					...
					case "DATE_MODIFY_2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "L.TIMESTAMP_X < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						break;
					case "MODIFIED_BY":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="N" && $match_value_set) ? "Y" : "N";
		...
		$strSql = "
			SELECT
				...
				'('+U.LOGIN+') '+isnull(U.NAME,'')+' '+isnull(U.LAST_NAME,'') USER_NAME,
				S.TITLE STATUS_TITLE
			FROM
				b_workflow_log L
	...
	function CleanUpHistory()
	{
		...
		if ($HISTORY_DAYS>=0)
		{
			$strSql = "
				...
				WHERE
					datediff(day, TIMESTAMP_X, getdate())>=$HISTORY_DAYS
				";
			$DB->Query($strSql, false, $err_mess.__LINE__);
		}
	...
	function CleanUpPublished()
	{
		...
		if ($DAYS_AFTER_PUBLISHING>=0)
		{
			$strSql = "
				...
				WHERE
					STATUS_ID = 1
				and datediff(day, DATE_MODIFY, getdate())>=$DAYS_AFTER_PUBLISHING
				";
			$z = $DB->Query($strSql, false, $err_mess.__LINE__);
			while($zr=$z->Fetch())
			{
