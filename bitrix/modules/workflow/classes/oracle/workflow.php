...
class CWorkflow extends CAllWorkflow
{
	...
	function Insert($arFields)
	{
		...
		$ID = $DB->NextID("sq_b_workflow_document");
		$strSql = "INSERT INTO b_workflow_document(ID, DATE_MODIFY, DATE_ENTER, ".$arInsert[0].") VALUES(".$ID.", sysdate, sysdate, ".$arInsert[1].")";
		$arBinds["PROLOG"] = $arFields["PROLOG"];
		$arBinds["EPILOG"] = $arFields["EPILOG"];
	...
	function Update($arFields, $DOCUMENT_ID)
	{
		...
		$strSql = "UPDATE b_workflow_document SET ".$strUpdate.", DATE_MODIFY=sysdate, DATE_ENTER=sysdate WHERE ID=".$DOCUMENT_ID;
		$arBinds["PROLOG"] = $arFields["PROLOG"];
		$arBinds["EPILOG"] = $arFields["EPILOG"];
		$arBinds["BODY"] = $arFields["BODY"];
		$DB->QueryBind($strSql, $arBinds);
	...
	function GetLockStatus($DOCUMENT_ID, &$locked_by, &$date_lock)
	{
		...
		$strSql = "
			SELECT
				...
				decode(DATE_LOCK,
					null, 'green',
					decode(sign(1440*(sysdate-DATE_LOCK)-$MAX_LOCK),
						1, 'green',
						decode(LOCKED_BY,$uid,'yellow','red')))				LOCK_STATUS
			FROM
				b_workflow_document
			WHERE
				ID = '$DOCUMENT_ID'
		...
		if (is_array($arFilter))
		{
			...
			for ($i=0; $i<count($filter_keys); $i++)
			{
				...
				switch($key)
				{
					...
					case "DATE_MODIFY_2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "D.DATE_MODIFY < ".$DB->CharToDateFunction($val, "SHORT")."+1";
						break;
					case "MODIFIED_BY":
					...
					case "LOCK_STATUS":
						$arSqlSearch[] = "
							decode(D.DATE_LOCK, null, 'green',
								decode(sign(1440*(sysdate-D.DATE_LOCK)-$MAX_LOCK), 1, 'green',
									decode(D.LOCKED_BY,$uid,'yellow','red'))) = '".$DB->ForSql($val)."'";
						break;
					case "STATUS":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
					...
					case "BODY":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
						$arSqlSearch[] = GetFilterQuery("D.BODY",$val,$match,array(),"Y","Y","Y");
						break;
		...
		if (CWorkflow::IsAdmin())
		{
			$strSql = "
				SELECT
					...
					'('||UM.LOGIN||') '||nvl(UM.NAME,'')||' '||nvl(UM.LAST_NAME,'')		MUSER_NAME,
					'('||UE.LOGIN||') '||nvl(UE.NAME,'')||' '||nvl(UE.LAST_NAME,'')		EUSER_NAME,
					S.TITLE											STATUS_TITLE,
					decode(D.DATE_LOCK,
						null, 'green',
						decode(sign(1440*(sysdate-D.DATE_LOCK)-$MAX_LOCK),
							1, 'green',
							decode(D.LOCKED_BY,$uid,'yellow','red')))				LOCK_STATUS
				FROM
	...
	function GetByID($ID)
	{
		...
		$strSql = "
			SELECT
				D.ID, D.STATUS_ID, D.ENTERED_BY, D.MODIFIED_BY, D.LOCKED_BY, D.FILENAME, D.SITE_ID,
				D.TITLE, D.BODY_TYPE, D.BODY, D.PROLOG, D.EPILOG, D.COMMENTS,
				".$DB->DateToCharFunction("D.DATE_ENTER")."		DATE_ENTER,
				".$DB->DateToCharFunction("D.DATE_MODIFY")."	DATE_MODIFY,
				".$DB->DateToCharFunction("D.DATE_LOCK")."		DATE_LOCK,
				'('||UM.LOGIN||') '||nvl(UM.NAME,'')||' '||nvl(UM.LAST_NAME,'')		MUSER_NAME,
				'('||UE.LOGIN||') '||nvl(UE.NAME,'')||' '||nvl(UE.LAST_NAME,'')		EUSER_NAME,
				'('||UL.LOGIN||') '||nvl(UL.NAME,'')||' '||nvl(UL.LAST_NAME,'')		LUSER_NAME,
	...
	function GetHistoryList(&$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		if (is_array($arFilter))
		{
			...
			for ($i=0; $i<count($filter_keys); $i++)
			{
				...
				switch($key)
				{
					...
					case "DATE_MODIFY_2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "L.TIMESTAMP_X < ".$DB->CharToDateFunction($val, "SHORT")."+1";
						break;
					case "MODIFIED_BY":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="N" && $match_value_set) ? "Y" : "N";
					...
					case "BODY":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
						$arSqlSearch[] = GetFilterQuery("L.BODY",$val,$match,array(),"Y","Y","Y");
						break;
					case "STATUS":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
		...
		$strSqlSearch = GetFilterSqlSearch($arSqlSearch);
		$strSql = "
			SELECT L.ID, L.DOCUMENT_ID, L.MODIFIED_BY, L.TITLE,
				L.FILENAME, L.SITE_ID, L.STATUS_ID,
				".$DB->DateToCharFunction("L.TIMESTAMP_X")." TIMESTAMP_X,
				'('||U.LOGIN||') '||nvl(U.NAME,'')||' '||nvl(U.LAST_NAME,'') USER_NAME,
				S.TITLE STATUS_TITLE
			FROM
				b_workflow_log L
	...
	function GetHistoryByID($ID)
	{
		...
		$strSql = "
			SELECT
				L.ID, L.DOCUMENT_ID, L.MODIFIED_BY, L.TITLE,
				L.FILENAME, L.SITE_ID, L.BODY, L.BODY_TYPE, L.STATUS_ID,
				".$DB->DateToCharFunction("L.TIMESTAMP_X")." TIMESTAMP_X,
				'('||U.LOGIN||') '||nvl(U.NAME,'')||' '||nvl(U.LAST_NAME,'') USER_NAME,
				S.TITLE STATUS_TITLE
			FROM
	...
	function CleanUpHistory()
	{
		...
		if ($HISTORY_DAYS>=0)
		{
			$strSql = "
				...
				WHERE
					TRUNC(SYSDATE)-TRUNC(TIMESTAMP_X)>=$HISTORY_DAYS
				";
	...
	function CleanUpPublished()
	{
		...
		if ($DAYS_AFTER_PUBLISHING>=0)
		{
			$strSql = "
				...
				WHERE
					STATUS_ID = 1
				and TRUNC(SYSDATE)-TRUNC(DATE_MODIFY)>=$DAYS_AFTER_PUBLISHING
				";
			$z = $DB->Query($strSql, false, $err_mess.__LINE__);
			while($zr=$z->Fetch())
			{
