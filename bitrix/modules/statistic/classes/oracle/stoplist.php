	...
	function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		$strSql =	"
			SELECT
				...
				DECODE (SIGN(SYSDATE - NVL(S.DATE_START,SYSDATE-1)),-1,'red',
					DECODE(SIGN(SYSDATE - NVL(S.DATE_END, SYSDATE+1)), 1, 'red',
					DECODE(S.ACTIVE, 'N', 'red', 'green')
					)) LAMP
			FROM
				b_stop_list S
			WHERE
...
*/
		...
		if(!$arStopRecord)
		{
			...
			$strSql = "
				...
				and TEST='$test'
				$site_where
				and (DATE_START<=SYSDATE or DATE_START is null)
				and (DATE_END>=SYSDATE or DATE_END is null)
				and	(((BITAND(MASK_1,".intval($arr[0]).")=IP_1 and
						BITAND(MASK_2,".intval($arr[1]).")=IP_2 and
						BITAND(MASK_3,".intval($arr[2]).")=IP_3 and
						BITAND(MASK_4,".intval($arr[3]).")=IP_4)
