...
class CPage
{
	function GetDynamicList($URL, &$by, &$order, $arFilter=Array())
	{
		...
		$exit_counter = "SUM(decode(sign(D.EXIT_COUNTER),1,D.EXIT_COUNTER,0))";
		if (is_array($arFilter))
		{
			if (strlen($arFilter["ADV"])>0)
			{
				...
				if ($arFilter["ADV_DATA_TYPE"]=="B")
				{
					...
					$exit_counter = "SUM(decode(sign(A.EXIT_COUNTER_BACK),1,A.EXIT_COUNTER_BACK,0))";
				}
				elseif ($arFilter["ADV_DATA_TYPE"]=="P")
				{
					$counter = "SUM(A.COUNTER)";
					$enter_counter = "SUM(A.ENTER_COUNTER)";
					$exit_counter = "SUM(decode(sign(A.EXIT_COUNTER),1,A.EXIT_COUNTER,0))";
				}
				else
				{
					$counter = "SUM(A.COUNTER + A.COUNTER_BACK)";
					$enter_counter = "SUM(A.ENTER_COUNTER + A.ENTER_COUNTER_BACK)";
					$exit_counter = "SUM(decode(sign(A.EXIT_COUNTER),1,A.EXIT_COUNTER,0) + decode(sign(A.EXIT_COUNTER_BACK),1,A.EXIT_COUNTER_BACK,0))";
				}
			}
	...
	function GetList($COUNTER_TYPE, &$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		if (is_array($arFilter))
		{
			if (strlen($arFilter["ADV"])>0)
			{
				...
				{
					$counter = "decode(sign(A.".$COUNTER_TYPE."),1,A.".$COUNTER_TYPE.",0) + decode(sign(A.".$COUNTER_TYPE."_BACK),1,A.".$COUNTER_TYPE."_BACK,0)";
			...
			foreach ($arFilter as $key => $val)
			{
				...
				switch($key)
				{
					...
					case "DATE2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "V.DATE_STAT<".$DB->CharToDateFunction($val, "SHORT")."+1";
						break;
					case "SHOW":
					case "DIR":
