...
class CTraffic extends CAllTraffic
{
	...
	function GetDailyList(&$by, &$order, &$arMaxMin, $arFilter=Array(), &$is_filtered, $get_maxmin="Y")
	{
		...
		$strSql = "
			SELECT
				...
				decode(
					to_char(D.DATE_STAT, 'Dy', 'nls_date_language=english'),
					'Mon', 0,
					'Tue', 1,
					'Wed', 2,
					'Thu', 3,
	...
	function GetCommonValues($arFilter=Array(), $bIgnoreErrors=false)
	{
		...
		if(strlen($date1)>0 && CheckDateTime($date1))
		{
			...
			if(strlen($date2)>0 && CheckDateTime($date2))
			{
				...
				$strSqlPeriod = "sum(decode(sign(DATE_STAT-$date_from),-1,0, decode(sign(DATE_STAT-$date_to),1,0,";
				$strT=")))";
			}
			else
			{
				$strSqlPeriod = "sum(decode(sign(DATE_STAT-$date_from),-1,0,";
				$strT="))";
			}
		}
		elseif(strlen($date2)>0 && CheckDateTime($date2))
		{
			$is_filtered = true;
			$date_to = "TO_DATE('".ConvertDateTime($date2,"D.M.Y")." 23:59:59','dd.mm.yyyy hh24:mi:ss')";
			$strSqlPeriod = "sum(decode(sign(DATE_STAT-$date_to),1,0,";
			$strT="))";
		}
		else
		{
		...
		$strSql = "
			SELECT
				sum(HITS)							TOTAL_HITS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate),HITS,0))		TODAY_HITS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-1),HITS,0))		YESTERDAY_HITS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-2),HITS,0))		B_YESTERDAY_HITS,

				sum(SESSIONS)							TOTAL_SESSIONS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate),SESSIONS,0))		TODAY_SESSIONS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-1),SESSIONS,0))	YESTERDAY_SESSIONS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-2),SESSIONS,0))	B_YESTERDAY_SESSIONS,

				sum(C_EVENTS)							TOTAL_EVENTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate),C_EVENTS,0))		TODAY_EVENTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-1),C_EVENTS,0))	YESTERDAY_EVENTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-2),C_EVENTS,0))	B_YESTERDAY_EVENTS,

				sum(C_HOSTS)							TOTAL_HOSTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate),C_HOSTS,0))		TODAY_HOSTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-1),C_HOSTS,0))	YESTERDAY_HOSTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-2),C_HOSTS,0))	B_YESTERDAY_HOSTS,

				sum(NEW_GUESTS)							TOTAL_GUESTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate),GUESTS,0))		TODAY_GUESTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-1),GUESTS,0))		YESTERDAY_GUESTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-2),GUESTS,0))		B_YESTERDAY_GUESTS,

				sum(decode(trunc(DATE_STAT),trunc(sysdate),NEW_GUESTS,0))	TODAY_NEW_GUESTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-1),NEW_GUESTS,0))	YESTERDAY_NEW_GUESTS,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-2),NEW_GUESTS,0))	B_YESTERDAY_NEW_GUESTS,

				sum(FAVORITES)							TOTAL_FAVORITES,
				sum(decode(trunc(DATE_STAT),trunc(sysdate),FAVORITES,0))	TODAY_FAVORITES,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-1),FAVORITES,0))	YESTERDAY_FAVORITES,
				sum(decode(trunc(DATE_STAT),trunc(sysdate-2),FAVORITES,0))	B_YESTERDAY_FAVORITES
				".
				($is_filtered ? ','.
	...
	function GetRefererList(&$by, &$order, $arFilter=Array(), &$is_filtered, $limit=10)
	{
		...
		if (strlen($date1)>0)
		{
			...
			if (strlen($date2)>0)
				$strSqlPeriod = " sum(decode(sign(DATE_HIT-$date_from),-1,0, decode(sign(DATE_HIT-$date_to),1,0,1)))";
			else
				$strSqlPeriod = " sum(decode(sign(DATE_HIT-$date_from),-1,0,1))";
		}
		elseif (strlen($date2)>0)
		{
			$date_filtered = $is_filtered = true;
			$strSqlPeriod = " sum(decode(sign(DATE_HIT-$date_to),1,0,1))";
		}
		...
		$strSql = "
			SELECT
				...
				sum(decode(trunc(DATE_HIT),trunc(sysdate),1,0)) TODAY_REFERERS,
				sum(decode(trunc(DATE_HIT),trunc(sysdate-1),1,0)) YESTERDAY_REFERERS,
				sum(decode(trunc(DATE_HIT),trunc(sysdate-2),1,0)) B_YESTERDAY_REFERERS
				".
		...
		if(intval($limit)>0)
		{
			$strSql = "SELECT * FROM ($strSql) WHERE ROWNUM<=".intval($limit);
		}
	...
	function GetPhraseList(&$s_by, &$s_order, $arFilter=Array(), &$is_filtered, $limit=10)
	{
		...
		$strSql = "
			SELECT
				...
				sum(decode(trunc(DATE_HIT),trunc(sysdate),1,0)) TODAY_PHRASES,
				sum(decode(trunc(DATE_HIT),trunc(sysdate-1),1,0)) YESTERDAY_PHRASES,
				sum(decode(trunc(DATE_HIT),trunc(sysdate-2),1,0)) B_YESTERDAY_PHRASES
				".
