...
class CUserOnline
{
	function GetGuestCount()
	{
		...
		$strSql = "
			...
			WHERE
				datediff(day, S.DATE_STAT, getdate())=0
				and S.DATE_LAST > dateadd(second, - $interval, getdate())
		";
	...
	function GetList(&$guest_count, &$session_count, $arOrder=Array(), $arFilter=Array())
	{
		...
		$strSql = "
			SELECT
				...
				datediff(second, S.DATE_FIRST, S.DATE_LAST) SESSION_TIME,
				".$DB->DateToCharFunction("S.DATE_LAST")." DATE_LAST,
				case when G.SESSIONS<=1 then 'Y' else 'N' end NEW_GUEST,
				G.FIRST_URL_FROM,
				G.FIRST_SITE_ID,
				S.URL_FROM,
				S.COUNTRY_ID,
			...
			WHERE
				S.DATE_STAT >= dateadd(day, -1, getdate())
				and S.DATE_LAST > dateadd(second, - $interval, getdate())
				and ".$strSqlSearch."
			".$strSqlOrder."
		";
