...
class CSession
{
	function GetAttentiveness($DATE_STAT, $SITE_ID=false)
	{
		...
		$strSql = "
			SELECT
				round(sum(datediff(second, S.DATE_FIRST, S.DATE_LAST))/count(S.ID), 2)								AM_AVERAGE_TIME,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) < 60 then 1 else 0 end)					AM_1,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 60 and 179 then 1 else 0 end)		AM_1_3,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 180 and 359 then 1 else 0 end)	AM_3_6,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 360 and 539 then 1 else 0 end)	AM_6_9,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 540 and 719 then 1 else 0 end)	AM_9_12,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 720 and 899 then 1 else 0 end)	AM_12_15,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 900 and 1079 then 1 else 0 end)	AM_15_18,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 1080 and 1259 then 1 else 0 end)	AM_18_21,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) between 1260 and 1439 then 1 else 0 end)	AM_21_24,
				sum(case when datediff(second, S.DATE_FIRST, S.DATE_LAST) >= 1440 then 1 else 0 end)				AM_24,
	...
	function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		if (is_array($arFilter))
		{
			foreach ($arFilter as $key => $val)
			{
				...
				switch($key)
				{
					...
					case "DATE_START_2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "S.DATE_FIRST<dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						break;
					case "DATE_END_1":
						if (CheckDateTime($val))
					...
					case "DATE_END_2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "S.DATE_LAST<dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						break;
					case "IP":
		...
		$strSql = "
			SELECT /*TOP*/
				...
				S.FIRST_SITE_ID,
				S.LAST_SITE_ID,
				round(datediff(second, S.DATE_FIRST, S.DATE_LAST), 0) SESSION_TIME,
