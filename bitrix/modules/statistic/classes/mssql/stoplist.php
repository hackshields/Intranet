	...
	function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		$strSql =	"
			SELECT /*TOP*/
				...
				case
					when
						(S.DATE_START<=getdate() or S.DATE_START is null) and
						(S.DATE_END>=getdate() or S.DATE_END is null) and
						S.ACTIVE='Y'
					then 'green'
					else 'red'
				end LAMP
...
*/
		...
		if(!$arStopRecord)
		{
			...
			if (strlen($site_id)>0)
			{
				$site_where = "and (SITE_ID = '".$DB->ForSql($site_id, 2)."' or SITE_ID is null or len(SITE_ID)<=0)";
			}
			...
			$strSql = "
				...
				and TEST='$test'
				$site_where
				and (DATE_START<=getdate() or DATE_START is null)
				and (DATE_END>=getdate() or DATE_END is null)
				and	((((MASK_1 & ".intval($arr[0]).")=IP_1 and
						...
						and (upper('".$DB->ForSql($user_agent)."') like '%'+upper(USER_AGENT)+'%'
							or len(USER_AGENT)<=0 or USER_AGENT is null)
						and ($user_agent_len=0 or USER_AGENT_IS_NULL<>'Y')
						and (upper('$url_from') like '%'+upper(URL_FROM)+'%'
							or len(URL_FROM)<=0 or URL_FROM is null)
						and (upper('$url_to') like '%'+upper(URL_TO)+'%'
							or len(URL_TO)<=0 or URL_TO is null)
					)
				";
