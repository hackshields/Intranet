...
class CStatEvent extends CAllStatEvent
{
	function GetListByGuest($GUEST_ID, $EVENT_ID=false, $EVENT3=false, $SEC=false)
	{
		...
		if ($SEC!==false)
			$strSqlSearch .= " and E.DATE_ENTER > dateadd(second, - ".intval($SEC).", getdate()) ";
	...
	function Add($EVENT_ID, $EVENT3, $DATE_ENTER, $PARAM, $MONEY="", $CURRENCY="", $CHARGEBACK="N")
	{
		...
		if ($arEvent = $rsEvent->Fetch())
		{
			...
			$DATE_ENTER = strlen(trim($DATE_ENTER))>0 ? $DATE_ENTER : GetTime(time()+CTimeZone::GetOffset(),"FULL");
			$TIME_ENTER_TMSTMP = MakeTimeStamp($DATE_ENTER);
			if (!$TIME_ENTER_TMSTMP)
			{
				$DATE_ENTER = GetTime(time()+CTimeZone::GetOffset(),"FULL");
				$TIME_ENTER_TMSTMP = MakeTimeStamp($DATE_ENTER);
			}

			$TIME_ENTER_SQL = $DB->CharToDateFunction($DATE_ENTER, "FULL");
			$DAY_ENTER_SQL = $DB->CharToDateFunction($DATE_ENTER, "SHORT");
			...
			$rows = $DB->Update("b_stat_event_day",$arFields,"WHERE EVENT_ID='".$EVENT_ID."' and datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0",$err_mess.__LINE__);
			...
			{
				// удалим лишние
				$strSql = "SELECT ID FROM b_stat_event_day WHERE EVENT_ID='".$EVENT_ID."' and datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0 ORDER BY ID";
				$i=0;
			...
			$arFields = Array("C_EVENTS" => "C_EVENTS + 1");
			$DB->Update("b_stat_day",$arFields,"WHERE datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0",$err_mess.__LINE__);
			...
			if (strlen($SITE_ID)>0)
			{
				// обновляем дневной счетчик
				$arFields = Array("C_EVENTS" => "C_EVENTS+1");
				$DB->Update("b_stat_day_site", $arFields, "WHERE SITE_ID='".$DB->ForSql($SITE_ID,2)."' and datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0", $err_mess.__LINE__);
			...
			if ($ADV_ID>0)
			{
				...
				if ($ar = $a->Fetch())
				{
					...
					$rows = $DB->Update("b_stat_adv_event_day",$arFields,"WHERE ADV_ID='$ADV_ID' and EVENT_ID='$EVENT_ID' and datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0", $err_mess.__LINE__);
	...
	function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		if($arFilter["GROUP"]=="total")
		{
			$strSql =	"
				SELECT
					COUNT(*)			COUNTER,
					round(sum((case when E.CHARGEBACK='Y' then -E.MONEY else E.MONEY end)*$rate),2)	MONEY,
					'$view_currency'		CURRENCY
				FROM
					b_stat_event_list E
		...
		else
		{
			$strSql =	"
				SELECT /*TOP*/
					...
					C.NAME																	COUNTRY_NAME,
					case when V.NAME is null or len(V.NAME)<=0 then isnull(V.EVENT1,'')+' / '+isnull(V.EVENT2,'') else V.NAME end EVENT
				FROM
					b_stat_event_list E
				INNER JOIN b_stat_event V ON (V.ID=E.EVENT_ID)
				LEFT JOIN b_stat_country C ON (C.ID=E.COUNTRY_ID)
				...
				$strSqlOrder
				";
				$strSql = str_replace("/*TOP*/", "TOP ".COption::GetOptionString("statistic","RECORDS_LIMIT"), $strSql);
		}
	...
	function Delete($ID)
	{
		...
		if ($ar = $a->Fetch())
		{
			...
			$TIME_ENTER_SQL = $DB->CharToDateFunction($ar["DATE_ENTER"], "FULL");
			...
			$rows = $DB->Update("b_stat_event_day",$arFields,"WHERE EVENT_ID='".intval($ar["EVENT_ID"])."' and datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0",$err_mess.__LINE__);
			...
			{
				// уменьшаем доход рекламной кампании
				...
				$DB->Update("b_stat_adv_event_day",$arFields,"WHERE ADV_ID='".intval($ar["ADV_ID"])."' and EVENT_ID='".$ar["EVENT_ID"]."' and datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0",$err_mess.__LINE__);
			}
			...
			{
				$arFields = Array("C_EVENTS" => "C_EVENTS-1");
				$DB->Update("b_stat_day_site",$arFields,"WHERE SITE_ID = '".$DB->ForSql($ar["SITE_ID"], 2)."' and datediff(day, $TIME_ENTER_SQL, DATE_STAT)=0", $err_mess.__LINE__);
