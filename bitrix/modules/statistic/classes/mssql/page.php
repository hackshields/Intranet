...
class CPage
{
	function GetDynamicList($URL, &$by, &$order, $arFilter=Array())
	{
		...
		$exit_counter = "SUM(case when D.EXIT_COUNTER>0 then D.EXIT_COUNTER else 0 end)";
		if (is_array($arFilter))
		{
			if (strlen($arFilter["ADV"])>0)
			{
				...
				if ($arFilter["ADV_DATA_TYPE"]=="B")
				{
					...
					$exit_counter = "SUM(case when A.EXIT_COUNTER_BACK>0 then A.EXIT_COUNTER_BACK else 0 end)";
				}
				elseif ($arFilter["ADV_DATA_TYPE"]=="P")
				{
					$counter = "SUM(A.COUNTER)";
					$enter_counter = "SUM(A.ENTER_COUNTER)";
					$exit_counter = "SUM(case when A.EXIT_COUNTER>0 then A.EXIT_COUNTER else 0 end)";
				}
				else
				{
					$counter = "SUM(A.COUNTER + A.COUNTER_BACK)";
					$enter_counter = "SUM(A.ENTER_COUNTER + A.ENTER_COUNTER_BACK)";
					$exit_counter = "SUM(case when A.EXIT_COUNTER>0 then A.EXIT_COUNTER else 0 end + case when A.EXIT_COUNTER_BACK>0 then A.EXIT_COUNTER_BACK else 0 end)";
				}
			}
	...
	function GetList($COUNTER_TYPE, &$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		if (is_array($arFilter))
		{
			if (strlen($arFilter["ADV"])>0)
			{
				...
				{
					$counter = "case when A.".$COUNTER_TYPE.">0 then A.".$COUNTER_TYPE." else 0 end + case when A.".$COUNTER_TYPE."_BACK>0 then A.".$COUNTER_TYPE."_BACK else 0 end";
			...
			foreach ($arFilter as $key => $val)
			{
				...
				switch($key)
				{
					...
					case "DATE2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "V.DATE_STAT < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						break;
					case "SHOW":
					case "DIR":
		...
		$strSql = "
			...
			$where_adv
			$where_counter
			GROUP BY V.URL, V.DIR, V.SITE_ID
			HAVING
				'1'='1'
				$strSqlSearch_h
			$strSqlOrder
