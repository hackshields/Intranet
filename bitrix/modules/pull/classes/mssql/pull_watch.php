...
class CPullWatch extends CAllPullWatch
{
	...
	public static function CheckExpireAgent()
	{
		...
		$strSql = "SELECT count(ID) CNT FROM b_pull_watch WHERE DATE_CREATE < dateadd(MINUTE, -10, getdate())";
		$dbRes = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		if ($arRes = $dbRes->Fetch())
		{
			$strSql = "DELETE TOP (1000) FROM b_pull_watch WHERE DATE_CREATE < dateadd(MINUTE, -10, getdate())";
