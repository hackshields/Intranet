...
?><script type="text/javascript">BX.onCustomEvent('onTMClockRegister', [{<?=CUtil::JSEscape($_REQUEST['clock_id'])?>:'<?=$clock_input_id_1?>',<?=CUtil::JSEscape($_REQUEST['clock_id_1'])?>:'<?=$clock_input_id_2?>'}])</script><?
		...
		elseif($action == "editor")
		{
			...
			require($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/main/interface/admin_lib.php");
		...
		elseif ($action == "upload_attachment")
		{
			if ($_POST["report_id"])
			{
				$dbreport = CTimeManReportFull::GetByID($_POST["report_id"]);
			...
			elseif ($_POST["mode"] == "delete")
			{
				...
				if ($report)
				{
					$report["FILES"] = unserialize($report["FILES"]);
					foreach($report["FILES"] as $key=>$file)
					{
						if($file["fileID"] == $_POST["fileID"])
						{
							unset($report["FILES"][$key]);
							CTimeManReportFull::Update($report["ID"],Array("FILES"=>array_values($report["FILES"])));
							break;
						}
					}
		...
		elseif ($action == "get_attachment") {
			...
			$result["FILE"] = array();
			$result["FILE"] = CFile::GetFileArray($fid);
			...
			set_time_limit(0);
			if (
				strpos($result["FILE"]["CONTENT_TYPE"], "image/")!==false
				&& strpos($result["FILE"]["CONTENT_TYPE"], "html")===false
				&& (
					(
						file_exists($_SERVER["DOCUMENT_ROOT"].$result["FILE"]["SRC"])
						&& CFile::GetImageSize($_SERVER["DOCUMENT_ROOT"].$result["FILE"]["SRC"])
					) || (
						$result["FILE"]["WIDTH"] > 0
						&& $result["FILE"]["HEIGHT"] > 0
					)
				)
			)
			{
				CFile::ViewByUser($result["FILE"], array("content_type" => $result["FILE"]["CONTENT_TYPE"]));
			}
			else
			{
				$ct = strtolower($result["FILE"]["CONTENT_TYPE"]);
				if (strpos($ct, "excel") !== false)
					CFile::ViewByUser($result["FILE"], array("content_type" => "application/vnd.ms-excel"));
				elseif (strpos($ct, "word") !== false)
					CFile::ViewByUser($result["FILE"], array("content_type" => "application/msword"));
				elseif (strpos($ct, "flash") !== false)
					CFile::ViewByUser($result["FILE"], array("content_type" => "application/octet-stream"));
				else
				{
					switch($ct)
					{
						case "text/xml":
							CFile::ViewByUser($result["FILE"], array("content_type" => "application/octet-stream", "force_download" => true));
							break;
						...
						case "pdf":
							...
							break;
						default:
							CFile::ViewByUser($result["FILE"], array("content_type" => "application/octet-stream", "specialchars" => true));
					}
				}
			}
		}
			...
			else
			{
				...
				switch($action)
				{
					...
					case 'user_report_edit':
						...
						if ($arReport["MARK"]=="X" && ($bCanEditAll || $bCanEditUser || CTimeMan::IsAdmin() || $USER_ID == $USER->GetID()))
						{
							if (CTimeManReportFull::Update($ID,Array("REPORT"=>$REPORT,"PLANS"=>$PLAN)))
								$res = Array("success"=>true);
						}

						$GLOBALS['CACHE_MANAGER']->ClearByTag('timeman_report_full_'.$GLOBALS['USER']->GetID());
					...
					case 'admin_report_full':
						...
						elseif($_POST["mode"] && $OBJID>0)
						{
							...
							if ($_POST["object"] == "user")
							{
								...
								if($bCanEdit)
								{
									...
									$GLOBALS['CACHE_MANAGER']->ClearByTag('timeman_report_full_'.$OBJID);
								}
							}
							elseif($_POST["object"] == "dep")
							{
								...
								$GLOBALS['CACHE_MANAGER']->ClearByTag('timeman_report_full');
							}
						}
						if ($bCanReadUser && !$_POST["mode"])
						{
							...
							$arUser['PHOTO'] =
								...
								? CIntranetUtils::InitImage($arUser['PERSONAL_PHOTO'], 32, 0, BX_RESIZE_IMAGE_EXACT)
								: array();
								...
								if(!$res["INFO"]["APPROVER_INFO"])
								{
									...
									if($approver = $dbaprrove->Fetch())
									{
										$approver['PHOTO'] =
											...
											? CIntranetUtils::InitImage($approver['PERSONAL_PHOTO'], 32, 0, BX_RESIZE_IMAGE_EXACT)
											: array();
					...
					case 'save_full_report':
						...
						$GLOBALS['CACHE_MANAGER']->ClearByTag('timeman_report_full_'.$curUser);
					...
					case "add_comment_full_report":
						...
						$arAccessUsers = CTimeMan::GetAccess();
						$ID = intval($_POST["report_id"]);
						$OWNER = intval($_POST["report_owner"]);
						$CURRENT_USER = $USER->GetID();
						$bCanEditAll = in_array('*', $arAccessUsers['WRITE']);
						$bCanReadAll = in_array('*', $arAccessUsers['READ']);
;
						...
						if ($_POST["add_comment"] == "Y" && $bCanAddComment)
						{
							...
							ob_start();
									...
									$res["COMMENTS"] = ob_get_contents();
							ob_end_clean();
							$report = CTimeManReportFull::GetByID($ID)->Fetch();
							if ($report && $report['FORUM_TOPIC_ID'] && CModule::IncludeModule("forum"))
							{
					...
					case "add_comment_entry":
						...
						$OWNER = intval($_REQUEST["report_owner"]);
						$bCanEditAll = in_array('*', $arAccessUsers['WRITE']);
						$bCanReadAll = in_array('*', $arAccessUsers['READ']);
;
						$bCanAddComment = (in_array($OWNER, $arAccessUsers['WRITE']) || CTimeMan::IsAdmin() || $bCanEditAll || $OWNER == $CURRENT_USER);
						if ($bCanAddComment)
						{
							...
							$entry = CTimeManEntry::GetByID($ID)->Fetch();
							if ($entry && $entry['FORUM_TOPIC_ID'] && CModule::IncludeModule("forum"))
							{
					...
					case 'close':
						...
						$arCurrentUser['PHOTO'] =
							...
							? CIntranetUtils::InitImage($arCurrentUser['PERSONAL_PHOTO'], 32, 0, BX_RESIZE_IMAGE_EXACT)
							: array();
						...
						if($arInfo['PLANNER'])
						{
							...
							{
								foreach($arInfo['TASKS'] as $key => $arTask)
								{
									$arInfo['TASKS'][$key]['TIME'] = CTimeman::GetTaskTime(
										array(
											'TASK_ID' => $arTask['ID'],
											'USER_ID' => $USER->GetID(),
											'DATE_START' => $arInfo['INFO']['DATE_START'],
											'DATE_FINISH' => $arInfo['INFO']['DATE_FINISH'],
						...
						if (is_array($res['INFO']['EVENTS']))
						{
							foreach ($res['INFO']['EVENTS'] as $key => $arEvent)
							{
								if ($arEvent['STATUS'] && $arEvent['STATUS'] != 'Y')
									unset($res['INFO']['EVENTS'][$key]);
							}
							$res['INFO']['EVENTS'] = array_values($res['INFO']['EVENTS']);
						}
						...
						if ($arUserSettings['UF_TM_REPORT_REQ'] == 'A')
						{
							...
							if ($arInfo['TASKS_ENABLED'])
							{
								...
								foreach ($arFields['TASKS'] as $key => $arTask)
								{
									$time = isset($arTask['TIME'])
										? $arTask['TIME']
										: CTimeMan::GetTaskTime(
											array(
												'TASK_ID' => $arTask['ID'],
												'USER_ID' => $USER->GetID(),
												'DATE_START' => $arInfo['INFO']['DATE_START'],
									...
									if ($time > 0)
									{
										$arFields['TASKS'][$key]['TIME'] = $time;
										CTimeMan::SetTaskTime(
											array(
												'TASK_ID' => $arTask['ID'],
												'USER_ID' => $USER->GetID(),
												'TIME' => $time
					...
					case 'open':
						...
						$obUser->OpenDay($timestamp, $report);
					break;
					...
					case 'task_status':
						...
						$obUser->TaskStatus(intval($_REQUEST['id']), intval($_REQUEST['status']));
					break;
					case 'task_show':
						if (!CBXFeatures::IsFeatureEnabled('Tasks'))
							break;
						...
						if (CModule::IncludeModule('tasks'))
						{
							...
							$arFilter = array('ID' => $id, 'RESPONSIBLE_ID' => $USER->GetID());
							$dbRes = CTasks::GetByID($id);
							if ($res = $dbRes->Fetch())
							{
								$res['TASKS_ENABLED'] = true;
								$res['URL'] = str_replace(array('#USER_ID#', '#TASK_ID#'), array($USER->GetID(), $res['ID']), COption::GetOptionString('intranet', 'path_task_user_entry', '/company/personal/user/#USER_ID#/tasks/task/view/#TASK_ID#/', $_REQUEST['site_id']));

								$res['PROPS'] = array();

								$arPropsGroup = array();
								$arPropsGroup[] = array('title' => GetMessage('TM_TASK_STATUS'), 'value' => GetMessage('TM_TASK_STATUS_'.$res['STATUS']), 'value1' => FormatDate('^X', MakeTimeStamp($res['STATUS_CHANGED_DATE'])));
								$arPropsGroup[] = array('title' => GetMessage('TM_TASK_CREATED_DATE'), 'value' => FormatDate('X', MakeTimeStamp($res['CREATED_DATE'])));

								if ($res['DEADLINE'])
									$arPropsGroup[] = array('title' => GetMessage('TM_TASK_DEADLINE'), 'value' => FormatDate('^X', MakeTimeStamp($res['DEADLINE'])));
								if ($res['START_DATE_PLAN'])
									$arPropsGroup[] = array('title' => GetMessage('TM_TASK_DATE_START'), 'value' => FormatDate('^X', MakeTimeStamp($res['START_DATE_PLAN'])));
								if ($res['END_DATE_PLAN'])
									$arPropsGroup[] = array('title' => GetMessage('TM_TASK_DATE_END'), 'value' => FormatDate('^X', MakeTimeStamp($res['END_DATE_PLAN'])));

								if ($res['GROUP_ID'] > 0 && CModule::IncludeModule('socialnetwork'))
								{
									if ($arRes = CSocNetGroup::GetByID($res['GROUP_ID']))
									{
										$res['GROUP_NAME'] = $arRes['NAME_FORMATTED'];
										$res['GROUP_URL'] = str_replace('#group_id#', $arRes['ID'], COption::GetOptionString('socialnetwork', 'group_path_template', $_REQUEST['site_id']));
									}
								}

								$res['PROPS'][] = $arPropsGroup;

								$res['DESCRIPTION'] = HTMLToTxt($res['DESCRIPTION']);
								$res['DESCRIPTION'] = preg_replace('/\[ [^\]]* \]/', '', $res['DESCRIPTION']);
								if (strlen($res['DESCRIPTION']) > 150)
								{
									$res['DESCRIPTION'] = CUtil::closetags(substr($res['DESCRIPTION'], 0, 150)).'...';
								}

								$res["ACCOMPLICES"] = $res["AUDITORS"] = array();
								$rsMembers = CTaskMembers::GetList(array(), array("TASK_ID"=>$id));

								$arUserIDs = is_array($res['RESPONSIBLE_ID']) ? $res['RESPONSIBLE_ID'] : array($res['RESPONSIBLE_ID']);
								$arUserIDs[] = $res['CREATED_BY'];
								$arUserIDs[] = $res['CHANGED_BY'];

								while($arMember = $rsMembers->Fetch())
								{
									$arUserIDs[] = $arMember["USER_ID"];
									if ($arMember["TYPE"] == "A")
									{
										$res["ACCOMPLICES"][] = $arMember["USER_ID"];
									}
									elseif ($arMember["TYPE"] == "U")
									{
										$res["AUDITORS"][] = $arMember["USER_ID"];
									}
								}
								...
								{
									$user_url = COption::GetOptionString('intranet', 'path_user', '/company/personal/user/#USER_ID#/', $_REQUEST['site_id']);
									$dbRes = CUser::GetList($by = 'ID', $order = 'ASC', array('ID' => implode('|', $arUserIDs)));

									$arUsers = array();
									while ($arUser = $dbRes->Fetch())
									{
										$link = str_replace(array('#ID#', '#USER_ID#'), $arUser['ID'], $user_url);
									...
									$arPropsGroup = array();
									$arUserFields = array('CREATED_BY', 'RESPONSIBLE_ID', 'AUDITORS', 'ACCOMPLICES');
									foreach ($arUserFields as $fld)
									{
										if ($res[$fld])
										{
											$r = array(
												'title' => GetMessage('TM_TASK_'.$fld.(count($res[$fld])>1?'_PL':'')),
												'value' => array(),
												'value_href' => array()
											);

											$v = is_array($res[$fld]) ? $res[$fld] : array($res[$fld]);
											foreach ($v as $user_id)
											{
												$r['value'][] = $arUsers[$user_id]['NAME'];
												$r['value_href'][] = $arUsers[$user_id]['URL'];
											}
								...
								{
									$arFile = CFile::GetFileArray($arFile["FILE_ID"]);
								...
								$arActions = $GLOBALS['BX_TIMEMAN_TASKS_MIGRATION_RULES'][$res['STATUS']];
								if ($res['RESPONSIBLE_ID'] == $USER->GetID() && is_array($arActions))
								{
									$res['BUTTONS'] = array();
									foreach ($arActions as $action)
									{
										$res['BUTTONS'][] = array($action, GetMessage('TM_TASK_SET_STATUS_'.($res['STATUS'] == 1 && $action == 2 ? '2_1' : $action)));
									}
								}
							}
							else
					...
					case 'admin_data':
						...
						$res['USERS'] = array_values($res['USERS']);
