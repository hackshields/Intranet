...
class CTimeManNotify
{
	...
	public static function SendMessage($ENTRY_ID, $SEND_TYPE = false)
	{
		...
		$dbEntry = CTimeManEntry::GetList(array(), array('ID' => $ENTRY_ID));
		if (CModule::IncludeModule("socialnetwork") && ($arEntry = $dbEntry->Fetch()))
		{
			...
			$date = (strlen($arEntry["DATE_START"]) > 0?
				(MakeTimeStamp($arEntry["DATE_START"], CSite::GetDateFormat("FULL", $site_id)) > time()+CTimeZone::GetOffset()?
					$DB->CharToDateFunction($arEntry["DATE_START"], "FULL", $site_id) :
			...
			$arSoFields = Array(
				...
				"TITLE" => GetMessage("TIMEMAN_NOTIFY_TITLE").$date_text,
				"MESSAGE" => '',
				"TEXT_MESSAGE" => '',
				"CALLBACK_FUNC" => false,
				"SOURCE_ID" => $ENTRY_ID,
	...
	public static function FormatEvent($arFields, $arParams, $bMail = false)
	{
		...
		if(!$bMail)
		{
			...
			ob_start();
			$GLOBALS['APPLICATION']->IncludeComponent('bitrix:timeman.livefeed.workday', ($arParams["MOBILE"] == "Y" ? 'mobile' : ''), array(
				'USER' => $arUser,
	...
	public static function FormatComment($arFields, $arParams, $bMail = false, $arLog = array())
	{
		...
		if ($bMail)
		{
			$reportURL = COption::GetOptionString("timeman","TIMEMAN_REPORT_PATH","/company/timeman.php");
			if (strlen($reportURL) == 0)
				$reportURL = "/company/timeman.php";
			...
			// if (strlen($reportURL) > 0)
				// $arResult["EVENT_FORMATTED"]["URL"] = $reportURL."#user_id=".$arLog["USER_ID"]."&report=".$arLog["SOURCE_ID"];
		}
		else
		{
			...
			$arAllow = array("HTML" => "Y", "ANCHOR" => "Y", "BIU" => "Y", "IMG" => "Y", "QUOTE" => "Y", "CODE" => "Y", "FONT" => "Y", "LIST" => "Y", "SMILES" => "Y", "NL2BR" => "N", "VIDEO" => "N", "LOG_VIDEO" => "N");
