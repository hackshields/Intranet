...
class CTimeManUser
{
	...
	protected static $LAST_ENTRY = null;
...
//			'DATE_START' => $GLOBALS['DB']->FormatDate(ConvertTimeStamp(($timestamp ? $timestamp : time()) + CTimeZone::GetOffset(), 'FULL'), FORMAT_DATETIME, "DD.MM.YYYY HH:MI:SS"),
	...
	public function CloseDay($timestamp = false, $report = '', $bFieldsOnly = false)
	{
		...
		if (($last_entry = $this->_GetLastData(true)) && (!$last_entry['DATE_FINISH'] || $last_entry['PAUSED'] == 'Y'))
		{
			...
			{
				$timestamp = CTimeMan::ConvertShortTS(
					$timestamp - CTimeZone::GetOffset(),
					ConvertTimeStamp(MakeTimeStamp($last_entry['DATE_START'], FORMAT_DATETIME), 'SHORT')
				);
			}
...
//				'DATE_FINISH' => $GLOBALS['DB']->FormatDate(ConvertTimeStamp(($timestamp ? $timestamp : time()) + CTimeZone::GetOffset(), 'FULL'), FORMAT_DATETIME, "DD.MM.YYYY HH:MI:SS"),
	...
	{
		return (is_array($this->_GetLastData()) && !CTimeManUser::$LAST_ENTRY['DATE_FINISH']);
	}

	// check if day is paused
	public function isDayPaused()
	{
		return (is_array($this->_GetLastData()) && !$this->isDayOpen() && CTimeManUser::$LAST_ENTRY['PAUSED'] === 'Y');
	}

	public function getDayStartOffset($entry, $bTs)
	{
	...
	public function isDayOpenedToday()
	{
		...
		$ts_start = !empty(CTimeManUser::$LAST_ENTRY['DATE_START']) ? (MakeTimeStamp(CTimeManUser::$LAST_ENTRY['DATE_START']) - CTimeZone::GetOffset()) : time();
		$ts_start_day = MakeTimeStamp(ConvertTimeStamp($ts_start, 'SHORT'));
		...
		$timezone_diff = CTimeManUser::$LAST_ENTRY['TIME_START'] - $time_start;
		...
		if ($last_entry = $this->_GetLastData())
		{
			...
			if ($this->isDayOpenedToday())
			{
				if (
					$bSkipCheck
					|| (
						$last_entry['USER_ID'] == $last_entry['MODIFIED_BY']
	...
	protected function __GetSettings($arNeededSettings, $bPersonal = false)
	{
		...
		if(!is_array($this->SETTINGS[$cat]))
		{
			...
			$cache_dir = '/timeman/structure_settings/u'.$this->USER_ID.'_'.$cat;
			$cache_id = 'timeman|structure_settings|u'.$this->USER_ID.'_'.$cat;

			$obCache = new CPHPCache();

			if (CACHED_timeman_settings !== false
				&& $obCache->InitCache(CACHED_timeman_settings, $cache_id, $cache_dir))
			{
				$this->SETTINGS[$cat] = $obCache->GetVars();
			}
			else
			{
				if (CACHED_timeman_settings !== false)
				{
					...
					$CACHE_MANAGER->RegisterTag("USER_CARD_".intval($this->USER_ID / 100));

					if(!$bPersonal)
					{
						$CACHE_MANAGER->RegisterTag("iblock_id_".COption::GetOptionInt('intranet', 'iblock_structure', false));
					}
				}
				...
				if (CACHED_timeman_settings !== false)
				{
					...
					$obCache->EndDataCache($this->SETTINGS[$cat]);
				}
			}
		}
...
// if user is not attached to company structure tm can be allowed only in his own profile
			...
			$arRes['UF_TM_REPORT_REQ'] = $arRes['UF_TM_REPORT_REQ']
				...
				: COption::GetOptionInt('timeman', 'workday_report_required', 'A');
	...
	protected function _GetLastData($clear = false)
	{
		...
		else if (!CTimeManUser::$LAST_ENTRY)
		{
