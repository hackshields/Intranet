...
{
	SITE_ID = site_id;
	window.BXTIMEMAN = new BX.CTimeMan(id, data);
}
...
BX.CTimeManWindow.prototype.Create = function(DATA)
{
	...
	if (this.bindOptions.mode == 'popup')
	{
		if (!this.POPUP)
		{
			this.POPUP = new BX.PopupWindow('timeman_main', this.bindOptions.node, this.bindOptions.popupOptions || {
				autoHide: true,
				bindOptions : {
	...
	{
		this.LAYOUT.appendChild(this.CreateSocServRow(DATA));
	}
	...
	if (DATA.INFO)
	{
		...
		if(!!DATA.SOCSERV_ENABLED)
		{
			BX.addCustomEvent(this.PARENT, 'onTimeManDataRecieved', BX.delegate(this.CreateSocServRow, this));
		}
...
BX.CTimeManWindow.prototype.CreateSocServRow = function(DATA)
{
	if(!this.SOCSERV_ROW)
	{
		var row = BX.create('DIV', {
			props: {className: 'bx-timeman-socserv-div'},
			children: [
				BX.create('SPAN', {
					children: [
						BX.create('INPUT', {
							props: {
								type: 'checkbox',
								id: 'ss-send-to-socserv',
								defaultChecked: !!DATA.SOCSERV_ENABLED_USER
							},
							events: {
								click: function()
								{
									BX.ajax.post(
										'/bitrix/tools/oauth/socserv.ajax.php?action=saveuserdata&site_id=' + BX.message('SITE_ID') + '&sessid=' + BX.bitrix_sessid(),
										{ENABLED:row.firstChild.firstChild.checked?'Y':'N'}
									);
								}
							}
						}),
						BX.create('LABEL', {
							props: {
								htmlFor: 'ss-send-to-socserv'
							},
							text: BX.message('JS_CORE_SS_SEND_TO_SOCSERV')
						})
					]
				}),
				BX.create('SPAN', {
		...
		this.SOCSERV_ROW = row;
	}


	if(!!DATA.SOCSERV_ENABLED_USER)
	{
		var checkbox = this.SOCSERV_ROW.firstChild.firstChild;
		checkbox.checked = !!DATA.SOCSERV_ENABLED_USER;
	}

	this.SOCSERV_ROW.style.display = !!DATA.FULL ? 'block' : 'none';

	return this.SOCSERV_ROW;
}
...
BX.CTimeManTimeSelector = function(parent, params)
{
	...
	params.popup_buttons = [
		new BX.PopupWindowButton({
			text : parent.MAIN_BUTTON.innerText || parent.MAIN_BUTTON.textContent,
...
BX.CTimeManTimeSelector.prototype.setNode = function(node)
{
	...
	this.params.popup_buttons[0].setName(node.innerText || node.textContent);
}
			...
			BX.create('DIV', {
				...
				children: [
					(this.REPORT_BTN = BX.create('SPAN', {
						props: {className: 'popup-window-button tm-popup-report-button'},
						events: {click: BX.proxy(this._btnClick, this)},
						html: this._btnHTML(BX.message('JS_CORE_TM_B_SAVE'))
					}))
				]
			})
		]
...
BX.CTimeManReport.prototype._btnHTML = function(caption)
{
	return '<span class="popup-window-button-left"></span><span class="popup-window-button-text">' + caption + '</span><span class="popup-window-button-right"></span>';
}
...
BX.CTimeManReport.prototype.Save = function()
{
	...
	this.REPORT_BTN.innerHTML = this._btnHTML(BX.message('JS_CORE_TM_B_SAVING'));
...
BX.CTimeManReport.prototype.SaveFinished = function(data)
{
	...
	this.REPORT_BTN.innerHTML = this._btnHTML(BX.message('JS_CORE_TM_B_SAVE'));
...
BX.CTimeManTabControl.prototype.createTab = function(id)
{
	this.arTabs[id].tab = this.HEAD.appendChild(BX.create('SPAN', {
		...
		html: '<span class="tm-tab-left"></span><span class="tm-tab-text">' + this.arTabs[id].title + '</span><span class="tm-tab-right"></span>'
	}));
...
BX.CTimeManTabEditorControl.prototype.addTab = function(params)
{
	...
	BX.bind(this.TABCONTROL.arTabs[tab.ID].tab, "click",BX.proxy(function()
	{
		...
		this.tabs[tab.ID].LHEDIV.appendChild(BX("bxlhe_frame_"+this.editor.id));
		this.editor.SetContent(this.tabs[tab.ID].CONTENT);
		this.editor.CreateFrame();
		this.editor.SetEditorContent(this.tabs[tab.ID].CONTENT);
	...
	if (this.files && this.files.length>0)
	{
		...
		for(i=0;i<files.length;i++)
		{
			filelist.appendChild(
				...
				children : [
					BX.create("a", {
						props : { href : "/bitrix/tools/timeman.php?action=get_attachment&fid="+files[i].fileID+"&sessid="+BX.bitrix_sessid(), target : "_blank", className : "upload-file-name"},
						text : files[i].name
					}),
			...
			BX.create("DIV",{
				...
				children:[
					...
					?BX.create("DIV",{
						...
						children:[
							BX.create("SPAN",{
								props:{className:"tm-reject-button webform-button-upload"},
								children:[
									BX.create("SPAN",{
										props:{className:"tm-reject-l"}
									}),
									BX.create("SPAN",{
										props:{className:"tm-reject-c"},
										children:[
											BX.create("SPAN",{
												props:{className:"tm-reject-icon"}
											}),
											BX.message('JS_CORE_TM_UPLOAD_FILES')
										]
									}),
									BX.create("SPAN",{
										props:{className:"tm-reject-r"}
									})
								]
							}),
...
BX.CTimeManUploadForm.prototype.RefreshUpload = function(files,uniqueID)
{
	for(i = 0; i < files.length; i++)
	{
		...
		if (files[i].fileID)
		{
			...
			BX.adjust(elem.firstChild, {props : {href : "/bitrix/tools/timeman.php?action=get_attachment&fid="+files[i].fileID+"&sessid="+BX.bitrix_sessid()}});
...
{
	if (this.comment_text.value.length<=3)
		return;
	var data = {
