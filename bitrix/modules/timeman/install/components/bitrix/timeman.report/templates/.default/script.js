...
JCTimeManReport.prototype.loadAbsence = function()
{
	...
	var url =  '/bitrix/components/bitrix/intranet.absence.calendar/ajax.php?MODE=GET&TS_START=' + parseInt(TS_START.valueOf()/1000) + '&TS_FINISH=' + (parseInt(TS_FINISH.valueOf()/1000)-1) + '&SHORT_EVENTS=N&USERS_ALL=N&current_data_id=1111&site_id=' + this.SETTINGS.SITE_ID + '&sessid=' + BX.bitrix_sessid();
	...
	for (var i=0; i<data.length; i++)
	{
		...
		if (!!rows && rows.length > 0)
		{
			for (var q=0,l=rows.length;q<l;q++)
			{
				...
				for (j=0; j<data[i].DATA.length; j++)
				{
					var date_start = new Date(data[i].DATA[j].DATE_ACTIVE_FROM * 1000),
						date_finish = new Date(data[i].DATA[j].DATE_ACTIVE_TO * 1000);
					...
					while (date_start.getMonth() == cur_m && date_start.valueOf() <= date_finish.valueOf())
					{
						...
						else if (cell)
						{
							...
							cell.innerHTML = '<span style="width: ' + (75*cellcnt - 8) + 'px; display: inline-block; white-space: nowrap; overflow: hidden; ">' + BX.util.htmlspecialchars(data[i].DATA[j].NAME) + '</span>';
					...
					{
						cell.colSpan = cellcnt;
						cell.innerHTML = '<span style="width: ' + (75*cellcnt - 8) + 'px; display: inline-block; white-space: nowrap; overflow: hidden; ">' + BX.util.htmlspecialchars(data[i].DATA[j].NAME) + '</span><div class="bx-tm-view-arrival">&nbsp;</div><div class="bx-tm-view-departure">&nbsp;</div>';
						cell.style.textAlign = 'left';
						cell.style.paddingRight = '0px';
