		...
		extends CDavGroupdavClient
	{
		...
		private function GetICalContent(array $event, $siteId)
		{
			$arICalEvent = array(
				...
				"SUMMARY" => $event["NAME"],
				"DTSTART" => array(
					"VALUE" => date("Ymd\\THis", $event["DT_FROM_TS"]),
					"PARAMETERS" => array("TZID" => CDav::GetTimezoneId($siteId))
				),
				"DTEND" => array(
					"VALUE" => date("Ymd\\THis", $event["DT_TO_TS"]),
					"PARAMETERS" => array("TZID" => CDav::GetTimezoneId($siteId))
				),
			);
			...
			if (isset($event["RRULE"]) && is_array($event["RRULE"]))
			{
				...
				if ($event["DT_SKIP_TIME"] == 'Y')
				{
					...
					$arICalEvent["DTEND"] = array(
						"VALUE" => date("Ymd", $event["DT_FROM_TS"] + $event["PROPERTY_EVENT_LENGTH"]),
						"PARAMETERS" => array("TZID" => CDav::GetTimezoneId($siteId))
					);
				}
				else
				{
					$arICalEvent["DTEND"] = array(
						"VALUE" => date("Ymd\\THis", $event["DT_FROM_TS"] + $event["PROPERTY_EVENT_LENGTH"]),
						"PARAMETERS" => array("TZID" => CDav::GetTimezoneId($siteId))
					);
				}
				...
				foreach ($arUserCalendars as $userCalendar)
				{
					...
					foreach ($arCalendarItemsList as $value)
					{
						...
						$arModifyEventArray = array(
							...
							"DT_FROM_TS" => $value["calendar-data"]["DT_FROM_TS"],
							"DT_TO_TS" => $value["calendar-data"]["DT_TO_TS"],
