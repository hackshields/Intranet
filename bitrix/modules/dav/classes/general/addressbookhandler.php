	...
	extends CDavGroupdavHandler
{
	...
	private function GetVCardContent(array $contact)
	{
		$arVCardContact = array(
			"TYPE" => "VCARD",
			"VERSION" => "3.0",
			"N" => $contact["LAST_NAME"].";".$contact["NAME"].";;;",
			"FN" => $contact["NAME"]." ".$contact["LAST_NAME"],
			"EMAIL" => array(
				"VALUE" => $contact["EMAIL"],
		...
		{
			$arTempFile = CFile::MakeFileArray($contact["PERSONAL_PHOTO"]);
			if ($arTempFile && isset($arTempFile["tmp_name"]))
			{
				$cnt = file_get_contents($arTempFile["tmp_name"]);
				if (!empty($cnt))
				{
					$arImageTypes = array(
						IMAGETYPE_JPEG => 'JPEG',
					...
					$imageType = "JPEG";
					if ($imageInfo = @getimagesize($arTempFile["tmp_name"]) && isset($arImageTypes[$imageInfo[2]]))
