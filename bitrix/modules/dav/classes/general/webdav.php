...
abstract class CDavWebDav
{
	...
	public function __construct($request)
	{
		...
		$this->response = new CDavResponse($this->request->GetParameter('REQUEST_URI'), $cs);
	}
	...
	*/
	public function ProcessRequest()
	{
		...
		CDav::Report("CDavWebDav::ProcessRequest", "method", $method);
	...
	*/
	...
	protected function CheckAuthWrapper()
	{
		...
		{
			$authType = $this->request->GetParameter("AUTH_TYPE");
			$phpAuthUser = $this->request->GetParameter("PHP_AUTH_USER");
			$phpAuthPw = $this->request->GetParameter("PHP_AUTH_PW");

			$authorization = $this->request->GetParameter("Authorization");
			if (is_null($authorization))
				$authorization = $this->request->GetParameter("REMOTE_USER");
			if (is_null($authorization))
				$authorization = $this->request->GetParameter("REDIRECT_REMOTE_USER");
	...
	*/
	...
	{
		$httpIf = $this->request->GetParameter("HTTP_IF");
		if ($httpIf != null)
		{
	...
	protected function LockDiscovery($path)
	{
		...
		if (is_array($lock) && count($lock))
		{
			...
			if ($this->request->IsRedundantNamespaceDeclarationsRequired())
			{
				$activelocks.= "
					<activelock>
		...
		foreach ($arResources as $resource)
		{
			...
			foreach ($arRequestedProps as &$requestedProp)
			{
				...
				if (!$bFound)
				{
					...
					elseif ($this->request->GetParameter('HTTP_BRIEF') != 't')
					{
						$arPropStat["404"][] = CDavResource::MakeProp($requestedProp["tagname"], "", $requestedProp["xmlns"]);
					}
				}

				if (!empty($requestedProp["xmlns"]) && ($bFound || $this->request->GetParameter('HTTP_BRIEF') != 't'))
				{
			...
			$href = $this->UrlEncode(rtrim($this->request->GetBaseUri(), '/')."/".ltrim($resource->GetPath(), '/'));
	...
	{
		$response->SetHttpStatus('501 Not Implemented');
	}
	...
	protected function PUTWrapper($handler = "PUT")
	{
		...
		CDav::Report("CDavWebDav::PUTWrapper", "arContentParameters", $arContentParameters);
	...
	private function CopyMove($what)
	{
		...
		extract(parse_url($httpDestination));
		...
		$httpHeaderHost = preg_replace("/:80$/", "", $request->GetParameter("HTTP_HOST"));
	...
	protected function LOCKWrapper()
	{
		...
		if (is_null($contentLength) && !is_null($httpIf))
		{
			if (!$this->CheckLockStatus($request->GetPath()))
			{
				$response->SetHttpStatus("423 Locked");
		...
		{
			// deal with getting xmlDocument!
			throw new Exception("TODO: xmlDocument");

			if (!$this->CheckLockStatus($request->GetPath(), false/*true if shared in xmlDocument*/))
			{
				$response->SetHttpStatus("423 Locked");
				return;
			}

			$locktoken = uniqid("bxlocktoken:", true);
			$updateLock = false;
			$owner = "unknown"; /*in xmlDocument*/
			$scope = "exclusive"; /*shared in xmlDocument*/
			$type = "write"; /*write in xmlDocument*/
		}
