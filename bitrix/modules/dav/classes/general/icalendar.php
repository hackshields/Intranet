...
class CDavICalendar
{
	...
	public function __construct($cal, $siteId = null)
	{
		...
		else
		{
			...
			$tz = CDavICalendarTimeZone::GetTimezone(CDav::GetTimezoneId($siteId));
			if (!empty($tz))
			{
...
class CDavICalendarProperty
{
	...
	public function Render()
	{
		...
		{
			if (isset($this->arParameters['VALUE']) && $this->arParameters['VALUE'] == 'DATE')
				$str = substr($str, 0, 8);
		}
...
//		if (!isset($this->arParameters['ENCODING']) || $this->arParameters['ENCODING'] != 'BASE64')
//		{
...
//		}
	...
	private function RenderParameters()
	{
		...
		{
			$str = preg_replace("/([;:\"])/", '\\\\$1', $val);
			$result .= sprintf(";%s=%s", $key, $str);
		}
		return $result;
	}
