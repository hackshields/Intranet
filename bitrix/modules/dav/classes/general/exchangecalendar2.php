...
if (!class_exists("CDavExchangeCalendar"))
{
		...
		extends CDavExchangeClient
	{
		...
		private function FormatFieldsArray($arFields)
		{
			...
			if (isset($arFields['RRULE']))
			{
				...
				if (is_array($rrule) && in_array($rrule["FREQ"], array("DAILY", "WEEKLY", "MONTHLY", "YEARLY")))
				{
					...
					$arFields["RECURRING_TYPE"] = $ar[$rrule["FREQ"]];
					...
					if ($rrule["FREQ"] == "WEEKLY")
					{
						...
						if (empty($bydays))
							$bydays = array("MO");
						static $arWeekDayMap = array('SU' => "Sunday", 'MO' => "Monday", 'TU' => "Tuesday", 'WE' => "Wednesday", 'TH' => "Thursday", 'FR' => "Friday", 'SA' => "Saturday");
		...
		private function ConvertCalendarToArray($calendarItem)
		{
			...
			//$bAllDay = false;
			//$arAllDay = $calendarItem->GetPath("/CalendarItem/IsAllDayEvent");
			//if (count($arAllDay) > 0)
			//	$bAllDay = $arAllDay[0]->GetContent();
