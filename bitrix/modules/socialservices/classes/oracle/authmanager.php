	...
	extends CSocServAuth
{
	...
	function Add($arFields)
	{
		...
		$ID = IntVal($DB->NextID("SQ_B_SOCIALSERVICES_USER"));

		$strSql =
			"INSERT INTO b_socialservices_user(ID, ".$arInsert[0].") ".
	...
	function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		if (count($arSelectFields) <= 0)
			$arSelectFields = array("ID", "LOGIN", "NAME", "LAST_NAME", "EMAIL", "PERSONAL_PHOTO", "EXTERNAL_AUTH_ID", "USER_ID", "XML_ID", "CAN_DELETE", "PERSONAL_WWW", "PERMISSIONS", "OATOKEN", "OASECRET", "REFRESH_TOKEN", "ACTIVE", "SEND_ACTIVITY");
...
class CSocServMessage extends CSocServAllMessage
{
	function CleanUp()
	{
		...
		$strSql = "DELETE FROM b_socialservices_message WHERE INSERT_DATE < SYSDATE-4";
	...
	function Add($arFields)
	{
		...
		$ID = IntVal($DB->NextID("SQ_B_SOCIALSERVICES_MESSAGE"));

		$strSql =
			"INSERT INTO B_SOCIALSERVICES_MESSAGE(ID, ".$arInsert[0].") ".
				"VALUES(".$ID.", ".$arInsert[1].")";
		$res = $DB->Query($strSql, true, "File: ".__FILE__."<br>Line: ".__LINE__);
