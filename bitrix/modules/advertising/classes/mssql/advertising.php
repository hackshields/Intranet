...
class CAdvContract extends CAdvContract_all
{
	...
	function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered, $CHECK_RIGHTS="Y")
	{
		...
		$lamp = "
			CASE
				WHEN
					(C.DATE_SHOW_FROM<=GETDATE() or C.DATE_SHOW_FROM is null or len(C.DATE_SHOW_FROM)<=0) and
					(C.DATE_SHOW_TO>=GETDATE() or C.DATE_SHOW_TO is null or len(C.DATE_SHOW_TO)<=0) and
					(isnull(C.MAX_SHOW_COUNT,0)>isnull(C.SHOW_COUNT,0) or isnull(C.MAX_SHOW_COUNT,0)=0) and
					(isnull(C.MAX_CLICK_COUNT,0)>isnull(C.CLICK_COUNT,0) or isnull(C.MAX_CLICK_COUNT,0)=0) and
					(isnull(C.MAX_VISITOR_COUNT,0)>isnull(C.VISITOR_COUNT,0) or isnull(C.MAX_VISITOR_COUNT,0)=0) and
					(C.ACTIVE='Y')
				THEN 'green'
				ELSE 'red'
			END
				...
				for ($i=0; $i<count($filter_keys); $i++)
				{
					...
					switch($key)
					{
						...
						case "DATE_MODIFY_1":
							...
							break;
						case "DATE_MODIFY_2":
							$arSqlSearch[] = "C.DATE_MODIFY < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
							break;
						case "NAME":
						case "DESCRIPTION":
						case "ADMIN_COMMENTS":
						...
						case "CTR_1":
							$arSqlSearch[] = "
								(
								CASE
									WHEN C.SHOW_COUNT<=0 THEN 0
									ELSE round((C.CLICK_COUNT*100)/C.SHOW_COUNT,2)
								END
								) >= '".DoubleVal(str_replace(',', '.', $val))."'";
							break;
						case "CTR_2":
							$arSqlSearch[] = "
								(
		...
		elseif ($by == "s_sort")				$strSqlOrder = "ORDER BY isnull(C.SORT,0)";
		...
		elseif ($by == "s_max_show_count")		$strSqlOrder = "ORDER BY isnull(C.MAX_SHOW_COUNT,0)";
		elseif ($by == "s_click_count")			$strSqlOrder = "ORDER BY C.CLICK_COUNT";
		elseif ($by == "s_max_click_count")		$strSqlOrder = "ORDER BY isnull(C.MAX_CLICK_COUNT,0)";
		elseif ($by == "s_visitor_count")		$strSqlOrder = "ORDER BY C.VISITOR_COUNT";
		elseif ($by == "s_max_visitor_count")	$strSqlOrder = "ORDER BY isnull(C.MAX_VISITOR_COUNT,0)";
		else
		{
			$by = "s_sort";
			$strSqlOrder = "ORDER BY isnull(C.SORT,0)";
		}
		if ($order!="asc")
		{
			$strSqlOrder .= " desc ";
...
class CAdvBanner extends CAdvBanner_all
{
	...
	function getCTRSQL()
	{
		return 'case when SUM(D.SHOW_COUNT)>0 then round((SUM(D.CLICK_COUNT)*100)/SUM(D.SHOW_COUNT),2) else 0 end CTR';
	}
	...
	function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered, $CHECK_RIGHTS="Y")
	{
		...
		if ($DONT_USE_CONTRACT == "Y")
		{
			...
			CASE
				WHEN
					(
					(B.DATE_SHOW_FROM<=GETDATE() or B.DATE_SHOW_FROM is null or len(B.DATE_SHOW_FROM)<=0) and
					(B.DATE_SHOW_TO>=GETDATE() or B.DATE_SHOW_TO is null or len(B.DATE_SHOW_TO)<=0) and
					(isnull(B.MAX_SHOW_COUNT,0)>isnull(B.SHOW_COUNT,0) or isnull(B.MAX_SHOW_COUNT,0)=0) and
					(isnull(B.MAX_CLICK_COUNT,0)>isnull(B.CLICK_COUNT,0) or isnull(B.MAX_CLICK_COUNT,0)=0) and
					(isnull(B.MAX_VISITOR_COUNT,0)>isnull(B.VISITOR_COUNT,0) or isnull(B.MAX_VISITOR_COUNT,0)=0) and
					(B.ACTIVE='Y') and
				...
				for ($i=0; $i<count($filter_keys); $i++)
				{
					...
					switch($key)
					{
						...
						case "DATE_MODIFY_1":
							...
							break;
						case "DATE_MODIFY_2":
							$arSqlSearch[] = "B.DATE_MODIFY < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
							break;
						case "DATE_CREATE_1":
							$arSqlSearch[] = "B.DATE_CREATE >= ".$DB->CharToDateFunction($val, "SHORT");
							break;
						case "DATE_CREATE_2":
							$arSqlSearch[] = "B.DATE_CREATE < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
							break;
						case "DATE_SHOW_FROM_1":
							$arSqlSearch[] = "B.DATE_SHOW_FROM >= ".$DB->CharToDateFunction($val, "SHORT");
							break;
						case "DATE_SHOW_FROM_2":
							$arSqlSearch[] = "B.DATE_SHOW_FROM < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
							break;
						case "DATE_SHOW_TO_1":
							$arSqlSearch[] = "B.DATE_SHOW_TO >= ".$DB->CharToDateFunction($val, "SHORT");
							break;
						case "DATE_SHOW_TO_2":
							$arSqlSearch[] = "B.DATE_SHOW_TO < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
							break;
						case "ACTIVE":
						case "FIX_SHOW":
							...
							break;
						case "WEIGHT_1":
							$arSqlSearch[] = "isnull(B.WEIGHT,0)>='".intval($val)."'";
							break;
						case "WEIGHT_2":
							$arSqlSearch[] = "isnull(B.WEIGHT,0)<='".intval($val)."'";
							break;
						case "MAX_VISITOR_COUNT_1":
							$arSqlSearch[] = "isnull(B.MAX_VISITOR_COUNT,0)>='".intval($val)."'";
							break;
						case "MAX_VISITOR_COUNT_2":
							$arSqlSearch[] = "isnull(B.MAX_VISITOR_COUNT,0)<='".intval($val)."'";
							break;
						case "VISITOR_COUNT_1":
							$arSqlSearch[] = "isnull(B.VISITOR_COUNT,0)>='".intval($val)."'";
							break;
						case "VISITOR_COUNT_2":
							$arSqlSearch[] = "isnull(B.VISITOR_COUNT,0)<='".intval($val)."'";
							break;
						case "MAX_SHOW_COUNT_1":
							$arSqlSearch[] = "isnull(B.MAX_SHOW_COUNT,0)>='".intval($val)."'";
							break;
						case "MAX_SHOW_COUNT_2":
							$arSqlSearch[] = "isnull(B.MAX_SHOW_COUNT,0)<='".intval($val)."'";
							break;
						case "SHOW_COUNT_1":
							$arSqlSearch[] = "isnull(B.SHOW_COUNT,0)>='".intval($val)."'";
							break;
						case "SHOW_COUNT_2":
							$arSqlSearch[] = "isnull(B.SHOW_COUNT,0)<='".intval($val)."'";
							break;
						case "MAX_CLICK_COUNT_1":
							$arSqlSearch[] = "isnull(B.MAX_CLICK_COUNT,0)>='".intval($val)."'";
							break;
						case "MAX_CLICK_COUNT_2":
							$arSqlSearch[] = "isnull(B.MAX_CLICK_COUNT,0)<='".intval($val)."'";
							break;
						case "CLICK_COUNT_1":
							$arSqlSearch[] = "isnull(B.CLICK_COUNT,0)>='".intval($val)."'";
							break;
						case "CLICK_COUNT_2":
							$arSqlSearch[] = "isnull(B.CLICK_COUNT,0)<='".intval($val)."'";
							break;
						case "CTR_1":
							$arSqlSearch[] = "
								(
								CASE
									WHEN B.SHOW_COUNT<=0 THEN 0
									ELSE round((B.CLICK_COUNT*100)/B.SHOW_COUNT,2)
								END
								) >= '".DoubleVal(str_replace(',', '.', $val))."'";
							break;
						case "CTR_2":
		...
		elseif ($by == "s_max_visitor_count") $strSqlOrder = " ORDER BY isnull(B.MAX_VISITOR_COUNT,0) ";
		elseif ($by == "s_show_count") $strSqlOrder = " ORDER BY B.SHOW_COUNT ";
		elseif ($by == "s_max_show_count") $strSqlOrder = " ORDER BY isnull(B.MAX_SHOW_COUNT,0) ";
		...
		elseif ($by == "s_max_click_count") $strSqlOrder = " ORDER BY isnull(B.MAX_CLICK_COUNT,0) ";
		...
		if ($DONT_USE_CONTRACT == "N")
		{
			$strSql = "
				...
				INNER JOIN b_adv_banner B ON (
					...
					and	(isnull(B.MAX_SHOW_COUNT,0)>isnull(B.SHOW_COUNT,0) or isnull(B.MAX_SHOW_COUNT,0)=0)
					and (isnull(B.MAX_CLICK_COUNT,0)>isnull(B.CLICK_COUNT,0) or isnull(B.MAX_CLICK_COUNT,0)=0)
					and (isnull(B.MAX_VISITOR_COUNT,0)>isnull(B.VISITOR_COUNT,0) or isnull(B.MAX_VISITOR_COUNT,0)=0)
					and (B.DATE_SHOW_FROM<=getdate() or B.DATE_SHOW_FROM is null)
					and (B.DATE_SHOW_TO>=getdate() or B.DATE_SHOW_TO is null))
				...
				INNER JOIN b_adv_contract C ON (
						C.ID = B.CONTRACT_ID
					and C.ACTIVE='Y'
					and	(isnull(C.MAX_SHOW_COUNT,0)>isnull(C.SHOW_COUNT,0) or isnull(C.MAX_SHOW_COUNT,0)=0)
					and (isnull(C.MAX_CLICK_COUNT,0)>isnull(C.CLICK_COUNT,0) or isnull(C.MAX_CLICK_COUNT,0)=0)
					and (isnull(C.MAX_VISITOR_COUNT,0)>isnull(C.VISITOR_COUNT,0) or isnull(C.MAX_VISITOR_COUNT,0)=0)
					and (C.DATE_SHOW_FROM<=getdate() or C.DATE_SHOW_FROM is null)
					and (C.DATE_SHOW_TO>=getdate() or C.DATE_SHOW_TO is null))
				...
				LEFT JOIN b_adv_banner_2_country BC ON BC.BANNER_ID = B.ID AND (
					(
						(B.STAT_TYPE is null OR len(B.STAT_TYPE)=0 OR B.STAT_TYPE='COUNTRY')
				...
				and (BP1.ID is null or '".$DB->ForSQL($url)."' like BP1.PAGE+'%')
				and (CP1.ID is null or '".$DB->ForSQL($url)."' like CP1.PAGE+'%')
				and not exists (
					SELECT
						'x'
					FROM
					...
					and '".$DB->ForSQL($url)."' like BP2.PAGE+'%'
					)
				and not exists (
					SELECT
						'x'
					...
					and '".$DB->ForSQL($url)."' like CP2.PAGE+'%'
					)
	...
	function CleanUpDynamics()
	{
		...
		$strSql = "DELETE FROM b_adv_banner_2_day WHERE DATE_STAT <= dateadd(day, -$DAYS, getdate())";
	...
	function GetDynamicList_SQL($strSqlSearch)
	{
		...
		$strSql = "
			SELECT
				...
				DATEPART(dd, D.DATE_STAT)								DAY,
				DATEPART(mm, D.DATE_STAT)								MONTH,
				DATEPART(yyyy, D.DATE_STAT)								YEAR,
				D.SHOW_COUNT,
				D.CLICK_COUNT,
				D.VISITOR_COUNT,
				D.BANNER_ID,
