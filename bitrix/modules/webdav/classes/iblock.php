...
if (!function_exists("array_diff_key")) // php < 5.1.0
{
...
class CWebDavIblock extends CWebDavBase
{
	...
	//var $arRootSection = false;(general)
	...
	function CWebDavIblock($IBLOCK_ID, $base_url, $arParams = array())
	{
		...
		if (!$this->SetRootSection($arParams["ROOT_SECTION_ID"])) // socnet
		{
			$this->arError[] = array(
	...
	function GetPermission($type, $id, $op='')
	{
		if ($this->e_rights)
		{
			if ($op === '')
		...
		else
		{
			...
			if (!empty($options["depth"]))
			{
				...
				if (isset($arParams["FILTER"]["content"]) && (sizeof(array_diff_key($arParams["FILTER"], $arSearchOptParams)) === 1))
				{
					...
					foreach ($arSearchResults as $searchID => $itemID)
					{
				...
				else
				{
					...
					if ($db_res && $res = $db_res->Fetch())
					{
						do
						{
							if (!empty($arSearchResults) && (array_search($res["ID"], $arSearchResults) === false))
								continue;
							...
							if ($this->MetaNames($res))
							{
								if ($res["TYPE"] == "S")
								{
									if (!$this->MetaSectionHide($res))
									{
										if (empty($arParams["FILTER"]) || $this->meta_state == "TRASH")
										{
											$files["files"]["S".$res["ID"]] = $this->_get_section_info_arr($res);
											$arResult["RESULT"]["S".$res["ID"]] = $res;
										}
	...
	function _ib_elm_update($id, $arFields, $workflow=false)
	{
		...
		else
		{
			...
			if ($arProperties !== false)
			{
				...
				{
					$el->SetPropertyValues($id, $this->IBLOCK_ID, $value, $code);
				}

				if (! $this->ValidatePropertyValues($id, $arProperties)) // as SetPropertyValues doesn't handle SaveFile errors
				{
		...
		$DB->Commit();
		if(class_exists('CWebDavDiskDispatcher'))
		{
	...
	function put_commit(&$options)
	{
		...
		$arFile = array(
			...
			"type" => "",
			"error" => 0);
		if (!is_file($arFile['tmp_name']) && is_file($options['TMP_FILE']))
			$arFile['tmp_name'] = $options['TMP_FILE'];
		...
		if (isset($arFile['tmp_name']) && is_file($arFile['tmp_name']))
		{
			...
			if (($arFile['type'] == '') && ($arFile["size"] > 0))
				$arFile["type"] = CFile::GetContentType($arFile['tmp_name']);

			if (($arFile['type'] == '')
				|| ($arFile['type'] == 'application/zip') // fix old magic.mime
				|| ($arFile['type'] == 'image/jpeg') // fix FF15 invalid mime types for *.doc
			)
...
//			"For Mac OS";
	...
	static function _move_from_iblock_to_iblock($ID, $targetIblockID, $targetSectionID=0, $delete=true)
	{
		global $DB;
		$ID = intval($ID);
		$targetIblockID = intval($targetIblockID);
		$targetSectionID = intval($targetSectionID);
		if (($ID <= 0) || ($targetIblockID <= 0))
			return false;
		...
		if($ar_element = $db_element->Fetch())
		{
			...
			else
			{
				...
				$ar_element["IBLOCK_SECTION_ID"] = $targetSectionID;
				$ar_element["IN_SECTIONS"] = "N";
				...
				$ar_element["IBLOCK_ID"] = $targetIblockID;
				...
				else
				{
					if(class_exists('CWebDavDiskDispatcher'))
					{
						$ar_element['ID'] = $NEW_ID;
						CWebDavDiskDispatcher::sendEventToOwners($ar_element, null, 'move_from_block_to_block');
					}
					if ($delete === true)
					{
	...
	function COPY($options, $drop = false)
	{
		...
		{ // found and is_file
			//if ($this->check_creator && $arFrom["element_array"]["CREATED_BY"] != $GLOBALS["USER"]->GetID())
			//{
				//return $this->ThrowAccessDenied("USER_IS_NOT_CREATOR", __LINE__);
			//}
		}
					...
					if ($this->arParams["not_found"] === false)
					{
						...
						if ($arTo["dir_array"]["ID"] === false)
						{
							return $this->ThrowError( "409 Conflict", "FOLDER_IS_NOT_MOVED", str_replace(
								...
								array("/".implode("/", $arPath), $se->LAST_ERROR), GetMessage("WD_FILE_ERROR103")), __LINE__);
						} else {
		...
		if (!empty($arFromSections))
		{
			...
			do
			{
				...
				if (!array_key_exists($res["PATH"], $arToSections))
				{
					...
					{
						$se_result = $se->Add(array("IBLOCK_ID" => $this->IBLOCK_ID, "IBLOCK_SECTION_ID" => end($arParentId), "NAME" => $res["NAME"]));
						if (!$se_result)
						{
							$result = $this->ThrowError("409 Conflict", "FOLDER_IS_NOT_MOVED", str_replace(
								array("#FOLDER#", "#TEXT_ERROR#"),
			...
			foreach ($arSectionsToCopyElements as $iFromSectionID => $arElement)
			{
				...
				if ($arElement["NEED_CHECK_NAME"] != false)
				{
					$db_res = CIBlockElement::GetList(
						array("NAME" => "ASC"),
						array("IBLOCK_ID" => $this->IBLOCK_ID, "SECTION_ID" => $arElement["TO_ID"], "SHOW_NEW" => "Y", "CHECK_PERMISSIONS" => "N"),
						false,
						false,
						array("ID", "NAME"));
					if ($db_res && $res = $db_res->Fetch())
				...
				$db_res = CIBlockElement::GetList(
					array("NAME" => "ASC"),
					array("IBLOCK_ID" => $this->IBLOCK_ID, "SECTION_ID" => $iFromSectionID, "SHOW_NEW" => "Y"),
					false,
					false,
	...
	function MKCOL($options)
	{
		...
		else
		{
			...
			if(class_exists('CWebDavDiskDispatcher'))
			{
				...
				CWebDavDiskDispatcher::sendEventToOwners(null, $section, 'mkcol');
			}
		}
		$this->ClearCache("section");
		return "201 Created";
	...
	function _isInMeta($id, $metaID)
	{
		$trashID = $this->GetMetaID($metaID);
		$elm = $this->GetObject(array('element_id' => intval($id)));
		if ($elm['not_found'])
			return false;

		$arSectionsChain = array();
		$db_res = CIBlockSection::GetNavChain($this->IBLOCK_ID, $elm['parent_id']);
		while($res = $db_res->Fetch())
			$arSectionsChain[] = $res["ID"];

		return (in_array($trashID, $arSectionsChain));
	}
	...
	function DeleteDroppedFile($fileID)
	{
		...
		$options = array('element_id' => $fileID);
		$arFile = $this->GetObject($options);

		if (! $arFile['is_file'])
			return false;

		$bDropTargetFound = false;
		$db_res = CIBlockSection::GetNavChain($this->IBLOCK_ID, $arFile['parent_id']);
		while($res = $db_res->Fetch())
		{
			if (!$bDropTargetFound && $res["ID"] == $dropTargetID)
			{
		...
		// delete
		if ($bDropTargetFound)
			$this->DELETE( $options+array('force' => true) );
	}
	...
	function CleanUpDropped()
	{
		...
		//{
			...
			if ($arParentSection = $rsParentSection->GetNext())
			{
				...
				$dbFolders = $ise->GetList(array(),$arFilter, true);
				while($arFolder = $dbFolders->Fetch())
				{
					if (intval($arFolder['ELEMENT_CNT']) <= 0)
	...
	function InTrash($arElement)
	{
		...
		$result = false;
		$db_res = CIBlockSection::GetNavChain($this->IBLOCK_ID, $parentID);
		while($res = $db_res->Fetch())
		{
			if($res["ID"] == $trashID)
			{
	...
	function DELETE($options)
	{
		...
		else
		{
			...
			if (($this->arParams["is_dir"] === true) || (intVal($this->arParams["element_id"]) > 0) && !isset($options['force']))
			{
				if ($trashID = $this->GetMetaID("TRASH"))
				{
					...
					$arSectionsChain = array();
					$db_res = CIBlockSection::GetNavChain($this->IBLOCK_ID, $this->arParams['parent_id']);
	...
	function _eventElementParams($elementID, $elementType)
	{
		...
		if ($sFileName !== '.Trash')
		{
			...
			if ($this->_isInMeta($arElement['item_id'], 'DROPPED'))
				$result['dropped'] = true;
	...
	function _delete_section($sectionID)
	{
		...
		{
			if (! $this->GetPermission('SECTION', $sectionID, 'section_delete')) // check perms on target folder
			{
			...
			if ($result === true) // no errors
			{
				...
				{
					$DB->StartTransaction();
					if ($se->Delete($ID, false)) // delete each section in separate transaction
					{
						$DB->Commit();
					}
					else
	...
	{
		$id = md5(intval($this->IBLOCK_ID).'|'.intval($this->arRootSection['ID']).'|'.$path.($replace_symbols ? "Y" : "N"));
		return $id;
	}

	function GetObject($options = array(), $replace_symbols = false)
	{
		global $DB;
		$arParams = array();
		...
		if (!isset($options['check_permissions']))
			$options['check_permissions'] = true;
		$path = $this->_udecode(array_key_exists("path", $options) ? $options["path"] :
			(array_key_exists('section_id', $options) ? 'section' . $options['section_id'] :
			(array_key_exists('element_id', $options) ? 'element' . $options['element_id'] :
			$this->_path)));
		$arSelectFields = array("ID", "NAME", "IBLOCK_ID", "IBLOCK_SECTION_ID", "PREVIEW_TEXT",
			"CREATED_BY", "TIMESTAMP_X", "PROPERTY_".$this->file_prop, "PROPERTY_WEBDAV_INFO",
			"WF_NEW", "WF_PARENT_ELEMENT_ID", "WF_STATUS_ID",
			"IBLOCK_CODE", "BP_PUBLISHED");
		$id = $this->_get_element_cacheID($path, $replace_symbols).($options['check_permissions']?'Y':'N');

		if (isset($this->CACHE[__METHOD__][$id]))
		{
			...
			preg_match_all("/(?<=\#)([a-z_]+)(?=\#)/is", strToLower($this->short_path_template), $arKeys))
		{
			...
			if (is_array($arKeys) && !empty($arKeys))
			{
				...
				if (preg_match($pageTemplateReg, $path, $arValues))
				{
					...
					{
						if (intval($arValues["element_id"]) > 0)
							$options["element_id"] = $arValues["element_id"];
						elseif (intval($arValues["section_id"]) > 0)
							$options["section_id"] = $arValues["section_id"];
					}
				}
			}
		}
	 ...
	 * @return bool
	 */
	function fileViewByUser($id)
	{
		$id = (int)$id;
		$filter = array('IBLOCK_ID' => $this->IBLOCK_ID, 'ID' => $id, 'CHECK_PERMISSIONS' => 'Y', 'SHOW_NEW' => 'Y');
		...
		{
			CFile::ViewByUser($row['PROPERTY_FILE_VALUE'], array('content_type' => 'application/octet-stream', 'force_download' => true));
		}
		return false;
	}
	...
	function SendHistoryFile($ID, $WF_ID = 0, $NotCheckWebRights = false, $params = array()) // wrong
	{
		...
		{
			$io = self::GetIo();
			$arFilter = array("IBLOCK_ID" => $this->IBLOCK_ID, "ID" => $ID, "SHOW_HISTORY"=>"Y", "CHECK_PERMISSIONS" => "Y");
			if ($this->permission < "U")
			{
				$arFilter["SHOW_HISTORY"] = "N";
			}
			if (($this->workflow == 'bizproc' || $this->workflow == 'bizproc_limited') && $WF_ID > 0)
			{
				$history = new CBPHistoryService();
				$db_res = $history->GetHistoryList(
					null,//array($by => $order),
					array(
						"DOCUMENT_ID" => array($this->wfParams['DOCUMENT_TYPE'][0], $this->wfParams['DOCUMENT_TYPE'][1], $ID),
						"ID" => $WF_ID),
					false,
					false,
					array("ID", "DOCUMENT", "NAME")
					...
					if (CFile::IsImage($arFile["FILE_NAME"], $arFile["CONTENT_TYPE"]))
					{
						if ($params["width"] > 0 || $params["height"] > 0)
						{
							$arTmpFile = CFile::ResizeImageGet(
								...
								($params["exact"] == "Y" ? BX_RESIZE_IMAGE_EXACT : BX_RESIZE_IMAGE_PROPORTIONAL),
								true,
								false,
								true
							);
			...
			if (!empty($arFile))
			{
				...
				(!empty($arFile["CONTENT_TYPE"]) && $arFile["CONTENT_TYPE"] != "unknown" && $arFile["CONTENT_TYPE"] != "application/octet-stream") ?
					$arFile["CONTENT_TYPE"] : $this->get_mime_type($file_name));
				$options["size"] = (!empty($arFile["FILE_SIZE"]) ? $arFile["FILE_SIZE"] : filesize($fullpath));
				...
				{
					$arPatamsC = $this->GetObject(array("element_id" => $ID, "check_permissions" => false), false);
					if(!$this->CheckWebRights("",  array("action" => "read", "arElement" => $arPatamsC), false)){
						return;
					}
				}
	...
	function CheckUniqueName($basename, $section_id, &$res)
	{
		...
		$arFilter = array(
			...
			"=NAME" => $basename,
			'CHECK_PERMISSIONS' => 'N',
			"SHOW_NEW" => ($this->workflow == 'workflow' || $this->workflow == 'bizproc' ? "Y" : "N")
		);
		$arOrder = array("ID" => "ASC");

		$db_res = CIBlockElement::GetList($arOrder, $arFilter, false, $arSelectFields);
		if ($db_res && $res = $db_res->Fetch())
		{
			$arObject = array(
				"object" => "element",
		...
		if (!empty($arObject))
		{}
		elseif ($this->workflow == 'workflow' || $this->workflow == 'bizproc')
		{
			$arFilter["SHOW_HISTORY"] = "Y";
			$db_res = CIBlockElement::GetList($arOrder, $arFilter, false, false, $arSelectFields);
			if ($db_res && $res = $db_res->Fetch())
				...
				do
				{
					...
					if (empty($arBuff[$res["LAST_ID"]]))
					{
						...
						if (!($db_res && $res = $db_res->Fetch())):
							...
							break;
						elseif ($arBuff[$res["LAST_ID"]]["NAME"] == $basename &&  $arBuff[$res["LAST_ID"]]["IBLOCK_SECTION_ID"] == $section_id):
							$arObject = array(
								"object" => "element",
								"id" => $res["ID"],
								"data" => $res);
	...
	function _get_mixed_list($section_id, $arParams = array(), $element_id = 0)
	{
		...
		else
		{
			...
			if (!isset($arFilter["ID"]))
			{
				...
				$rsSection = $obSection->GetList($arOrder, $arSectionFilter, false, $arSelectedFields);
				...
				//if ($this->e_rights && is_array($this->arRootSection) && ($this->arRootSection['ID'] == $arSectionFilter['SECTION_ID'])) // force to show user shared folder
				//{
					//$arSectionFilter['CHECK_PERMISSIONS'] = 'N';
					//$arSectionFilter['XML_ID'] = 'SHARED_FOLDER';
					//$rsSection = $obSection->GetList($arOrder, $arSectionFilter, false, $arSelectedFields);
					//while($arSection = $rsSection->Fetch())
					//{
	...
	function GetMetaID($sMetaName)
	{
		...
		if (!$arMetaIDs || !isset($arMetaIDs[$sMetaName]))
		{
			if (!isset($this->meta_names[$sMetaName]))
				return null;
			$arFilter = array(
				"IBLOCK_ID" => $this->IBLOCK_ID,
				"SECTION_ID" => ($this->arRootSection?$this->arRootSection['ID']:0),
				"NAME" => $this->meta_names[$sMetaName]['name'],
				"CHECK_PERMISSIONS" => "N"
			);
			$db_res = CIBlockSection::GetList(array(), $arFilter, false, array("ID"));
			if ($db_res && $arMeta = $db_res->GetNext())
			{
				$value = $arMeta['ID'];
			}
			else
			{
				...
				$b = $se->Add(array(
					"NAME" => $this->meta_names[$sMetaName]['name'],
					"IBLOCK_ID" => $this->IBLOCK_ID,
					"IBLOCK_SECTION_ID" => ($this->arRootSection?$this->arRootSection['ID']:0),
					"ACTIVE" => "Y"));
	...
	function MetaSectionHide(&$res)
	{
		static $arMetaID = null;
		if ($arMetaID === null)
		{
			foreach ($this->meta_names as $metaID => $arMeta)
			{
				if ($arMeta['show_in_list'] === false)
					$arMetaID[$metaID] = intval($this->GetMetaID($metaID)); // GetMetaID is already cached
			}
		}
		return in_array(intval($res['ID']), $arMetaID, true);
	}
		...
		if (!in_array($res["ID"], $arFiles))
		{
			...
			if ($this->permission >= "U") // 'X' if e_rights
			{
				...
				elseif($this->workflow == "bizproc")
				{
					...
					if ($res['LOCK_STATUS'] != "red"):
						$res['LOCK_STATUS_BP'] =
							(call_user_func(array($this->wfParams["DOCUMENT_TYPE"][1], "IsDocumentLocked"), $res["ID"], "") ? "red" : "green");
	...
	function GetSectionsTree($options = array())
	{
		...
		$id = md5(serialize($options));
		$section = $this->_dataCache($dataType . $id);
		if ($section === false)
		{
			$arElement = $this->GetObject($options);
			...
			else
			{
				...
				$arResult = array();
				$db_res = CIBlockSection::GetTreeList($arFilter); // TODO: add e_rights check
				$trashID = $this->GetMetaID('TRASH');
				if ($db_res && $res = $db_res->Fetch())
				{
					$deep = -1;
					...
					do
					{
						...
						{
							$res["DEPTH_LEVEL"] = intVal($res["DEPTH_LEVEL"]);
							$allow = array_search((int) $res["IBLOCK_SECTION_ID"], $arExclude);
							if ($allow !== false || $this->MetaSectionHide($res))
							{
								$arExclude[] =(int) $res["ID"];
							}
							else
							{
			...
			$this->_dataCache($dataType . $id, $section);
		}
		return $section;
	}
	...
	function CheckRights($method = "", $strong = false, $path = "")
	{
		...
		{
			$result = false;
			$GLOBALS["APPLICATION"]->ThrowException(GetMessage('WD_ACCESS_DENIED'), 'ACCESS_DENIED', __LINE__);
		}
		elseif (!empty($path))
		{
				...
				if (empty($arError))
				{
					...
					elseif ($action == 'delete' || $action == 'undelete') // aka move to trash, move op will be checked later
					{
						...
						if ($type == 'ELEMENT')
						{
							...
							$res = $this->GetPermission($type, $arElement['item_id'], 'section_delete');
							if (!$res)
								$arError[] = array("id" => "", "text" => GetMessage("WD_ACCESS_DENIED"));
						}
					}
	...
	function _ib_elm_add($arFields, $bWorkFlow=false, $bUpdateSearch=true, $bResizePictures=false)
	{
		...
		else
		{
			if (
				...
				&& ! $this->ValidatePropertyValues($ID, $arFields['PROPERTY_VALUES']) // as SetPropertyValues doesn't handle SaveFile errors
			)
			{
				$this->LAST_ERROR = GetMessage("WD_FILE_ERROR111");
				$DB->Rollback();
		...
		if(class_exists('CWebDavDiskDispatcher'))
		{
			...
			CWebDavDiskDispatcher::sendEventToOwners(null, $section, 'add');
		}
		return (int) $result;
	}

	function ValidatePropertyValues($ID, $arProperties)
	{
		$result = true;
		...
		foreach ($arProperties as $code => $value)
		{
			$dbProps = CIBlockElement::GetProperty($this->IBLOCK_ID, $ID, array(), array('CODE' => $code));
				...
				if ($arProp['PROPERTY_TYPE'] == 'F')
				{
					if (((int) $arProp['VALUE']) > 0)
					{
						...
						if (
							...
							&& ($arFile['FILE_SIZE'] == $arProperties[$code]['size'])
						)
						{
	...
	function SetRootSection($id, $force=false)
	{
		...
		if($this->arRootSection === false)
		{
			...
			$db_res = CIBlockSection::GetList(array(), $arFilter, false, array('UF_USE_EXT_SERVICES'));
	...
	public static function checkUfUseExtServices($iblockId)
	{
		...
		$query = CUserTypeEntity::GetList(array($by=>$order), array('ENTITY_ID' => 'IBLOCK_'.$iblockId.'_SECTION', 'FIELD_NAME' => 'UF_USE_EXT_SERVICES'));
