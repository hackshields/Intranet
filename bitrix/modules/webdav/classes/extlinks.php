...
class CWebDavExtLinks
{
	...
	static $urlGoogleViewer = 'https://docs.google.com/viewer?url=';
...
*/
		...
		$fileName = htmlspecialcharsbx(GetFileName($url));
		$size = "";
			...
			foreach($arLinks as $v)
			{
				...
				echo '
								<p id="ext-link-list-p-' . $n . '" class="ext-link-list-row">
									...
									<span class="ext-link-list-delete" onclick="ExtLinkDialogDeleteLink(\'' . htmlspecialcharsbx($urlFull) . '?DeleteLink=' . $v["HASH"] . '&' . bitrix_sessid_get() . '&IFRAME=Y\',\'ext-link-list-p-' . $n . '\')"></span>
									</span>' . $optionsStr . '
								</p>
				';
			}
?>
					...
					<tr>
						<td>
							<span class="ext-link-delite-all" onclick="ExtLinkDialogDeleteAllLinks('<? echo htmlspecialcharsbx($urlFull); ?>?<? echo bitrix_sessid_get(); ?>','<? echo htmlspecialcharsbx($url); ?>')"><span class="ext-link-comments-link-text"> <? echo GetMessage("WD_EXT_LINKS_DIALOG_DELETE_ALL_LINKS"); ?></span><i></i></span>
						</td>
					</tr>
				</tbody>
			</table>
...
<?
	...
	function InsertDialogCallText($urlT)
	{
		...
		return "ShowExtLinkDialog('" . $urlT . "&GetExtLink=1&IFRAME=Y','" . $urlT . "&GetDialogDiv=1&IFRAME=Y')";
	}
	...
	function LoadFile($arLink)
	{
		...
		if(!is_array($arLink))
		{
			$resF = self::GetList(array("HASH" => $hash, "ACTUAL" => true,), array("URL","RESOURCE_TYPE","FOLDER","IBLOCK_TYPE","IBLOCK_ID","BASE_URL","USER_ID","ROOT_SECTION_ID", "DOWNLOAD_COUNT"));
			$arF = $resF->Fetch();
		}
		if(is_array($arF))
		{
			...
			{
				$ob = new CWebDavIblock($arF['IBLOCK_ID'], $arF['BASE_URL'], $arF);
