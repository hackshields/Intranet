...
<D:response>
		...
		if (!headers_sent())
		{
			...
			if (isset($options['stream']))
			{
					...
					if (count($options['ranges']) === 1)
					{
						...
						if (isset($range['start']))
						{
							...
							if (isset($range['end']))
							{
								...
								self::set_header('Content-range: '.$range['start'].'-'.$range['end'].'/'. (isset($options['size']) ? $options['size'] : '*'));
								while ($size && !feof($options['stream']))
								{
									$buffer = fread($options['stream'], 8192);
							...
							else
							{
								...
								{
									self::set_header('Content-length: '.($options['size'] - $range['start']));
									self::set_header('Content-range: '.$range['start'].'-'.$range['end']. '/' . (isset($options['size']) ? $options['size'] : '*'));
								}
								while (@ob_end_clean());
						...
						{
							self::set_header('Content-length: '.$range['last']);
				...
				else
				{
					if (isset($options['size']))
					{
						self::set_header('Content-length: '.$options['size']);
					}
	...
	{
		while (ctype_space(substr($string, $pos, 1)))
		{
			++$pos;
		}
...
';
	...
	function CheckRights($method = "", $strong = false)
	{
