		...
		foreach($arParams['IBLOCK_OTHER_IDS'] as $id)
		{
			if (isset($modes[$id]))
			{
				...
				$folderTree[] = array(
					...
					'DEPTH_LEVEL' => $depth,
					'MODE' => 'remote',
					'DOCCOUNT' => _getIBlockItemsCount($id),
	...
	if ($mode == 'private' || $mode == 'root')
	{
		if (CSocNetFeatures::IsActiveFeature( SONET_ENTITY_USER, $currentUserID, "files")) 
		{
			...
			$folderTree[] = array(
				...
				'DEPTH_LEVEL' => 0,
				'MODE' => 'remote',
				'DOCCOUNT' => _getIBlockItemsCount($arParams["IBLOCK_USER_ID"], null, 'user', $currentUserID),
				'CLASS' => 'personal');
		}
	}
				...
				else
				{
					foreach ($arGroupSectionIDs as $sectionID=>$groupID)
					{
						...
						$groupTree[] = array(
							'NAME' => _getName($name),
							'PATH' => _getPath($path, $arParams['SEF_FOLDER']),
							'DOCCOUNT' => _getIBlockItemsCount($arParams['IBLOCK_GROUP_ID'], $sectionID, 'group', $groupID),
	...
	if ($mode == 'user')
	{
		...
		if (sizeof($arLocalPath) > 0)
		{
			...
			if (strpos($userName, '(') !== false)
			{
				...
				$dbUser = CUser::SearchUserByName($arName, '', true);
			}
			...
			else 
			{
				...
				while ($arSection = $dbSection->Fetch())
				{
					...
					$iDocCount = _getIBlockItemsCount($arParams["IBLOCK_USER_ID"], $arSection["ID"], 'user', $userID);
