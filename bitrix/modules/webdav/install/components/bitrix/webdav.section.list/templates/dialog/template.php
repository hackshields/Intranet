...
function __struct_get_files($ob, $doc_root, $path="", $open_path="", $dirsonly=false)
{
	...
	$ob->IsDir(array("path" => $abs_path));
	$sectionID = $ob->arParams['item_id'];
	if ($sectionID == false) $sectionID = 0;
	$rItems = $ob->_get_mixed_list($sectionID);
	if ($rItems && ($arItem = $rItems->GetNext()))
	{
		do {
			if (!$ob->MetaSectionHide($arItem))
			{
				$arFiles[] = __struct_get_file_info($ob, $abs_path, $arItem['NAME']);
			}
		} while ($arItem = $rItems->GetNext());
	}
