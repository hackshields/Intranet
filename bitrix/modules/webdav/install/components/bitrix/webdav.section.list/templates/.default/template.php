...
if (!$this->__component->__parent || $this->__component->__parent->__name != "bitrix:webdav"):
	...
	$GLOBALS['APPLICATION']->AddHeadString('<script src="'.BX_ROOT.'/components/bitrix/webdav/templates/.default/script.js"></script>', true);
endif;
...
foreach ($arProps as $code => &$arIBProp)
{
	...
	$arHeaders[] = array(
		"id" => "PROPERTY_".$code,
		"name" => trim(empty($res["NAME"]) ? $code : $res["NAME"]),
...
<script>
	BX( function() {
		...
		if (hilight_row)
		{
			...
			if (BX(hilight_row[1])) BX(hilight_row[1]).scrollIntoView();
		}
...
<?	} ?>
	...
	BX.loadScript('/bitrix/js/main/core/core_dd.js', function() {
