...
if (!function_exists("__build_item_info"))
{
	function __build_item_info(&$res, $arParams, $WrapLongWords = false)
	{
		...
		{
			$allowExtDocServicesGlobal ='Y' ==
				COption::GetOptionString('webdav', 'webdav_allow_ext_doc_services_global', CWebDavIblock::resolveDefaultUseExtServices());
			$allowExtDocServicesLocal = 'Y' ==
				COption::GetOptionString('webdav', 'webdav_allow_ext_doc_services_local', CWebDavIblock::resolveDefaultUseExtServices());
...
/************** Grid Data ******************************************/
		...
		if ($res["TYPE"] == "S")
		{
			...
			if ($res["SHOW"]["DELETE"] == "Y" && ((!$bInTrash) || ($bInTrash && $arParams["PERMISSION"] > "W")))
			{
				$arActions["section_drop"] = array(
					...
					"ONCLICK" => "WDConfirm('".CUtil::JSEscape(GetMessage("WD_DELETE_TITLE")).
		...
		else
		{
			...
			if($allowExtDocServices && in_array(ltrim($res["FILE_EXTENTION"], '.'), CWebDavExtLinks::$allowedExtensionsGoogleViewer) && $res['PROPERTY_WEBDAV_SIZE_VALUE'] < CWebDavExtLinks::$maxSizeForView)
			{
				//showInViewer
				$downloadUrl = CUtil::JSEscape($res["URL"]["~DOWNLOAD"]);
				$editInGoogleUrl = $editrUrl  = CUtil::JSEscape(($GLOBALS['APPLICATION']->IsHTTPS() ? 'https' : 'http').'://'.str_replace("//", "/", $_SERVER['HTTP_HOST']."/".$res["URL"]["THIS"])) . '?' . bitrix_sessid_get() . '&editInGoogle=1&start=1';
				$downloadUrl .= ((strpos($downloadUrl, "?") === false) ? "?" : "&") . "force_download=1";
			...
			else
			{
				...
				$arActions['preview_launch'] = array(
					...
					'size' => CFile::FormatSize($res['PROPERTY_WEBDAV_SIZE_VALUE']),
					'dateModify' => FormatDate('FULL', MakeTimeStamp($res["TIMESTAMP_X"])),
					'tooBigSizeMsg' => $res['PROPERTY_WEBDAV_SIZE_VALUE'] > CWebDavExtLinks::$maxSizeForView,
				);
			}
			$downloadUrl = CUtil::JSEscape($res["URL"]["~DOWNLOAD"]);
			$downloadUrl .= ((strpos($downloadUrl, "?") === false) ? "?" : "&") . "force_download=1";
		...
		if ($res["TYPE"] == "S")
		{
			...
			$aCols["NAME"] =
				'<div class="section-name">
					<div class="section-icon"></div>'.$aCols["NAME"]['shared'].'<a class="section-title" id="sec'.$res['ID'].'" href="'.$res["URL"]["THIS"].'"'.
