...
<p><?=GetMessage('WD_UPLOAD_EXPAND_PROPS', array("#LINK#" => 'id="wd_upload_expand" onclick="WDUploadExpand(this);" class="ajax"'));?></p>
<? endif; ?>
...
<p><?=GetMessage('WD_UPLOAD_EXTENDED', array("#LINK#" => htmlspecialcharsbx($APPLICATION->GetCurPageParam("",array('use_light_view')))));?></p>
<? endif; ?>
<script>
...
function WDUploadExpand(link)
{
	...
	if (wdExtTable.style.display != 'table')
	{
		...
		if (link)
			link.parentNode.innerHTML = '<?=GetMessage('WD_UPLOAD_COLLAPSE_PROPS', array("#LINK#" => 'id="wd_upload_expand" onclick="WDUploadExpand(this);" class="ajax"'))?>';
	}
	else
	{
		wdExtTable.style.display = 'none';
		if (link)
			link.parentNode.innerHTML = '<?=GetMessage('WD_UPLOAD_EXPAND_PROPS', array("#LINK#" => 'id="wd_upload_expand" onclick="WDUploadExpand(this);" class="ajax"'))?>';
	}
}
...
BX(function() {
	...
	BX.loadScript("<?=$this->__folder . '/script_deferred.js'?>", function(){
		WDUploadInit(dialog);
	});
});
</script>
