...
BX.WebDavFileDialog.getTabsHtml = function(name)
{
	...
	for (var i = 0, c = arTypeSort.length; i < c; i++)
	{
		...
		html += '<div class="bx-file-dialog-tab-group">';
		if (type.name)
			html += '<div class="bx-file-dialog-tab-group-title">'+type.name+'</div>';
		if (type.items)
		{
			for (var j = 0; j < type.items.length; j++)
			{
				...
				html += '<div class="bx-file-dialog-tab-item">'+
...
BX.WebDavFileDialog.getItemsHtml = function(name)
{
	...
	if (BX.WebDavFileDialog.obCurrentPath[name] == '/')
		...
		for (var i = 0, c = arTypeSort.length; i < c; i++)
		{
			...
			htmlElement = '<div class="bx-file-dialog-row '+extraClass+'" data-id="'+item.id+'">'; // bx-file-dialog-row-selected bx-file-dialog-row-disabled
			...
			for (var j in BX.WebDavFileDialog.obGridColumn[name])
			{
				...
				if (firstColumn)
				{
					htmlElement += '<span class="bx-file-dialog-column-row bx-file-dialog-column-'+column.id+''+extra+'" style="'+column.style+'"  title="'+column.name+': '+item[column.id]+'">';
					if (elementDisable)
					{
					...
					{
						var clickfunc = 'onclick="return BX.WebDavFileDialog.selectItem(this.parentNode.parentNode, \''+item.id+'\', \''+name+'\')"';
						var dblclickfunc = '';
						//if (!BX.WebDavFileDialog.obItemsSelectMulti[name])
						//	dblclickfunc = 'ondblclick="return BX.WebDavFileDialog.selectItem(this.parentNode.parentNode, \''+item.id+'\', true, \''+name+'\')"';
						if (item.type == 'folder')
						{
							clickfunc = selectDisable? 'onclick="return BX.WebDavFileDialog.openFolder(this.parentNode.parentNode, \''+item.id+'\', \''+name+'\')"': clickfunc;
							dblclickfunc = selectDisable? '': 'ondblclick="return BX.WebDavFileDialog.openFolder(this.parentNode.parentNode, \''+item.id+'\', \''+name+'\')"';
						}
						htmlElement += '<a href="#" class="bx-file-dialog-content-link bx-file-dialog-icon bx-file-dialog-icon-'+item.type+'" '+clickfunc+' '+dblclickfunc+'>'+item[column.id]+'</a>';
					}
