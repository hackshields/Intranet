...
class CVote extends CAllVote
{
	...
	function GetActiveVoteID($CHANNEL_ID)
	{
		...
		if ($CHANNEL_ID > 0)
		{
			if (!array_key_exists($CHANNEL_ID, $GLOBALS["VOTE_CACHE"]["CHANNEL"]))
			{
				$db_res = $GLOBALS["DB"]->Query("SELECT MAX(V.ID) AS ACTIVE_VOTE_ID ".
					...
					" SYSDATE >= V.DATE_START AND V.DATE_END >= SYSDATE");
	...
	function CheckVotingIP($VOTE_ID, $REMOTE_ADDR, $KEEP_IP_SEC, $params = array())
	{
		...
		if ($KEEP_IP_SEC > 0):
			$arSqlSelect[] = "(SYSDATE - VE.DATE_VOTE)*86400 AS KEEP_IP_SEC";
			$arSqlSearch[] = "((SYSDATE - VE.DATE_VOTE)*86400 <= ".$KEEP_IP_SEC.")";
		endif;
		...
		$strSql = "
			...
			FROM (
				SELECT V.CHANNEL_ID, V.ID, count(Q.ID) QUESTIONS,  
					CASE
						WHEN (C.VOTE_SINGLE = 'Y' AND V.ID = VV.ACTIVE_VOTE_ID) THEN 'green'  
						WHEN (C.VOTE_SINGLE <> 'Y' AND V.ACTIVE = 'Y' AND V.DATE_START <= SYSDATE AND SYSDATE <= V.DATE_END) THEN 'green'
						ELSE 'red'
					END AS LAMP
				FROM b_vote V
					...
					LEFT JOIN (
						...
						WHERE VVV.CHANNEL_ID = CCC.ID AND CCC.VOTE_SINGLE='Y' AND VVV.ACTIVE = 'Y' 
							AND SYSDATE >= VVV.DATE_START AND VVV.DATE_END >= SYSDATE
							".$sSubFilter."
						GROUP BY VVV.CHANNEL_ID) VV ON (VV.CHANNEL_ID = V.CHANNEL_ID)
					WHERE
						$strSqlSearch
					GROUP BY V.CHANNEL_ID, V.ID, C.VOTE_SINGLE, VV.ACTIVE_VOTE_ID, V.ACTIVE, V.DATE_START, V.DATE_END
			) VV
			INNER JOIN b_vote_channel C ON (C.ID = VV.CHANNEL_ID)
			INNER JOIN b_vote V ON (V.ID = VV.ID) ".
			$strSqlOrder;
		$res = new _CVoteDBResult($DB->Query($strSql, false, $err_mess.__LINE__));
	...
	function GetListEx($arOrder = array(), $arFilter = array())
	{
		...
		$strSql = "
			SELECT V.*,
				...
				".$DB->DateToCharFunction("V.DATE_END")." DATE_END,
				CASE
					WHEN (NOT(V.ACTIVE = 'Y' AND V.DATE_START <=SYSDATE AND SYSDATE <= V.DATE_END)) THEN 'red'
					WHEN (C.VOTE_SINGLE = 'Y') THEN 'yellow'
					ELSE 'green'
				END AS LAMP
			FROM b_vote V
	...
	function GetPublicList($arFilter=Array(), $strSqlOrder="ORDER BY C.C_SORT, C.ID, V.DATE_START desc")
	{
		...
		$strSql = "
			...
			FROM (
				...
				WHERE
					$strSqlSearch
					AND V.ACTIVE = 'Y' AND V.DATE_START <= SYSDATE
				GROUP BY V.CHANNEL_ID, V.ID, C.VOTE_SINGLE, VV.ACTIVE_VOTE_ID, V.ACTIVE, V.DATE_START, V.DATE_END
				".($is_admin ? "" : "
	...
	function GetNowTime($ResultType = "timestamp")
	{
		...
		if (empty($result)):
			$db_res = $DB->Query("SELECT ".$DB->DateToCharFunction($DB->GetNowFunction(), "FULL")." FORUM_DATE FROM DUAL", false, "File: ".__FILE__."<br>Line: ".__LINE__);
			$res = $db_res->Fetch();
