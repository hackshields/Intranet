...
class CVote extends CAllVote
{
		...
		if ($CHANNEL_ID > 0)
		{
			if (!array_key_exists($CHANNEL_ID, $GLOBALS["VOTE_CACHE"]["CHANNEL"]))
			{
				$db_res = $GLOBALS["DB"]->Query("SELECT MAX(V.ID) AS ACTIVE_VOTE_ID ".
					...
					" getdate() >= V.DATE_START AND V.DATE_END >= getdate()");
	...
	function CheckVotingIP($VOTE_ID, $REMOTE_ADDR, $KEEP_IP_SEC, $params = array())
	{
		...
		if ($KEEP_IP_SEC > 0):
			$arSqlSelect[] = "DATEDIFF(SECOND, VE.DATE_VOTE, GETDATE()) AS KEEP_IP_SEC";
			$arSqlSearch[] = "(DATEDIFF(SECOND, VE.DATE_VOTE, GETDATE()) <= ".$KEEP_IP_SEC.")";
		endif;
			...
			FROM (
				SELECT V.CHANNEL_ID, V.ID, count(Q.ID) QUESTIONS,  
					CASE
						...
						WHEN (C.VOTE_SINGLE <> 'Y' AND V.ACTIVE = 'Y' AND V.DATE_START <= getdate() AND getdate() <= V.DATE_END) THEN 'green'
						ELSE 'red'
					END AS LAMP
				FROM b_vote V
					...
					LEFT JOIN (
						...
						WHERE VVV.CHANNEL_ID = CCC.ID AND CCC.VOTE_SINGLE='Y' AND VVV.ACTIVE = 'Y' 
							AND getdate() >= VVV.DATE_START AND VVV.DATE_END >= getdate()
							".$sSubFilter."
	...
	function GetListEx($arOrder = array(), $arFilter = array())
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch($key)
			{
				...
				case "ACTIVE":
					if (empty($val))
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(V.".$key." IS NULL OR LEN(V.".$key.")<=0)";
					else
		...
		$strSql = "
			SELECT V.*,
				...
				".$DB->DateToCharFunction("V.DATE_END")." DATE_END,
				CASE
					WHEN (NOT(V.ACTIVE = 'Y' AND V.DATE_START <= getdate() AND getdate() <= V.DATE_END)) THEN 'red'
	...
	function GetPublicList($arFilter=Array(), $strSqlOrder="ORDER BY C.C_SORT, C.ID, V.DATE_START desc")
	{
		...
		$strSql = "
			...
			FROM (
				...
				WHERE
					$strSqlSearch
					AND V.ACTIVE = 'Y' AND V.DATE_START <= getdate()
				GROUP BY V.CHANNEL_ID, V.ID, C.VOTE_SINGLE, VV.ACTIVE_VOTE_ID, V.ACTIVE, V.DATE_START, V.DATE_END
				".($is_admin ? "" : "
