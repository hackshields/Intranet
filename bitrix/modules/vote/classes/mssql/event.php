	...
	function GetUserAnswerStat($arSort = array(), $arFilter = array(), $arParams = array())
	{
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch($key)
			{
				...
				case "VALID":
					if (empty($val))
						$arSqlSearch[] = ($strNegative=="Y"?"NOT":"")."(VE.".$key." IS NULL OR LEN(VE.".$key.")<=0)";
					else
		...
		$strSql =
			"SELECT VEQ.QUESTION_ID, VEA.ANSWER_ID, COUNT(VEA.ID) as COUNTER, ".
				"MIN(DATEDIFF(SECOND, VE.DATE_VOTE, GETDATE())) AS LAST_VOTE".$strSqlSelect.
		...
		if (isset($arFilter["bGetVoters"]))
		{
			...
			if ($db_res && ($res = $db_res->Fetch()))
			{
				$strSql = "SELECT VEQ.QUESTION_ID, VEA.ANSWER_ID, COUNT(VEA.ID) as COUNTER, ".
					" MIN(DATEDIFF(SECOND, VE.DATE_VOTE, GETDATE())) AS LAST_VOTE, ".
					($arFilter["bGetVoters"] > 0 ?
						" SUM(case when RV0.ID is not null then 1 else 0 end) RANK, ": " 0 as RANK, ").
					" MAX(RV.VALUE) RV_VALUE, MIN(RV.ID) RV_ID".$strSqlSelect.
				" FROM b_vote_event VE ".
					" INNER JOIN b_vote_event_question VEQ ON (VEQ.EVENT_ID = VE.ID) ".
					" INNER JOIN b_vote_event_answer VEA ON (VEA.EVENT_QUESTION_ID = VEQ.ID) ".
					" LEFT JOIN b_vote_user VU ON (VU.ID = VE.VOTE_USER_ID)".
				...
				" WHERE 1=1 ".$strSqlSearch.
				" GROUP BY VEQ.QUESTION_ID, VEA.ANSWER_ID".$strSqlGroup.
				" ORDER BY RANK DESC, RV_VALUE DESC";
