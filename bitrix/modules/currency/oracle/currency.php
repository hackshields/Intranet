...
class CCurrency extends CAllCurrency
{
	function __GetList(&$by, &$order, $lang = LANGUAGE_ID)
	{
		...
		$strSql =
			"SELECT CUR.CURRENCY, CUR.AMOUNT, CUR.AMOUNT_CNT, CUR.SORT, CUR.DATE_UPDATE, ".
			"	CURL.LID, CURL.FORMAT_STRING, CURL.FULL_NAME, CURL.DEC_POINT, ".
			"	CURL.THOUSANDS_SEP, CURL.DECIMALS ".
			"FROM b_catalog_currency CUR, b_catalog_currency_lang CURL ".
			"WHERE CUR.CURRENCY = CURL.CURRENCY(+) ".
			"	AND CURL.LID (+)= '".$DB->ForSql($lang, 2)."' ";
