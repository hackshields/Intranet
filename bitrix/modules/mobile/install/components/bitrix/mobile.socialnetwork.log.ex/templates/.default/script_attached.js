	...
	if (windowScroll.scrollTop >= maxScroll)
	{
		...
		BMAjaxWrapper.Wrap({
			...
			'callback': function(data) 
			{
				...
				setTimeout(function() { __MSLCheckNodesHeight(); __MSLShowLentaImages(); }, 1000);
			},
...
function __MSLShowNewComment(arComment)
{
	...
	if (BX('comment_send_form_comment'))
	...
	app.onCustomEvent('onLogEntryCommentAdd', { log_id: BX.message('MSLLogId') });		
}
			...
			if (get_response_data["arDestinations"] != 'undefined')
			{
				...
				if (typeof (arDestinations) == "object")
				{
					if (BX(bindElement))
					{
						...
						if (
							get_response_data["iDestinationsHidden"] != 'undefined'
...
function __MSLShowImages(parentNode, checkOffset)
{
	...
	checkOffset = typeof(checkOffset) == "undefined" ? true : !!checkOffset;
	for (var i = 0; i < images.length; i++)
	{
		var image = images[i];
		var previewImage = image.getAttribute("data-preview-src");
		var realImage = image.getAttribute("data-src");

		if (!realImage && !previewImage)
			continue;
		if (checkOffset && image.offsetTop >= 175)
			continue;

		if (BitrixMobile.Utils.isElementVisibleOn2Screens(image))
		{
			image.src = realImage;
...
function __MSLShowLentaImages()
{
	__MSLShowImages(document, true);
}

function __MSLShowPostImages(id)
{
	var postContainer = BX("post_block_check_cont_" + id);
	if (!postContainer)
		return;

	if (BX.hasClass(postContainer, "post-item-post-block-full"))
		__MSLShowImages(postContainer, false);
	else
		__MSLShowImages(postContainer, true);
}

function __MSLShowCommentsImages()
{
	var commentContainer = BX("post-comments-wrap", true);
	if (!commentContainer)
		return;
	__MSLShowImages(commentContainer, false);
}
...
function __MSLSetFollow(log_id)
{
	...
	BMAjaxWrapper.Wrap({
		'type': 'json',
		'method': 'POST',
		'url': '/bitrix/components/bitrix/mobile.socialnetwork.log.ex/ajax.php',
		'data': follow_data,
		'callback': function(get_response_data) 
		{
			if (
			...
			if (parseInt(BX.message('MSLLogId')) > 0)
				app.onCustomEvent('onLogEntryFollow', { log_id: log_id });
		},
		'callback_failure': function() {
			if (followBlock)
			{
