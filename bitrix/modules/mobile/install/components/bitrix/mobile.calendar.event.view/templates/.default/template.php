...
{
	if (data.module_id == 'calendar' && data.params.NEW == 'Y')
		app.removeTableCache('calendar_list');

	if (data.module_id == 'calendar' && data.params.EVENT.ID == eventId)
	{
		app.removeTableCache('calendar_list');
...
(function() {
	...
	ViewEventManager.prototype.Init = function(data)
	{
		...
		BX.addCustomEvent("onCalendarEventRemoved", function(data)
		{
			if (data.event_id == _this.oEvent.ID)
				BX.addCustomEvent("onOpenPageAfter", function(){app.closeController({drop: true});});
		});

		//this.pForm.acceptBut.onclick = BX.proxy(this.Accept, this);
		//this.pForm.declineBut.onclick = BX.proxy(this.Decline, this);
	...
	ViewEventManager.prototype.UpdateForm = function()
	{
		...
		else
		{
			...
			this.pForm.description.innerHTML = this.oEvent['~DESCRIPTION'];
			// Location
			...
			// From - to
			this.pForm.fromToCont.innerHTML = this.GetFromToHtml(this.oEvent.DT_FROM_TS, this.oEvent.DT_TO_TS, this.oEvent.DT_SKIP_TIME, this.oEvent.DT_LENGTH);
			...
			if (this.oEvent.IS_MEETING)
			{
				...
				{
					if (this.oEvent.USER_MEETING && this.oEvent.USER_MEETING.STATUS == 'Y')
					{
	...
	ViewEventManager.prototype.DisplayEditButton = function()
	{
		...
		{
			app.addButtons({ // Edit event button
				editButton:
				{
					type: 'right_text',
					style: 'custom',
					...
					callback: function()
					{
						app.openNewPage('/mobile/calendar/edit_event.php?event_id=' + eventId + '&ver=' + version);
					}
				}
			});
		}
	...
	ViewEventManager.prototype.GetFromToHtml = function(DT_FROM_TS, DT_TO_TS, DT_SKIP_TIME, DT_LENGTH)
	{
		var
			from = BX.date.getBrowserTimestamp(DT_FROM_TS),
			to = BX.date.getBrowserTimestamp(DT_TO_TS),
