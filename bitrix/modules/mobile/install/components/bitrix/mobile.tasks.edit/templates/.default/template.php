...
*/
	...
	function __MB_TASKS_TASK_EDIT_selectGroup(data)
	{
		...
		app.openTable({
			...
			url: '/bitrix/components/bitrix/mobile.tasks.edit/ajax.php?USER_ID=<?php echo $arParams['USER_ID']; ?>&action=get_group_list_where_user_is_member&lang=<?php echo LANGUAGE_ID; ?>&sessid=<?php echo bitrix_sessid(); ?>',
			markmode: true,
			multiple: false,
	...
	function __MB_TASKS_TASK_EDIT_selectResponsible(data)
	{
		...
		__MB_TASKS_TASK_EDIT_showUserSelector(function(data){
			...
			if (BX('task-responsible-name'))
				BX('task-responsible-name').innerHTML = user['NAME'];
	...
	function __MB_TASKS_TASK_EDIT_saveTask()
	{
		if (BX('TITLE').value.length == 0)
		{
			alert(BX.message('MB_TASKS_TASK_EDIT_NO_TITLE'));
			return;
		}
		...
		var taskData = {
			...
			DESCRIPTION:    BX('DESCRIPTION').value.replace(/\r?\n|\r/g, '<br>'),
		...
		var ajaxUrl = '/bitrix/components/bitrix/mobile.tasks.detail/ajax.php';
