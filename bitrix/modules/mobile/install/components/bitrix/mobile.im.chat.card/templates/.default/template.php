...
<div class="chat-profile-wrap">
	<div class="chat-profile-top">
		<div class="avatar chat-profile-avatar"></div>
	...
	<div class="chat-profile-users-list-block">
		...
		<div class="chat-profile-users-list" id="chat-profile-users-list" >
			<?foreach ($arResult['USERS'] as $user):?>
				<a id="chat-profile-user-<?=$user['id']?>" class="chat-profile-user" href="#" onclick="app.openNewPage('/mobile/users/?user_id=<?=$user['id']?>'); return false;"><span class="avatar"><span class="avatar_sub" style="background:url(<?=$user['avatar']?>) center no-repeat; background-size:29px"></span></span><span class="chat-profile-user-name"><?=$user['name']?></span></a>
			<?endforeach;?>
		</div>
	</div>
</div>
			...
			else if (data.command == 'chatUserAdd')
			{
				for (var i = 0; i < data.params.newUsers; i++)
				{
					...
					BX('chat-profile-users-list').appendChild(BX.create("a", {
						...
						attrs : { id: 'chat-profile-user-'+user.id, href : '#', onclick : 'app.openNewPage(\'/mobile/users/?user_id='+user.id+'\'); return false;'},
						children : [
