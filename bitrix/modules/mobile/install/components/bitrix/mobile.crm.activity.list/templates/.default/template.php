...
<?$qty = count($arResult['ITEMS']);
	for($i = 0; $i < $qty; $i++):
		...
		?><li class="crm_company_list_item" data-entity-id="<?=$item['ID']?>" style="<?=$wrapperStyle?>" onclick="BX.CrmMobileContext.redirect({ url: '<?=CUtil::JSEscape($item['SHOW_URL'])?>' });">
			<?if($dataItem['LIST_IMAGE_URL'] !== ''):?>
				<img src="<?=htmlspecialcharsbx($dataItem['LIST_IMAGE_URL'])?>" style="width:28px; padding:10px 7px 20px 8px; float:left;" />
			<?endif;?>
			...
			<div class="crm_company_company">
				<?if($isExpired):?>
					<span class="fwb"<?=$isExpired ? ' style="color:#e20707;"' : ''?>><?=htmlspecialcharsbx($dataItem['START_TIME'])?></span>
				<?else:?>
					<?=htmlspecialcharsbx($dataItem['END_TIME'] !== '' ? $dataItem['END_TIME'] : GetMessage('M_CRM_ACTIVITY_LIST_TIME_NOT_DEFINED'))?>
				<?endif;?>
