...
function GetFilrIcon($compPath, $pathQ, $arParams, $arF)
{
	...
	{
		$fileID = intval($arF["PROPERTY_FILE_VALUE"]);
	}
		...
		while ($arF = $resQ["NAV_RESULT"]->Fetch())
		{
			...
			else
			{
				...
				if ($arParams["RESOURCE_TYPE"] == "IBLOCK")
				{
					...
					$resFP = CFile::GetByID($fileID);
					if(is_object($resFP))
					{
						if($arFP = $resFP->Fetch())
						{
							$mime = $arFP["CONTENT_TYPE"];
						}
					}
					if($mime = "unknown" || $mime == "")
					{
						$mime = CWebDavBase::get_mime_type(CFile::GetPath($fileID));
					}*/
					$fSize = CFile::FormatSize($arF["PROPERTY_WEBDAV_SIZE_VALUE"]);
					$fDateCreate = $arF["DATE_CREATE"];
					//$fDateCreate = GetTime(MakeTimeStamp($arF["DATE_CREATE"]),"FULL");
				}
