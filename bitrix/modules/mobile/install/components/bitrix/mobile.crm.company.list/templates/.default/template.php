...
foreach($arResult['ITEMS'] as &$item):
	...
	?><li class="crm_company_list_item crm_arrow" onclick="BX.CrmMobileContext.redirect({ url: '<?=CUtil::JSEscape($item['SHOW_URL'])?>' });">
		...
		<div class="crm_company_company">
			...
			<?=htmlspecialcharsbx(GetMessage("M_CRM_COMPANY_LIST_TYPE"))?>: <span><?=htmlspecialcharsbx($item['COMPANY_TYPE_NAME'])?></span>
			<?endif;?>
			<?if($item['INDUSTRY_NAME'] !== ''):?>
				<?if($item['COMPANY_TYPE_NAME'] !== ''):?>
					<br/>
				<?endif;?>
				<?=htmlspecialcharsbx(GetMessage("M_CRM_COMPANY_LIST_INDUSTRY"))?>: <span><?=htmlspecialcharsbx($item['INDUSTRY_NAME'])?></span>
			<?endif;?>
		</div>
		<div class="clb"></div>
	</li><?
