...
class CUser extends CAllUser
{
	...
	public function Add($arFields)
	{
		...
		else
		{
			...
			$ID = $arFields["ID"] = $DB->NextID("sq_b_user");
	...
	function GetList(&$by, &$order, $arFilter=Array(), $arParams = Array())
	{
		...
		$obUserFieldsSqlWhere = new CUserTypeSQL;
		$obUserFieldsSqlWhere->SetEntity("USER", "U.ID");
		$obUserFieldsSqlWhere->obWhere->AddFields(array(
			"F_LAST_NAME" => array(
				"TABLE_ALIAS" => "U",
		...
		$obUserFieldsSqlWhere->SetSelect($arParams["SELECT"]);
		$obUserFieldsSqlWhere->SetOrder($arOrder);
		$obUserFieldsSqlWhere->SetFilter($arFilter);

		$obUserFieldsSqlSelect = new CUserTypeSQL;
		$obUserFieldsSqlSelect->SetEntity("USER", "U.ID");
		$obUserFieldsSqlSelect->SetSelect($arParams["SELECT"]);
		...
		$online_interval = (array_key_exists("ONLINE_INTERVAL", $arParams) && intval($arParams["ONLINE_INTERVAL"]) > 0 ? intval($arParams["ONLINE_INTERVAL"]) : 120);
		if (isset($arParams['FIELDS']) && is_array($arParams['FIELDS']) && count($arParams['FIELDS']) > 0 && !in_array("*", $arParams['FIELDS']))
		{
			foreach ($arParams['FIELDS'] as $field)
			{
				...
				elseif ($field == 'IS_ONLINE')
					$arSelectFields[$field] =	"CASE WHEN U.LAST_ACTIVITY_DATE > (sysdate-".$online_interval."/86400) THEN 'Y' ELSE 'N' END IS_ONLINE";
				...
				elseif ($field == 'PASSWORD')
					$arSelectFields[$field] =	'U."PASSWORD"';
				elseif (in_array($field, $arFields_all))
					$arSelectFields[] = 'U.'.$field;
			}
		}
		if (empty($arSelectFields))
		{
			foreach ($arFields_all as $field)
				$arSelectFields[$field] = "U.".$field;

			$arSelectFields['PASSWORD'] = 'U."PASSWORD"';
			$arSelectFields['TIMESTAMP_X'] = $DB->DateToCharFunction("U.TIMESTAMP_X")." TIMESTAMP_X";
			$arSelectFields['IS_ONLINE'] =	"CASE WHEN U.LAST_ACTIVITY_DATE > (sysdate-".$online_interval."/86400) THEN 'Y' ELSE 'N' END IS_ONLINE";
		...
		if(is_array($arFilter))
		{
			foreach ($arFilter as $key => $val)
			{
				...
				case "TIMESTAMP_2":
					...
					break;
				case "TIMESTAMP_X_1":
					$arSqlSearch[] = "U.TIMESTAMP_X>=TO_DATE('".FmtDate($val,"DD.MM.YYYY HH:MI:SS")."','dd.mm.yyyy hh24:mi:ss')";
					break;
				case "TIMESTAMP_X_2":
					$arSqlSearch[] = "U.TIMESTAMP_X<=TO_DATE('".FmtDate($val,"DD.MM.YYYY HH:MI:SS")."','dd.mm.yyyy hh24:mi:ss')";
					break;
				...
				case "DATE_REGISTER_1":
					$arSqlSearch[] = "U.DATE_REGISTER>=TO_DATE('".FmtDate($val, "D.M.Y")." 00:00:00','dd.mm.yyyy hh24:mi:ss')";
					break;
				case "DATE_REGISTER_2":
					$arSqlSearch[] = "U.DATE_REGISTER<=TO_DATE('".FmtDate($val, "D.M.Y")." 23:59:59','dd.mm.yyyy hh24:mi:ss')";
					break;
				case "ACTIVE":
				...
				case "EXTERNAL_AUTH_ID":
					if($val <> '')
						...
						$arSqlSearch[] = "U.EXTERNAL_AUTH_ID IS NULL";
					break;
				...
				case "!CONFIRM_CODE":
					if($val <> '')
						...
						$arSqlSearch[] = "U.CONFIRM_CODE IS NOT NULL";
					break;
				case "COUNTRY_ID":
				case "WORK_COUNTRY":
				...
				case "EMAIL":
					...
					break;
				case "=EMAIL":
					$arSqlSearch[] = "UPPER(U.EMAIL) = UPPER('".$DB->ForSQL(trim($val))."')";
					break;
				case "GROUP_MULTI":
				case "GROUPS_ID":
					...
					if(is_array($val) && count($val)>0)
					{
						...
						$arSqlSearch[] = "EXISTS(SELECT 'x' FROM b_user_group UG WHERE UG.USER_ID=U.ID AND UG.GROUP_ID in (".implode(",", $ar).") AND ((UG.DATE_ACTIVE_FROM IS NULL) OR (UG.DATE_ACTIVE_FROM <= ".$DB->CurrentTimeFunction().")) AND ((UG.DATE_ACTIVE_TO IS NULL) OR (UG.DATE_ACTIVE_TO >= ".$DB->CurrentTimeFunction()."))) ";
					}
					break;
				...
				case "PERSONAL_BIRTHDAY_DATE":
					$arSqlSearch[] = "(TO_CHAR(U.PERSONAL_BIRTHDAY, 'mm-DD') = '".$DB->ForSql($val)."')";
					break;
				case "KEYWORDS":
				...
				case "NOT_ADMIN":
					if($val !== true)
						break;
					$arSqlSearch[] = "NOT EXISTS(SELECT 'x' FROM b_user_group UGNA WHERE UGNA.USER_ID=U.ID AND UGNA.GROUP_ID = 1)";
					break;
				case "LAST_ACTIVITY":
					if ($val === false)
						$arSqlSearch[] = "U.LAST_ACTIVITY_DATE IS NULL";
					elseif (intval($val)>0)
						$arSqlSearch[] = "U.LAST_ACTIVITY_DATE > (sysdate-".intval($val)."/86400)";
					break;
				case "!LAST_ACTIVITY":
					if ($val === false)
		...
		foreach ($arOrder as $field => $dir)
		{
			...
			if($field == "CURRENT_BIRTHDAY")
			{
				...
				$arSqlOrder[$field] = "CASE
					WHEN U.PERSONAL_BIRTHDAY IS NULL
						THEN '9999-99-99'
					WHEN '".$cur_year."'||TO_CHAR(U.PERSONAL_BIRTHDAY, '-MM-DD') < TO_CHAR(SYSDATE, 'YYYY-MM-DD')
						THEN '".($cur_year + 1)."'||TO_CHAR(U.PERSONAL_BIRTHDAY, '-MM-DD')
					ELSE '".($cur_year)."'||TO_CHAR(U.PERSONAL_BIRTHDAY, '-MM-DD')
					END
				".$dir;
			}
			elseif($field == "IS_ONLINE")
			...
			elseif($s = $obUserFieldsSqlSelect->GetOrder($field))
			{
		...
		$userFieldsSelect = $obUserFieldsSqlSelect->GetSelect();
		$arSqlSearch[] = $obUserFieldsSqlWhere->GetFilter();
		$strSqlSearch = GetFilterSqlSearch($arSqlSearch);

		$sSelect = implode(', ',$arSelectFields)." ".$userFieldsSelect;
		...
		$strFrom = "
			FROM
				b_user U
				".$obUserFieldsSqlWhere->GetJoin("U.ID")."
				".$strJoin."
			WHERE
				".$strSqlSearch."
			";
		...
		if($obUserFieldsSqlWhere->GetDistinct())
		{
			$strSql = "
				SELECT ".$sSelect."
				FROM
					b_user U
					".$obUserFieldsSqlSelect->GetJoin("U.ID")."
					INNER JOIN (SELECT DISTINCT U.ID ".$strFrom.") U2 ON U2.ID = U.ID
					".$strJoin."
				".$strSqlOrder."
			";
		}
		...
		if(isset($arParams["NAV_PARAMS"]) && is_array($arParams["NAV_PARAMS"]))
		{
			...
			{
				$res_cnt = $DB->Query("SELECT COUNT(".($obUserFieldsSqlWhere->GetDistinct() ? "DISTINCT " : "")."U.ID) as C ".$strFrom);
	...
	function IsOnLine($id, $interval = 120)
	{
		...
		$dbRes = $DB->Query("SELECT 'x' FROM b_user WHERE ID = ".$id." AND LAST_ACTIVITY_DATE > (sysdate-".$interval."/86400)");
...
class CGroup extends CAllGroup
{
	...
	function Add($arFields)
	{
		...
		$ID = $arFields["ID"] = $DB->NextID("sq_b_group");
	...
	function GetList(&$by, &$order, $arFilter=Array(), $SHOW_USERS_AMOUNT="N")
	{
		...
		$strSql = "
			...
			".$strSqlSearch."
			GROUP BY
				G.ID, G.ACTIVE, G.C_SORT, G.TIMESTAMP_X, G.ANONYMOUS, G.NAME, G.DESCRIPTION, G.STRING_ID
			HAVING
				1=1
				".$strSqlSearch_h."
			".$strSqlOrder;
