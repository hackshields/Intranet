...
class CRatings extends CAllRatings
{
	...
	function BuildRating($ID)
	{
		...
		if ($arRating && $arRating['ACTIVE'] == 'Y')
		{
			...
			// If the current value has changed, then update the field previous value
			$strSql = "
				UPDATE
					b_rating_results RR
				SET
					RR.PREVIOUS_VALUE = (
						CASE WHEN
							RR.CURRENT_VALUE = (
								SELECT	".$sqlFunc."(CURRENT_VALUE) CURRENT_VALUE
								FROM b_rating_component_results RC
								WHERE RC.RATING_ID = RR.RATING_ID and RC.ENTITY_ID = RR.ENTITY_ID
								GROUP BY RC.ENTITY_ID
							)
						THEN RR.PREVIOUS_VALUE
						ELSE RR.CURRENT_VALUE
						END
					)
				WHERE
					RR.RATING_ID=".$ID."
			...
			if ($arRating['POSITION'] == 'Y') {
				// If the current position has changed, then update the field previous position
				$strSql = "
					UPDATE
						b_rating_results RR
					SET
						RR.PREVIOUS_POSITION = (
							CASE WHEN
								RR.CURRENT_POSITION = (
									SELECT CURRENT_POSITION FROM (
										SELECT row_number() over (ORDER BY CURRENT_VALUE DESC NULLS LAST) CURRENT_POSITION, ENTITY_ID
										FROM b_rating_results
										WHERE RATING_ID = ".$ID." AND CURRENT_VALUE > 0
									) RR1
									WHERE RR1.ENTITY_ID = RR.ENTITY_ID
								)
							THEN RR.PREVIOUS_POSITION
							ELSE RR.CURRENT_POSITION
							END
						)
					WHERE
						RR.RATING_ID=".$ID."
				...
				if ($sRatingWeightType == 'auto')
				{
					...
					$strSql =  "
						UPDATE
							b_rating_user RU
						SET
							RU.VOTE_COUNT = ".intval($ratingCountVote)."+(
								SELECT CURRENT_VALUE
								FROM b_rating_results RR
								WHERE RR.RATING_ID = ".$ID."
								AND RR.ENTITY_ID = RU.ENTITY_ID
							),
							RU.VOTE_WEIGHT = ".$voteWeight." + 1*(
								SELECT CURRENT_VALUE
								FROM b_rating_results RR
								WHERE RR.RATING_ID = ".$ID."
								AND RR.ENTITY_ID = RU.ENTITY_ID
	...
	function DeleteByUser($ID)
	{
		...
		$strSql = "
			UPDATE
				b_rating_voting RV
			SET (RV.TOTAL_VALUE, RV.TOTAL_VOTES, RV.TOTAL_POSITIVE_VOTES, RV.TOTAL_NEGATIVE_VOTES) = (
				SELECT
					SUM(case when RVV.USER_ID <> $ID then RVV.VALUE else 0 end) as TOTAL_VALUE,
					SUM(case when RVV.USER_ID <> $ID then 1 else 0 end) as TOTAL_VOTES,
					SUM(case when RVV.VALUE > 0 AND RVV.USER_ID <> $ID then 1 else 0 end) as TOTAL_POSITIVE_VOTES,
					SUM(case when RVV.VALUE < 0 AND RVV.USER_ID <> $ID then 1 else 0 end) as TOTAL_NEGATIVE_VOTES
				FROM b_rating_vote RVV
				WHERE RV.ID = RVV.RATING_VOTING_ID
				GROUP BY RVV.RATING_VOTING_ID
			)
			WHERE
				RV.ID IN (
	...
	function AddComponentResults($arComponentConfigs)
	{
		...
		$strSql  = "
			...
			SET LAST_CALCULATED = ".$DB->GetNowFunction().",
				NEXT_CALCULATION = ".$DB->GetNowFunction().'+'.$arComponentConfigs['REFRESH_INTERVAL']."/86400
	...
	function GetCommunityInfo($ratingId)
	{
		...
		if (IsModuleInstalled("forum"))
		{
			$strModulesSql .= "
					...
					FROM b_forum_topic
					WHERE START_DATE > SYSDATE-".intval($communityLastVisit)."
					GROUP BY USER_START_ID
				UNION ALL
					SELECT AUTHOR_ID as ENTITY_ID
					FROM b_forum_message
					WHERE POST_DATE > SYSDATE-".intval($communityLastVisit)."
					GROUP BY AUTHOR_ID
				UNION ALL
			";
		}
		if (IsModuleInstalled("blog"))
		{
			$strModulesSql .= "
					SELECT	AUTHOR_ID as ENTITY_ID
					FROM b_blog_post
					WHERE DATE_PUBLISH > SYSDATE-".intval($communityLastVisit)."
					GROUP BY AUTHOR_ID
				UNION ALL
					SELECT AUTHOR_ID as ENTITY_ID
					FROM b_blog_comment
					WHERE DATE_CREATE > SYSDATE-".intval($communityLastVisit)."
					GROUP BY AUTHOR_ID
				UNION ALL";
		}
		...
		if (!empty($strModulesSql))
		{
			$strModulesSql = "
				(
					...
					FROM b_rating_vote
					WHERE CREATED > SYSDATE-".intval($communityLastVisit)."
					GROUP BY USER_ID
				) MS,
			";
		}
		...
		if ($bAllGroups || empty($arGroups))
		{
			$strSql .= "
				...
				U.ACTIVE = 'Y'
				AND U.LAST_LOGIN > SYSDATE-".intval($communityLastVisit)."
			";
		}
		else
		{
	...
	function SetAuthorityDefaultValue($arParams)
	{
		...
		{
			$strSql =  "UPDATE b_rating_user SET BONUS = 0 WHERE RATING_ID IN (".implode(',', $arRatingList).")";
			$res = $DB->Query($strSql, false, $err_mess.__LINE__);
			$strSql =  "
				UPDATE
					b_rating_user RU
				SET
					RU.BONUS = 0+(
						...
						WHERE FUP.TO_USER_ID = RU.ENTITY_ID
						GROUP BY TO_USER_ID
					)
				WHERE
			...
			if (isset($arParams['DEFAULT_CONFIG_NEW_USER']) && $arParams['DEFAULT_CONFIG_NEW_USER'] == 'Y')
			{
				...
				$strSql =  "UPDATE b_rating_user SET BONUS = ".$DB->IsNull('BONUS', '0')."+".$ratingStartValue." WHERE RATING_ID IN (".implode(',', $arRatingList).")";
				$res = $DB->Query($strSql, false, $err_mess.__LINE__);
			}
		}
