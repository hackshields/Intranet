...
class CUser extends CAllUser
{
	...
	function GetList(&$by, &$order, $arFilter=Array(), $arParams = Array())
	{
		...
		{
			//MSSQL 2000 Help: if SELECT DISTINCT is specified, or if the SELECT statement contains a UNION operator, the sort columns must appear in the select list
			...
			foreach ($arParams['FIELDS'] as $field)
			{
				...
				elseif ($field == 'IS_ONLINE')
					$arSelectFields[$field] =	"case when U.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE())	then 'Y' else 'N' end AS IS_ONLINE";
		...
		if (empty($arSelectFields))
		{
			...
			$arSelectFields['IS_ONLINE'] =	"case when U.LAST_ACTIVITY_DATE > DATEADD(second, -".$online_interval.", GETDATE())	then 'Y' else 'N' end AS IS_ONLINE";
		...
		foreach ($arOrder as $field => $dir)
		{
			if (strtoupper($field) == "CURRENT_BIRTHDAY")
			{
				$arSelectFields['CURRENT_BIRTHDAY'] = "case
			...
			foreach ($arFilter as $key => $val)
			{
				...
				switch($key)
				{
					...
					case "TIMESTAMP_2":
						...
						break;
					case "TIMESTAMP_X_1":
						$arSqlSearch[] = "U.TIMESTAMP_X>=".$DB->CharToDateFunction($val, "FULL");
						break;
					case "TIMESTAMP_X_2":
						$arSqlSearch[] = "U.TIMESTAMP_X<=".$DB->CharToDateFunction($val, "FULL");
						break;
					...
					case "DATE_REGISTER_1":
						$arSqlSearch[] = "U.DATE_REGISTER >= ".$DB->CharToDateFunction($val, "SHORT");
						break;
					case "DATE_REGISTER_2":
						$arSqlSearch[] = "U.DATE_REGISTER < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						break;
					case "ACTIVE":
					...
					case "CONFIRM_CODE":
						if($val <> '')
							...
							$arSqlSearch[] = "(U.CONFIRM_CODE IS NULL OR LEN(U.CONFIRM_CODE) <= 0)";
						break;
					case "!CONFIRM_CODE":
						if($val <> '')
							$arSqlSearch[] = "U.CONFIRM_CODE <> '".$DB->ForSql($val)."'";
						else
							$arSqlSearch[] = "(U.CONFIRM_CODE IS NOT NULL AND LEN(U.CONFIRM_CODE) > 0)";
						break;
					case "COUNTRY_ID":
					case "WORK_COUNTRY":
					...
					case "NAME_SEARCH":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
						$arSqlSearch[] = GetFilterQuery("U.NAME, U.LAST_NAME, U.SECOND_NAME, U.EMAIL, U.LOGIN", $val, $match);
						break;
					case "EMAIL":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
					...
					case "GROUPS_ID":
						...
						if(is_array($val) && count($val)>0)
						{
							...
							$arSqlSearch[] =
								"EXISTS(SELECT 'x' FROM b_user_group UG
								WHERE UG.USER_ID=U.ID
									and UG.GROUP_ID in (".implode(",", $ar).")
					...
					case "PERSONAL_BIRTHDAY_DATE":
						$arSqlSearch[] = " ((REPLICATE('0',2-LEN(DATEPART(mm, U.PERSONAL_BIRTHDAY)))+CONVERT(varchar(2),DATEPART(mm, U.PERSONAL_BIRTHDAY))+'-'+REPLICATE('0',2-LEN(DATEPART(dd, U.PERSONAL_BIRTHDAY)))+CONVERT(varchar(2),DATEPART(dd, U.PERSONAL_BIRTHDAY))) = '".$DB->ForSql($val)."')";
						break;
					case "KEYWORDS":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
					...
					case "LAST_ACTIVITY":
						...
						elseif (intval($val)>0)
							$arSqlSearch[] = "U.LAST_ACTIVITY_DATE > DATEADD(second, -".intval($val).", GETDATE())";
						break;
					case "!LAST_ACTIVITY":
						if ($val === false)
	...
	function IsOnLine($id, $interval = 120)
	{
		...
		$dbRes = $DB->Query("SELECT 'x' FROM b_user WHERE ID = ".$id." AND LAST_ACTIVITY_DATE > DATEADD(second, -".$interval.", GETDATE())");
