...
class CRatings extends CAllRatings
{
		...
		if ($arRating && $arRating['ACTIVE'] == 'Y')
		{
			...
			$strSql =  "
					UPDATE
						b_rating_results
					SET
						b_rating_results.PREVIOUS_VALUE = (case when b_rating_results.CURRENT_VALUE = RCR.CURRENT_VALUE then b_rating_results.PREVIOUS_VALUE else b_rating_results.CURRENT_VALUE end),
						b_rating_results.CURRENT_VALUE = RCR.CURRENT_VALUE
					FROM
					...
					WHERE
						b_rating_results.RATING_ID=".$ID."
					and	b_rating_results.ENTITY_TYPE_ID = RCR.ENTITY_TYPE_ID
					and	b_rating_results.ENTITY_ID = RCR.ENTITY_ID
					";
			$res = $DB->Query($strSql, false, $err_mess.__LINE__);
			...
			if ($arRating['POSITION'] == 'Y') {
				$strSql =  "
					UPDATE
						b_rating_results
					SET
						b_rating_results.PREVIOUS_POSITION = (case when b_rating_results.CURRENT_POSITION = RP.POSITION then b_rating_results.PREVIOUS_POSITION else b_rating_results.CURRENT_POSITION end),
						b_rating_results.CURRENT_POSITION = RP.POSITION
					FROM
						(	SELECT ENTITY_TYPE_ID, ENTITY_ID, CURRENT_VALUE, row_number() over (ORDER BY CURRENT_VALUE DESC) POSITION
							FROM b_rating_results
							WHERE RATING_ID = ".$ID."
						) RP
					WHERE
						b_rating_results.RATING_ID=".$ID."
					and	b_rating_results.ENTITY_TYPE_ID = RP.ENTITY_TYPE_ID
					and	b_rating_results.ENTITY_ID = RP.ENTITY_ID
					";
				$res = $DB->Query($strSql, false, $err_mess.__LINE__);
			}
				...
				if ($sRatingWeightType == 'auto')
				{
					...
					$strSql =  "
						UPDATE
							b_rating_user
						SET
							b_rating_user.VOTE_COUNT = ".intval($ratingCountVote)."+RP.CURRENT_VALUE,
							b_rating_user.VOTE_WEIGHT = RP.CURRENT_VALUE*".$voteWeight."
						FROM
						...
						WHERE
							b_rating_user.RATING_ID=".$ID."
							and	b_rating_user.ENTITY_ID = RP.ENTITY_ID
					";
					$res = $DB->Query($strSql, false, $err_mess.__LINE__);
				}
				else
				{
					...
					$strSql =  "
						UPDATE
							b_rating_user
						SET
							b_rating_user.VOTE_COUNT = RP.COUNT,
							b_rating_user.VOTE_WEIGHT = RP.WEIGHT
						FROM
							(	SELECT
	...
	function DeleteByUser($ID)
	{
		...
		$strSql =  "
			UPDATE
				b_rating_voting
			SET
				b_rating_voting.TOTAL_VALUE = RP.TOTAL_VALUE,
				b_rating_voting.TOTAL_VOTES = RP.TOTAL_VOTES,
				b_rating_voting.TOTAL_POSITIVE_VOTES = RP.TOTAL_POSITIVE_VOTES,
				b_rating_voting.TOTAL_NEGATIVE_VOTES = RP.TOTAL_NEGATIVE_VOTES
			FROM
				(
					SELECT
						RATING_VOTING_ID,
			...
			WHERE
				b_rating_voting.ID = RP.RATING_VOTING_ID
		";
		$res = $DB->Query($strSql, false, $err_mess.__LINE__);
	...
	function AddResults($arResults)
	{
		...
		{
			$arFields = array(
				'RATING_ID' 	 => IntVal($arResult['RATING_ID']),
				'ENTITY_TYPE_ID' => $arResult['ENTITY_TYPE_ID'],
				'ENTITY_ID' 	 => $arResult['ENTITY_ID'],
				'CURRENT_VALUE'  => $arResult['CURRENT_VALUE'],
				'PREVIOUS_VALUE' => $arResult['PREVIOUS_VALUE'],
			);
			$DB->Add("b_rating_results", $arFields);
		}
	...
	function AddComponentResults($arComponentConfigs)
	{
		...
		$strSql  = "
			...
			SET LAST_CALCULATED = ".$DB->GetNowFunction().",
				NEXT_CALCULATION = dateadd(second, ".intval($arComponentConfigs['REFRESH_INTERVAL']).", getdate())
	...
	function GetCommunityInfo($ratingId)
	{
		...
		if (IsModuleInstalled("forum"))
		{
			$strModulesSql .= "
					...
					FROM b_forum_topic
					WHERE START_DATE > dateadd(day, -".intval($communityLastVisit).", getdate())
					GROUP BY USER_START_ID
				UNION ALL
					SELECT AUTHOR_ID as ENTITY_ID
					FROM b_forum_message
					WHERE POST_DATE > dateadd(day, -".intval($communityLastVisit).", getdate())
					GROUP BY AUTHOR_ID
				UNION ALL
			";
		}
		if (IsModuleInstalled("blog"))
		{
			$strModulesSql .= "
					SELECT	AUTHOR_ID as ENTITY_ID
					FROM b_blog_post
					WHERE DATE_PUBLISH > dateadd(day, -".intval($communityLastVisit).", getdate())
					GROUP BY AUTHOR_ID
				UNION ALL
					SELECT AUTHOR_ID as ENTITY_ID
					FROM b_blog_comment
					WHERE DATE_CREATE > dateadd(day, -".intval($communityLastVisit).", getdate())
					GROUP BY AUTHOR_ID
				UNION ALL";
		}
		...
		if (!empty($strModulesSql))
		{
			$strModulesSql = "
				(
					...
					FROM b_rating_vote
					WHERE CREATED > dateadd(day, -".intval($communityLastVisit).", getdate())
					GROUP BY USER_ID
				) MS,
			";
		}
		...
		if ($bAllGroups || empty($arGroups))
		{
			$strSql .= "
				...
				U.ACTIVE = 'Y'
				AND U.LAST_LOGIN > dateadd(day, -".intval($communityLastVisit).", getdate())
			";
		}
		else
		{
			...
			if(!array_key_exists($userId, $cacheAllowVote))
			{
				...
				if ($arInfo['RESULT'] && $sVoteType == 'A')
				{
					$strSql = '
						...
						AND RV.CREATED > dateadd(day, -1, getdate())';
					$res = $DB->Query($strSql, false, $err_mess.__LINE__);
					$countVote = $res->Fetch();
		...
		if (isset($arParams['DEFAULT_USER_ACTIVE']) && $arParams['DEFAULT_USER_ACTIVE'] == 'Y' && IsModuleInstalled("forum") && is_array($arRatingList) && !empty($arRatingList))
		{
			...
			$strSql =  "
				UPDATE
					b_rating_user
				SET
					b_rating_user.BONUS = ".$DB->IsNull('RP.CNT', '0')."+".$ratingStartValue."
				FROM
					(	SELECT
				...
				WHERE
					b_rating_user.RATING_ID IN (".implode(',', $arRatingList).")
					and	b_rating_user.ENTITY_ID = RP.ENTITY_ID
			";
			$res = $DB->Query($strSql, false, $err_mess.__LINE__);
		}
	...
	function AutoAssignGroup($groupId, $authorityValueAdd, $authorityValueDelete)
	{
		...
		$strSql = "
			DELETE ug
			FROM b_user_group as ug
			WHERE ug.GROUP_ID = $groupId AND EXISTS
			(SELECT
								rr.ENTITY_ID as USER_ID
							FROM
								b_rating_results rr
								INNER JOIN b_user_group ug2 ON ug2.GROUP_ID = $groupId AND ug2.USER_ID = rr.ENTITY_ID
							WHERE
		...
		)
		{
			...
			$sqlStr = "
				...
				WHERE
					...
					and RV.USER_ID = U.ID
					".($bplus? " and RV.VALUE > 0 ": " and RV.VALUE < 0 ")."
				GROUP BY RV.USER_ID, U.ID, RV.VALUE, RV.USER_ID, RV.ID
		...
		else
		{
			$sqlStr = "
				...
				WHERE
					...
					and RV.USER_ID = U.ID
					".($bplus? " and RV.VALUE > 0 ": " and RV.VALUE < 0 ")."
				GROUP BY RV.USER_ID, U.ID, U.NAME, U.LAST_NAME, U.SECOND_NAME, U.LOGIN, U.PERSONAL_PHOTO, RV.VALUE, RV.USER_ID, RV.ID
				ORDER BY ".($bIntranetInstalled? "RV.VALUE desc, RANK DESC, RV.ID DESC": "RANK DESC, RV.VALUE desc, RV.ID DESC");
		}
