...
class CDatabase extends CAllDatabase
{
	...
	function DoConnect()
	{
		...
		if(!$this->db_Conn)
		{
			$s = (DBPersistent && !$this->bNodeConnection? "sqlsrv_connect(p)":"sqlsrv_connect");
			$errors = "";
			...
			{
				echo "<br><font color=#ff0000>Error! ".$s."('-', '-', '-')</font><br>#".nl2br(htmlspecialcharsbx($errors))."<br>";
			}
...
class CDBResult extends CAllDBResult
{
	...
	function NavQuery($strSql, $cnt, $arNavStartParams, $bIgnoreErrors = false)
	{
		...
		if(!$this->NavShowAll)
		{
			...
			{
				if(!sqlsrv_fetch($res_tmp->result))
					return null;
			}
