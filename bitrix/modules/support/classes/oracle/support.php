...
class CTicket extends CAllTicket
{
	...
	function AutoClose()
	{
		...
		WHERE
			T.AUTO_CLOSE_DAYS > 0
		and T.DATE_CLOSE is null
		and	(sysdate - T.LAST_MESSAGE_DATE) > T.AUTO_CLOSE_DAYS
		";
	...
	function AddMessage($ticketID, $arFields, &$arrFILES, $checkRights="Y")
	{
		if (strlen($arFields["MESSAGE"])>0 || (is_array($arFields["FILES"]) && count($arFields["FILES"])>0))
		{
			...
			if (intval($MID)>0)
			{
				...
				if ( $log!="Y" )
				{
					CSupportSearch::writeWordsInTable( $MID, $siteID, $T_TITLE . " " . $arFields["MESSAGE"] );
				}
	...
	function GetList(&$by, &$order, $arFilter=Array(), &$isFiltered, $checkRights="Y", $getUserName="Y", $getExtraNames="Y", $siteID=false, $arParams = Array() )
	{
			...
			for ($i=0; $i<count($filterKeys); $i++)
			{
				...
				switch($key)
				{
					...
					case "AUTO_CLOSE_DAYS_LEFT1":
						$arSqlSearch[] = "ROUND(T.TIMESTAMP_X + T.AUTO_CLOSE_DAYS - sysdate)>='".intval($val)."'";
						break;
		...
		if (strlen($siteID)>0)
		{
			$dates_select = "
				...
				".$DB->DateToCharFunction("T.TIMESTAMP_X + T.AUTO_CLOSE_DAYS","FULL",$siteID,true)."	AUTO_CLOSE_DATE
			";
		}
		else
		{
			$dates_select = "
				...
				".$DB->DateToCharFunction("T.TIMESTAMP_X + T.AUTO_CLOSE_DAYS","FULL")."		AUTO_CLOSE_DATE
			";
		}
		...
		$strSql = "
			SELECT
				...
				ROUND(T.TIMESTAMP_X + T.AUTO_CLOSE_DAYS - sysdate)		AUTO_CLOSE_DAYS_LEFT,
			...
			FROM
				b_ticket T
			LEFT JOIN b_ticket_online TN ON (TN.TICKET_ID = T.ID and TN.TIMESTAMP_X >= SYSDATE - $onlineInterval/86400)
			$u_join
			$d_join
			$ugroup_join
		...
		$isFiltered = (IsFiltered($strSqlSearch) || $messSearch=="Y");
		return $res;
	}
