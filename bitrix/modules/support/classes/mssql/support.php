...
class CTicket extends CAllTicket
{
	...
	{
		return "isnull(" . $field . "," . $alternative . ")";
	}
	...
	function AutoClose()
	{
		...
		/*$strSql = "
			...
			and datediff(day, T.TIMESTAMP_X, getdate()) > T.AUTO_CLOSE_DAYS
			";*/
		...
		$strSql = "
			...
			WHERE
				T.AUTO_CLOSE_DAYS > 0
			and T.DATE_CLOSE is null
			and datediff(day, T.LAST_MESSAGE_DATE, getdate()) > T.AUTO_CLOSE_DAYS
			";
	...
	function CleanUpOnline()
	{
		...
		$strSql = "
			DELETE FROM b_ticket_online WHERE
				TIMESTAMP_X < dateadd(second, - $onlineInterval, getdate())
			";
	...
	function GetOnline($ticketID)
	{
		...
		$strSql = "
			SELECT
				...
				U.EMAIL											USER_EMAIL,
				U.LOGIN											USER_LOGIN,
				isnull(U.NAME,'')+' '+isnull(U.LAST_NAME,'')	USER_NAME
			FROM
				b_ticket_online T
			INNER JOIN b_user U ON (U.ID = T.USER_ID)
			WHERE
				T.TICKET_ID = $ticketID
			and T.TIMESTAMP_X >= dateadd(second, - $onlineInterval, getdate())
			GROUP BY
	...
	function UpdateMessage($messageID, $arFields, $checkRights="Y")
	{
		...
		if ($bAdmin=="Y")
		{
			...
			$arFieldsU = array(
				...
				"OWNER_SID"			=> "'".$DB->ForSql($arFields["OWNER_SID"], 255)."'",
			...
			$rows = $DB->Update("b_ticket_message",$arFieldsU,"WHERE ID='".$messageID."'",$err_mess.__LINE__);
			if (intval($rows)>0)
			{
				...
				if ($arMessage = $rsMessage->Fetch())
				{
					...
					$notImageExtensionSuffix = COption::GetOptionString("support", "NOT_IMAGE_EXTENSION_SUFFIX");
					$notImageUploadDir = COption::GetOptionString("support", "NOT_IMAGE_UPLOAD_DIR");
					$maxSize = COption::GetOptionString("support", "SUPPORT_MAX_FILESIZE");
					...
					if (is_array($arrFiles) && count($arrFiles)>0)
					{
						foreach ($arrFiles as $arFile)
						{
							...
							{
								if ($bSupportTeam!="Y" && $bAdmin!="Y") $max_file_size = intval($maxSize)*1024;
								$fes = "";
								$uploadDir = "support";
								if (!CFile::IsImage($arFile["name"], $arFile["type"]))
								{
									$fes = $notImageExtensionSuffix;
									$arFile["name"] .= $fes;
									$uploadDir = $notImageUploadDir;
								}

								if (!array_key_exists("MODULE_ID", $arFile) || strlen($arFile["MODULE_ID"]) <= 0)
									$arFile["MODULE_ID"] = "support";

								$fid = intval(CFile::SaveFile($arFile, $uploadDir, $max_file_size));
								...
								if ($fid>0)
								{
									...
									else // иначе
									{
										...
										$DB->Update("b_ticket_message_2_file", $arFieldsFu, "WHERE FILE_ID = ".intval($arFile["old_file"]),$err_mess.__LINE__);
									}
								}
							}
						}
					...
					if ( $log!="Y" )
					{
						...
						CSupportSearch::writeWordsInTable( $messageID, $arTicket["SITE_ID"], $arTicket["TITLE"] . " " . $arFields["MESSAGE"] );
					}
				}
			}
		}
	...
	function AddMessage($ticketID, $arFields, &$arrFILES, $checkRights="Y")
	{
		if (strlen($arFields["MESSAGE"])>0 || (is_array($arFields["FILES"]) && count($arFields["FILES"])>0))
		{
			...
			if (intval($ownerGuestID)<=0) $ownerGuestID = "null";
			...
			if (intval($createdGuestID)<=0) $createdGuestID = "null";
			...
			CTimeZone::Disable();
			$arFields_i["DATE_CREATE"] = $DB->CharToDateFunction( GetTime( time() ,"FULL" ) );
			CTimeZone::Enable();
			...
			{
				$arFields_i["CURRENT_RESPONSIBLE_USER_ID"] = $currentResponsibleUserID;
			}
			...
			if (intval($MID)>0)
			{
				...
				if ($notChangeStatus!="Y" && $hidden!="Y" && $log!="Y")
				{
					...
					if (!isset($arFields["AUTO_CLOSE_DAYS"])) $resetAutoClose = "Y";

					CTicket::UpdateLastParams($ticketID, $resetAutoClose, $changeLastMessageDate, true);
	...
	function GetStatus($ticketID)
	{
		...
		if ($bSupportTeam=="Y" || $bAdmin=="Y" || $bDemo=="Y")
		{
			$lamp = "
				case
					when T.DATE_CLOSE is not null then 'grey'
					else
						case
							when isnull(T.LAST_MESSAGE_USER_ID,0)='$uid' then 'green'
							else
								case
									when isnull(T.OWNER_USER_ID,0)='$uid' then 'red'
									else
										case
											when T.LAST_MESSAGE_BY_SUPPORT_TEAM='Y' then 'green_s'
											else
												case
													when isnull(T.RESPONSIBLE_USER_ID,0)='$uid' then 'red'
													else 'yellow'
												end
										end
								end
	...
	function GetList(&$by, &$order, $arFilter=Array(), &$isFiltered, $checkRights="Y", $getUserName="Y", $getExtraNames="Y", $siteID=false, $arParams = Array() )
	{
			...
			for ($i=0; $i<count($filterKeys); $i++)
			{
				...
				switch($key)
				{
					...
					case "DATE_CREATE_2":
						if (CheckDateTime($val))
						{
							$arSqlSearch[] = "T.DATE_CREATE < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						}
						break;
					...
					case "DATE_TIMESTAMP_2":
						if (CheckDateTime($val))
						{
							$arSqlSearch[] = "T.TIMESTAMP_X < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						}
						break;
					case "DATE_CLOSE_1":
						if (CheckDateTime($val))
					...
					case "DATE_CLOSE_2":
						if (CheckDateTime($val))
						{
							$arSqlSearch[] = "T.DATE_CLOSE < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						}
						break;
					case "CLOSE":
					...
					case "AUTO_CLOSE_DAYS2":
						...
						break;
					case "TICKET_TIME_1":
						$arSqlSearch[] = "datediff(second, T.DATE_CREATE, T.DATE_CLOSE) >= '".(intval($val)*86400)."'";
						break;
					case "TICKET_TIME_2":
						$arSqlSearch[] = "datediff(second, T.DATE_CREATE, T.DATE_CLOSE) <= '".(intval($val)*86400)."'";
						break;
					...
					case "AUTO_CLOSE_DAYS_LEFT1":
						$arSqlSearch[] = "
							datediff(day, getdate(), dateadd(day, T.AUTO_CLOSE_DAYS, T.TIMESTAMP_X)) >= '".intval($val)."'
							";
						break;
		...
		{
			$strSqlOrder = "ORDER BY CAST(T.LAST_MESSAGE_DATE as datetime)";
		}
		...
		{
			$strSqlOrder = "ORDER BY CAST(T.DATE_CREATE as datetime)";
		}
		elseif ($by == "s_timestamp")
		{
			$strSqlOrder = "ORDER BY CAST(T.TIMESTAMP_X as datetime)";
		}
		elseif ($by == "s_date_close")
		{
			$strSqlOrder = "ORDER BY CAST(T.DATE_CLOSE as datetime)";
		}
		elseif ($by == "s_owner")
		{
		...
		else
		{
			...
			$strSqlOrder = "ORDER BY IS_SUPER_TICKET DESC, T.IS_OVERDUE DESC, T.IS_NOTIFIED DESC, CAST (T.LAST_MESSAGE_DATE as datetime)";
		}
		if ($order!="asc")
		{
			$strSqlOrder .= " desc ";
		...
		if ($mess_search=="Y")
		{
			$strSqlSearch_m = GetFilterSqlSearch($arSqlSearch_m);
			if ($bSupportTeam=="Y" || $bAdmin=="Y" || $bDemo=="Y")
			{
				$mess_exists = "
					and EXISTS (
						SELECT 'x' FROM b_ticket_message M WHERE
							M.TICKET_ID = T.ID and $strSqlSearch_m
					)
					";
			}
			else
			{
				$mess_exists = "
					and EXISTS (
						SELECT 'x' FROM b_ticket_message M WHERE
							M.TICKET_ID = T.ID and M.IS_HIDDEN='N' and M.IS_LOG='N' and $strSqlSearch_m
					)
					";
			}
		}
		if ($getUserName=="Y")
		{
			$u_select = "
				...
				isnull(UO.NAME,'')+' '+isnull(UO.LAST_NAME,'')				OWNER_NAME,
				UR.LOGIN													RESPONSIBLE_LOGIN,
				UR.EMAIL													RESPONSIBLE_EMAIL,
				isnull(UR.NAME,'')+' '+isnull(UR.LAST_NAME,'')				RESPONSIBLE_NAME,
				UM.LOGIN													MODIFIED_BY_LOGIN,
				UM.EMAIL													MODIFIED_BY_EMAIL,
				isnull(UM.NAME,'')+' '+isnull(UM.LAST_NAME,'')				MODIFIED_BY_NAME,
				UM.LOGIN													MODIFIED_LOGIN,
				UM.EMAIL													MODIFIED_EMAIL,
				isnull(UM.NAME,'')+' '+isnull(UM.LAST_NAME,'')				MODIFIED_NAME,
				UL.LOGIN													LAST_MESSAGE_LOGIN,
				UL.EMAIL													LAST_MESSAGE_EMAIL,
				isnull(UL.NAME,'')+' '+isnull(UL.LAST_NAME,'')				LAST_MESSAGE_NAME,
				UC.LOGIN													CREATED_LOGIN,
				UC.EMAIL													CREATED_EMAIL,
				isnull(UC.NAME,'')+' '+isnull(UC.LAST_NAME,'')				CREATED_NAME
			";
			$u_join = "
		...
		if (strlen($siteID)>0)
		{
			$dates_select = "
				...
				".$DB->DateToCharFunction("dateadd(day, T.AUTO_CLOSE_DAYS, T.TIMESTAMP_X)","FULL",$siteID,true)."	AUTO_CLOSE_DATE
			";
		}
		else
		{
		...
		$strSql = "
			SELECT
				...
				T.SITE_ID												LID,
				$dates_select ,
				datediff(second, T.DATE_CREATE, T.DATE_CLOSE)			TICKET_TIME,
				datediff(day, getdate(), dateadd(day, T.AUTO_CLOSE_DAYS, T.TIMESTAMP_X))	AUTO_CLOSE_DAYS_LEFT,
			...
			FROM
				b_ticket T
			LEFT JOIN b_ticket_online TN ON (TN.TICKET_ID = T.ID and TN.TIMESTAMP_X >= dateadd(second, - $onlineInterval, getdate()))
			$u_join
			$d_join
			$messJoin
			$ugroup_join
		...
		$isFiltered = (IsFiltered($strSqlSearch) || $mess_search=="Y");
		return $res;
	}
	...
	function GetSupportTeamList()
	{
		...
		$strSql = "
			SELECT DISTINCT
				...
				isnull(U.LAST_NAME,'') + ' '+isnull(U.NAME,'') +' ('+U.LOGIN+') '+ '['+convert(varchar(8000),U.ID)+']' as REFERENCE,
				isnull(U.LAST_NAME,''), isnull(U.NAME,''), U.LOGIN, U.ACTIVE
			FROM
				b_user U,
				b_user_group G
			WHERE
			...
			ORDER BY isnull(U.LAST_NAME,''), isnull(U.NAME,''), U.LOGIN
			";
		$res = $DB->Query($strSql, false, $err_mess.__LINE__);
		return $res;
	}
	...
	/*function GetResponsibleList($user_id)
	{
		...
		$strSql = "
			SELECT DISTINCT
				U.ID as ID,
				U.LOGIN as LOGIN,
				isnull(U.LAST_NAME,'') + ' '+isnull(U.NAME,'') +' ('+U.LOGIN+') '+ '['+convert(varchar(8000),U.ID)+']' as NAME,
				U.EMAIL as EMAIL
			FROM
				b_user U,
	...
	function GetMessageList(&$by, &$order, $arFilter=Array(), &$isFiltered, $checkRights="Y", $getUserName="Y")
	{
		...
		if ($getUserName=="Y")
		{
			$u_select = "
				...
				isnull(UO.NAME,'')+' '+isnull(UO.LAST_NAME,'')			OWNER_NAME,
				UO.LOGIN												LOGIN,
				isnull(UO.NAME,'')+' '+isnull(UO.LAST_NAME,'')			NAME,
				UC.EMAIL												CREATED_EMAIL,
				UC.LOGIN												CREATED_LOGIN,
				isnull(UC.NAME,'')+' '+isnull(UC.LAST_NAME,'')			CREATED_NAME,
				UM.EMAIL												MODIFIED_EMAIL,
	...
	function GetDynamicList(&$by, &$order, $arFilter=Array())
	{
		...
		$strSql = "
			SELECT
				...
				sum(case when T.DATE_CLOSE is null then 1 else 0 end)	OPEN_TICKETS,
				sum(case when T.DATE_CLOSE is null then 0 else 1 end)	CLOSE_TICKETS,
				datepart(day, T.DAY_CREATE)			CREATE_DAY,
				datepart(month, T.DAY_CREATE)		CREATE_MONTH,
				datepart(year, T.DAY_CREATE)		CREATE_YEAR
			FROM
				b_ticket T
	...
	function GetMessageDynamicList(&$by, &$order, $arFilter=Array())
	{
		...
		if (is_array($arFilter))
		{
			...
			for ($i=0; $i<count($filterKeys); $i++)
			{
				...
				switch($key)
				{
					...
					case "DATE_CREATE_2":
						if (CheckDateTime($val))
							$arSqlSearch[] = "M.DATE_CREATE < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
						break;
					case "OWNER":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $matchValueSet) ? "N" : "Y";
		...
		$strSqlSearch = GetFilterSqlSearch($arSqlSearch);
		if ($by == "s_date_create") $strSqlOrder = "ORDER BY max(M.DAY_CREATE)";
		else
		{
			$by = "s_date_create";
			$strSqlOrder = "ORDER BY max(M.DAY_CREATE)";
		}
		if ($order!="asc")
		{
			$strSqlOrder .= " desc ";
		...
		$strSql = "
			SELECT
				count(M.ID)								COUNTER,
				sum(case when M.EXPIRE_AGENT_DONE='Y' then 1 else 0 end)	COUNTER_OVERDUE,
				datepart(day, M.DAY_CREATE)				CREATE_DAY,
				datepart(month, M.DAY_CREATE)			CREATE_MONTH,
				datepart(year, M.DAY_CREATE)			CREATE_YEAR
			FROM
				b_ticket_message M
			INNER JOIN b_ticket T ON (T.ID = M.TICKET_ID)
