...
class SiteSettingsStep extends CWizardStep
{
	...
	function InitStep()
	{
		...
		if (strlen(COption::GetOptionString("extranet", "extranet_site")) > 0)
		{
			...
			$rsSites = CSite::GetList($by="sort", $order="desc", array("ID"=>$siteID));
		...
		{
			if (strlen(COption::GetOptionString("main", "site_name")) > 0)
				$siteName = COption::GetOptionString("main", "site_name").GetMessage("site_name_suffix");
			else
		...
		$wizard->SetDefaultVars(
			Array(
				"siteName" => $siteName,
				"siteID" => COption::GetOptionString("extranet", "extranet_site", "co"),
				"siteFolder" => "/extranet/",				
			)
		);
	...
	function OnPostForm()
	{
		...
		COption::SetOptionString("main", "wizard_site_logo_extranet", "");
...
class DataInstallStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		if(WIZARD_IS_RERUN === true)
		{
			...
			if($wizard->GetVar("installStructureData")!="Y" && !file_exists(WIZARD_SITE_PATH.".superleft.menu.php"))
			{
				$s = Array();
...
class FinishStep extends CWizardStep
{
	...
	function ShowStep()
	{
		...
		{
			$rsSites = CSite::GetByID($ExtranetSiteID);
			if ($arSite = $rsSites->Fetch())
				$dir = $arSite["DIR"];
		}
