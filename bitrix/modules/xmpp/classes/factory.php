...
class CXMPPFactory
{
	...
	public function OnSocNetMessagesAdd($ID, $arFields)
	{
		...
		if (IsModuleInstalled("im") && CModule::IncludeModule("im"))
		{
			...
			&& !CIMNotifySchema::CheckEnableFeature($arFields["NOTIFY_MODULE"], $arFields["NOTIFY_EVENT"], IM_FEATURE_XMPP))
				return false;
	...
	public function SendUnreadMessages($receiverJId, $domain = "")
	{
		...
		if (IsModuleInstalled("im") && CModule::IncludeModule("im"))
		{
			...
			$arMessage = $CIMMessage->GetUnreadMessage(false, null, "ASC");
			if ($arMessage['result'])
			{
			...
			$arNotify = $CIMNotify->GetUnreadNotify(false, "ASC");
			if ($arNotify['result'])
			{
				foreach ($arNotify['original_notify'] as $id => $arNotify)
				{
					if (isset($arNotify["NOTIFY_MODULE"]) && isset($arNotify["NOTIFY_EVENT"])
					&& !CIMNotifySchema::CheckEnableFeature($arNotify["NOTIFY_MODULE"], $arNotify["NOTIFY_EVENT"], IM_FEATURE_XMPP))
						continue;
